...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: VIE_AD_Ledger_EstadoResultados_2019
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[VIE_AD_Ledger_EstadoResultados_2019]
AS
     SELECT CASE
                WHEN cco.Code LIKE 'N%'
                THEN 'Neiva'
                WHEN cco.Code LIKE 'F%'
                THEN 'Florencia'
                WHEN cco.Code LIKE 'T%'
                THEN 'Tunja'
                WHEN cco.Code LIKE 'P%'
                THEN 'Pitalito'
                WHEN cco.Code LIKE 'K%'
                THEN 'Facatativá'
                WHEN cco.Code LIKE 'M%'
                THEN 'Nacional'
                WHEN cco.Code LIKE 'U%'
                THEN 'UTMedisalud'
                WHEN cco.Code LIKE 'E%'
                THEN 'NorteNeivaESIMED'
            END AS Sede, 
            RTRIM(c.Number) + RTRIM(cco.Code) AS CuentaCentroCosto, 
         ...
         WHERE(Number = LEFT(c.Number, 6))
            ...
            CASE
                WHEN sc.Month = '11'
                     AND sc.Year = '2019'
                THEN CAST((SUM(sc.DebitValue) - SUM(sc.CreditValue)) AS MONEY)
            END AS Noviembre2019,
            CASE
                WHEN sc.Month = '12'
                     AND sc.Year = '2019'
                THEN CAST((SUM(sc.DebitValue) - SUM(sc.CreditValue)) AS MONEY)
            END AS Diciembre2019
