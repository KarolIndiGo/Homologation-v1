...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: VIE_AD_Payments_TrasladoCuentas
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[VIE_AD_Payments_TrasladoCuentas]
AS
SELECT tc.Code AS Consecutivo, F.Code + ' - ' + F.Name AS [Unidad de radicación], source.Code + ' - ' + source.Name AS [Unidad radicación origen], target.Code + ' - ' + target.Name AS [Unidad radicación destino], T.Nit + ' - ' + T.Name AS Tercero, O.UnitCode + ' - ' + O.UnitName AS Sede, 
           ...
           --CASE WHEN SecondLastName LIKE '%NVA' THEN 'Neiva' WHEN SecondLastName LIKE '%TJA' THEN 'Tunja' WHEN SecondLastName LIKE '%FLA' THEN 'Florencia' WHEN SecondLastName LIKE '%BTA' THEN 'Bogotá' WHEN SecondLastName LIKE '%PTO' THEN 'Pitalito' END AS Origen, 
		   A.Balance AS SaldoFactura, 
...
WHERE (A.CreationDate >= '01/04/2020 00:00:00')
