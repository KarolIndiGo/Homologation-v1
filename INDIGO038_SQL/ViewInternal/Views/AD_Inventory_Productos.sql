...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: AD_Inventory_Productos
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[AD_Inventory_Productos] AS
...
pr.CodeCUM AS [Código CUM], pr.CodeAlternative AS [ATC], pr.CodeAlternativeTwo AS [Código alterno], 
		   ...
		   case SanitaryRegistration when 1 then 'Vigente' 
									 ...
									 when 10 then 'Inactivo' end as [Estado R. Sanitario],	pr.ExpirationDate as FechaVencimientoRegSanitario, pr.Abbreviation as Abreviacion, IUM, 
								...
								lote.BatchCode AS Lote, lote.ExpirationDate AS FechaVencimientoLote
FROM   Inventory.InventoryProduct AS pr  INNER JOIN
           ...
           Inventory.PackagingUnit AS ue  ON ue.Id = pr.PackagingUnitId left outer join --INNER JOIN
           ...
           Inventory.ATC AS catc  ON catc.Id = pr.ATCId left outer join
		  [Inventory].[InventorySupplie] as ins on ins.id=pr.SupplieId left outer join 
		   GeneralLedger.GeneralLedgerIVA AS IVA ON IVA.ID=PR.IVAId left join
		   Inventory.PhysicalInventory as ph WITH (NOLOCK) ON ph.ProductId = pr.Id LEFT OUTER JOIN
		   Inventory.BatchSerial as lote WITH (NOLOCK) ON lote.Id = ph.BatchSerialId 
		   --WHERE pr.Code='300101080'
--WHERE (pr.Code NOT LIKE '800-%') --and pr.Status='1'-- and pr.code='25351-01'
--where pr.Code='20036851-02'

--select * from Inventory.InventoryProduct
--where code = '20036851-02'
