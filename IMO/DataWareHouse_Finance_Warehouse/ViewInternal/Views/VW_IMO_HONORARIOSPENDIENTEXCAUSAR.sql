...
-- Schema: ViewInternal
-- Object: VW_IMO_HONORARIOSPENDIENTEXCAUSAR
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[IMO_HonorariosPendienteXCausar]
AS
...
SELECT
    ...
    CASE dos.Presentation WHEN '1' THEN 'No Quirúrgico' WHEN '2' THEN 'Quirúrgico' WHEN '3' THEN 'Paquete' END AS [Presentación Servicio],
    ServiciosIPSQ.Code AS Subcodigo,
    ServiciosIPSQ.Name AS Subnombre,
    CASE WHEN dq.PerformsHealthProfessionalCode IS NULL THEN dos.PerformsHealthProfessionalCode ELSE dq.PerformsHealthProfessionalCode END AS CodigoMèdico,
...
INNER JOIN [INDIGO035].[dbo].[ADINGRESO] AS ing
    ON CONVERT(VARCHAR(20), ing.NUMINGRES) = F.AdmissionNumber
...
LEFT OUTER JOIN [INDIGO035].[dbo].[INPROFSAL] AS med
    ON med.CODPROSAL = dos.PerformsHealthProfessionalCode
LEFT OUTER JOIN [INDIGO035].[dbo].[INESPECIA] AS espmed
    ON espmed.CODESPECI = med.CODESPEC1
LEFT OUTER JOIN [INDIGO035].[dbo].[INDIAGNOS] AS diag
    ON diag.CODDIAGNO = ing.CODDIAEGR
LEFT OUTER JOIN [INDIGO035].[dbo].[HCREGEGRE] AS salida
    ON CONVERT(VARCHAR(20), salida.NUMINGRES) = F.AdmissionNumber
...
LEFT OUTER JOIN [INDIGO035].[dbo].[INESPECIA] AS espqx
...
INNER JOIN [INDIGO035].[MedicalFees].[HealthProfessionalContract] AS cm
    ON med.CODPROSAL = cm.HealthProfessionalCode
   AND cm.LiquidateDefault = 1
INNER JOIN [INDIGO035].[MedicalFees].[MedicalFeesContract] AS CP
    ON cm.MedicalFeesContractId = CP.Id
WHERE F.Status = '1'
  AND DF.Id NOT IN (
        SELECT InvoiceDetailId
        FROM [INDIGO035].[MedicalFees].[MedicalFeesCausation]
        WHERE Status <> 4
  );