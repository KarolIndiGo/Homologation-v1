...
-- Schema: ViewInternal
-- Object: VW_IMO_AD_INVENTORY_ALMACENES
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.IMO_AD_Inventory_Almacenes
AS

SELECT  pr.Id AS IdProducto, pr.Code AS Código, pr.Name AS Producto, 
           CASE pr.ProductTypeId WHEN '2' THEN 'MEDICAMENTO' WHEN '3' THEN 'DISPOSITIVO MÉDICO' WHEN '4' THEN 'ELEMENTOS DE CONSUMO' WHEN '5' THEN 'ALIMENTACION ESPECIAL' END AS [Tipo Producto], 
...
FROM   [INDIGO035].[Inventory].[InventoryProduct] AS pr  LEFT OUTER JOIN
		   ...
		   [INDIGO035].[Inventory].[InventorySupplie] as sp  on sp.Id=pr.SupplieId
