...
-- Schema: ViewInternal
-- Object: VW_IMO_V_TSR_FLUJOCAJA
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.IMO_V_TSR_FlujoCaja
AS
...
SELECT  RC.Code, T .Nit, T .Name AS Tercero, 'Ingresos' AS 'Ingreso/Egreso', CASE RC.CollectType WHEN 1 THEN 'Caja' WHEN 2 THEN 'Bancos' END Afecta, CASE WHEN RC.IdCashRegister = 1 AND RC.IdBankAccount IS NULL 
        ...
        THEN B.Name + ' - ' + CB.Number WHEN RC.IdBankAccount = 9 AND RC.IdCashRegister IS NULL THEN B.Name + ' - ' + CB.Number END AS 'Caja/Cuenta', RC.DocumentDate AS FechaDocumento, CONVERT(DECIMAL(19,4), RC.Value, 
        100) AS VlrGeneral, CRC.Name AS Concepto, CU.Number AS CuentaContable, CONVERT(DECIMAL(19,4), RCD.Value, 100) AS ValorDetalle, CASE RCD.Nature WHEN 1 THEN 'Debito' WHEN 2 THEN 'Credito' END AS Naturaleza, 
        '' AS 'Metodo', '' AS 'Numero', '' AS 'CuentaDestino', '' AS 'NombreDestino', ' ' AS 'Beneficiario', UO.UnitName AS Sede, RC.Detail AS Detalle
FROM    [INDIGO035].[Treasury].[CashReceipts] RC JOIN
        [INDIGO035].[Treasury].[CashReceiptDetails] RCD ON RCD.IdCashReceipt = RC.Id AND RC.Status = 2 JOIN
        [INDIGO035].[Common].[ThirdParty] T ON RC.IdThirdParty = T .Id LEFT JOIN
        [INDIGO035].[Treasury].[CashRegisters] C ON RC.IdCashRegister = C.Id LEFT JOIN
        [INDIGO035].[Treasury].[EntityBankAccounts] CB ON RC.IdBankAccount = CB.Id LEFT JOIN
        [INDIGO035].[Payroll].[Bank] B ON CB.IdBank = B.Id JOIN
        [INDIGO035].[Treasury].[CashReceiptConcepts] CRC ON RCD.IdCashReceiptConcept = CRC.Id JOIN
        [INDIGO035].[GeneralLedger].[MainAccounts] CU ON CRC.IdMainAccount = CU.Id JOIN
        [INDIGO035].[Common].[OperatingUnit] UO ON RC.OperatingUnitId = UO.Id
UNION ALL
SELECT  CE.Code, T .Nit, T .Name AS Tercero, CASE CE.VoucherClass WHEN 1 THEN 'Pagos' WHEN 2 THEN 'Reembolso' WHEN 3 THEN 'Traslados' END AS TipoComp, 
        ...
        THEN B.Name + ' - ' + CB.Number WHEN CE.IdEntityBankAccount = 9 AND CE.IdCashRegister IS NULL THEN B.Name + ' - ' + CB.Number END AS 'Caja/Cuenta', CE.DocumentDate AS FechaDocumento, CONVERT(DECIMAL(19,4), 
        ...
        CASE WHEN (CE.VoucherClass <> 3) THEN CONVERT(DECIMAL(19,4), CED.Value, 101) * - 1 WHEN CE.VoucherClass = 3 THEN CONVERT(DECIMAL(19,4), CED.Value, 101) END AS VlrDetalle, 
...
FROM    [INDIGO035].[Treasury].[VoucherTransaction] CE JOIN
        [INDIGO035].[Treasury].[VoucherTransactionDetails] CED ON CED.IdVoucherTransaction = CE.Id AND CE.Status = 2 AND CE.VoucherClass IN (1, 2) AND CE.ExpenseType <> 2  LEFT JOIN
        [INDIGO035].[Common].[ThirdParty] T ON CE.IdThirdParty = T .Id LEFT JOIN
        [INDIGO035].[Treasury].[CashRegisters] C ON CE.IdCashRegister = C.Id LEFT JOIN
        [INDIGO035].[Treasury].[EntityBankAccounts] CB ON CE.IdEntityBankAccount = CB.Id LEFT JOIN
        [INDIGO035].[Payroll].[Bank] B ON CB.IdBank = B.Id LEFT JOIN
        [INDIGO035].[Treasury].[ExpenseConcepts] CRC ON CED.IdExpenseConcept = CRC.Id LEFT JOIN
        [INDIGO035].[GeneralLedger].[MainAccounts] CU ON CE.IdMainAccount = CU.Id LEFT JOIN
        [INDIGO035].[Common].[OperatingUnit] UO ON CE.IdUnitOperative = UO.Id LEFT JOIN
        [INDIGO035].[GeneralLedger].[MainAccounts] CU1 ON CED.IdMainAccount = CU1.Id
UNION ALL
SELECT  N .Code, T .Nit, T .Name AS Tercero, 'Notas' AS 'Ingreso/Egreso', CASE N .NoteType WHEN 1 THEN 'Nota a Cuenta Bancaria ' END Afecta, 
        ...
        CONVERT(DECIMAL(19,4), N .Value, 100) AS VlrGeneral, CN.Description AS Concepto, CU.Number AS CuentaContable, (CONVERT(DECIMAL(19,4), ND.Value, 101) * - 1) AS ValorDetalle, 
...
FROM    [INDIGO035].[Treasury].[TreasuryNote] N JOIN
        [INDIGO035].[Treasury].[TreasuryNoteDetail] ND ON ND.TreasuryNoteId = N .Id AND N .Status = 2 AND N .NoteType = 1 LEFT JOIN
        [INDIGO035].[Common].[ThirdParty] T ON ND.ThirdPartyId = T .Id LEFT JOIN
        [INDIGO035].[Treasury].[EntityBankAccounts] CB ON N .EntityBankAccountId = CB.Id LEFT JOIN
        [INDIGO035].[Payroll].[Bank] B ON CB.IdBank = B.Id JOIN
        [INDIGO035].[Treasury].[NoteConcepts] CN ON ND.NoteConceptId = CN.Id JOIN
        [INDIGO035].[GeneralLedger].[MainAccounts] CU ON ND.MainAccountId = CU.Id JOIN
        [INDIGO035].[Common].[OperatingUnit] UO ON N .OperatingUnitId = UO.Id
