...
-- Schema: ViewInternal
-- Object: VW_IMO_LISTADO_TALENTO_HUMANO
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[IMO_Listado_Talento_Humano]
AS
...
LEFT OUTER JOIN (
    SELECT MAX(Id) AS idciu, IdPerson
    FROM [INDIGO035].[Common].[Address]
    GROUP BY IdPerson
) AS DE ON DE.IdPerson = p.Id
...
LEFT OUTER JOIN [INDIGO035].[Payroll].[Contract] AS co
    ON co.EmployeeId = E.Id AND co.Valid = '1'
...
LEFT OUTER JOIN [INDIGO035].[Payroll].[ContractType] AS ct
    ON co.ContractTypeId = ct.Id
LEFT OUTER JOIN [INDIGO035].[Payroll].[Group] AS g
    ON co.GroupId = g.Id
LEFT OUTER JOIN [INDIGO035].[Payroll].[CostCenter] AS cc
...
LEFT OUTER JOIN [INDIGO035].[Payroll].[FundContract] AS fc
    ON fc.ContractId = co.Id AND fc.FundType = '1' AND fc.State = '1'
...
LEFT OUTER JOIN [INDIGO035].[Payroll].[FundContract] AS fc1
    ON fc1.ContractId = co.Id AND fc1.FundType = '3' AND fc1.State = '1'
...
LEFT OUTER JOIN [INDIGO035].[Common].[Phone] AS TEL
    ON TEL.IdPerson = p.Id AND TEL.IdPhoneType = '12'
LEFT OUTER JOIN [INDIGO035].[Common].[Phone] AS mov
    ON mov.IdPerson = p.Id AND mov.IdPhoneType = '13'
...
LEFT OUTER JOIN [INDIGO035].[Payroll].[ProfessionalRisk] AS PSR
    ON PSR.Id = ca.ProfessionalRiskLevelId 
--left outer join [INDIGO035].[Common].[PersonStudy] as pp on pp.personid=p.id 
--left outer join [INDIGO035].[Payroll].[StudyCenter] as ps on ps.id=pp.studycenterid
WHERE (E.State = '1')
  AND (p.State = '1')
  AND (co.Status = '1')
  AND (co.Valid = '1')
