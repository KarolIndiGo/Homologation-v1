...
-- Schema: ViewInternal
-- Object: VW_IMO_AD_INVENTORY_INFERIDO
-- Extracted by Fabric SQL Extractor SPN v3.9.0



CREATE   VIEW ViewInternal.VW_IMO_AD_INVENTORY_INFERIDO
AS
...
SELECT Sucursal,
       ...
       Hijos,
       CASE 
        WHEN Cod_Producto LIKE '0%' THEN 'Medicamento' 
        ELSE 'Insumo' 
        END AS Tipo,
       CostoPromedio,
       Ene22,
       ...
       Nov23,
       Dic23,
       Promedio AS ConsumoPromedio,
      [200], [001],[002],[100],[101],
			[003],[102],[203],[103],[104],[105],[106],[107],
[108],[109],
       InventarioFisico,
       ceiling (Promedio * 1.55) AS Punto_Pedido,
       CASE
...
FROM (SELECT PROD.Sucursal,
             ...
             isnull (sum (PROD.Ene22), 0) AS Ene22,
             isnull (sum (PROD.Feb22), 0) AS Feb22,
             isnull (sum (PROD.Mar22), 0) AS Mar22,
             isnull (sum (PROD.Abr22), 0) AS Abr22,
             isnull (sum (PROD.May22), 0) AS May22,
             isnull (sum (PROD.Jun22), 0) AS Jun22,
             isnull (sum (PROD.Jul22), 0) AS Jul22,
             isnull (sum (PROD.Ago22), 0) AS Ago22,
             isnull (sum (PROD.Sep22), 0) AS Sep22,
             isnull (sum (PROD.Oct22), 0) AS Oct22,
             isnull (sum (PROD.Nov22), 0) AS Nov22,
             isnull (sum (PROD.Dic22), 0) AS Dic22,
             ...
             --Promedio
             CASE
                WHEN month (getdate ()) = 1 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Jul22), 0))
                      + (isnull (sum (PROD.Ago22), 0))
                      + (isnull (sum (PROD.Sep22), 0))
                      + (isnull (sum (PROD.Oct22), 0))
                      + (isnull (sum (PROD.Nov22), 0))
                      + (isnull (sum (PROD.Dic22), 0)))
                   / 6
                WHEN month (getdate ()) = 2 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Ago22), 0))
                      + (isnull (sum (PROD.Sep22), 0))
                      + (isnull (sum (PROD.Oct22), 0))
                      + (isnull (sum (PROD.Nov22), 0))
                      + (isnull (sum (PROD.Dic22), 0))
                      + (isnull (sum (PROD.Ene23), 0)))
                   / 6
                WHEN month (getdate ()) = 3 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Sep22), 0))
                      + (isnull (sum (PROD.Oct22), 0))
                      + (isnull (sum (PROD.Nov22), 0))
                      + (isnull (sum (PROD.Dic22), 0))
                      + (isnull (sum (PROD.Ene23), 0))
                      + (isnull (sum (PROD.Feb23), 0)))
                   / 6
                WHEN month (getdate ()) = 4 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Oct22), 0))
                      + (isnull (sum (PROD.Nov22), 0))
                      + (isnull (sum (PROD.Dic22), 0))
                      + (isnull (sum (PROD.Ene23), 0))
                      + (isnull (sum (PROD.Feb23), 0))
                      + (isnull (sum (PROD.Mar23), 0)))
                --   / 6
                WHEN month (getdate ()) = 5 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Nov22), 0))
                      + (isnull (sum (PROD.Dic22), 0))
                      + (isnull (sum (PROD.Ene23), 0))
                      + (isnull (sum (PROD.Feb23), 0))
                      + (isnull (sum (PROD.Mar23), 0))
                      + (isnull (sum (PROD.Abr23), 0)))
                   / 6
                WHEN month (getdate ()) = 6 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Dic22), 0))
                      + (isnull (sum (PROD.Ene23), 0))
                      + (isnull (sum (PROD.Feb23), 0))
                      + (isnull (sum (PROD.Mar23), 0))
                      + (isnull (sum (PROD.Abr23), 0))
                      + (isnull (sum (PROD.May23), 0)))
                   / 6
                WHEN month (getdate ()) = 7 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Ene23), 0))
                      + (isnull (sum (PROD.Feb23), 0))
                      + (isnull (sum (PROD.Mar23), 0))
                      + (isnull (sum (PROD.Abr23), 0))
                      + (isnull (sum (PROD.May23), 0))
                      + (isnull (sum (PROD.Jun23), 0)))
                   / 6
                WHEN month (getdate ()) = 8 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Feb23), 0))
                      + (isnull (sum (PROD.Mar23), 0))
                      + (isnull (sum (PROD.Abr23), 0))
                   ...
                   / 6
                WHEN month (getdate ()) = 9 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Mar23), 0))
                      + (isnull (sum (PROD.Abr23), 0))
                      + (isnull (sum (PROD.May23), 0))
                   ...
                   / 6
                WHEN month (getdate ()) = 10 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Abr23), 0))
                      + (isnull (sum (PROD.May23), 0))
                      + (isnull (sum (PROD.Jun23), 0))
                   ...
                   / 6
                WHEN month (getdate ()) = 11 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.May23), 0))
                      + (isnull (sum (PROD.Jun23), 0))
                      + (isnull (sum (PROD.Jul23), 0))
                   ...
                   / 6
                WHEN month (getdate ()) = 12 AND year (getdate ()) = 2023
                THEN
                     (  (isnull (sum (PROD.Jun23), 0))
                      + (isnull (sum (PROD.Jul23), 0))
                      + (isnull (sum (PROD.Ago23), 0))
			...
			+isnull (sum (ALMA.[109]), 0) 
             ...
             avg (ALMA.CostoPromedio) AS CostoPromedio,
               ...
               CASE WHEN SUM (PROD.Abr23) > 0 THEN 1 ELSE 0 END
             + CASE WHEN SUM (PROD.May23) > 0 THEN 1 ELSE 0 END
             + CASE WHEN SUM (PROD.Jun23) > 0 THEN 1 ELSE 0 END
             + CASE WHEN SUM (PROD.Jul23) > 0 THEN 1 ELSE 0 END
             + CASE WHEN SUM (PROD.Ago23) > 0 THEN 1 ELSE 0 END
             + CASE WHEN SUM (PROD.Sep23) > 0 THEN 1 ELSE 0 END AS Rotacion --+
      ...
      FROM (SELECT 
            ...
            FROM (SELECT Ene22,
                         ...
                         [Código Agrupador],
                         [Agrupador]
                  FROM [DataWareHouse_Finance].[ViewInternal].[VW_IMO_AD_INVENTORY_INFERIDO_INVENTARIO_PRODUCTO_2022]
                       AS PROD) AS PROD
...
[108],[109],
                         CostoPromedio,
                         [Código Agrupador]
                  FROM [DataWareHouse_Finance].[ViewInternal].[VW_IMO_AD_INVENTORY_ALMACENESPIVOT]
