...
from [INDIGO035].[Contract].[CareGroup] cg
inner join [INDIGO035].[Contract].[CareGroupDefinitionRate] cdr on cg.Id = cdr.CareGroupId
inner join [INDIGO035].[Contract].[DefinitionRate] dr on dr.Id = cdr.DefinitionRateId
inner join [INDIGO035].[Contract].[DefinitionRateDetail] drd on drd.DefinitionRateId = dr.Id
left join [INDIGO035].[Contract].[DefinitionRateDetailCondition] drdc on drdc.DefinitionRateDetailId = drd.Id
left join [INDIGO035].[Contract].[RateManual] rm on rm.Id = drd.RateManualId
left join [INDIGO035].[Contract].[RateManual] rmd on rmd.Id = drdc.RateManualId
