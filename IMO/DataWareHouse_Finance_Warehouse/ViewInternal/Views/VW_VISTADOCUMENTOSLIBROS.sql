...
from [INDIGO035].[GeneralLedger].[AccountingMovement] am
inner join (
...
inner join [INDIGO035].[GeneralLedger].[MainAccounts] ma on ma.Id = jvd.IdMainAccount
inner join [INDIGO035].[GeneralLedger].[JournalVoucherTypes] jvt on jvt.Id = jv.IdJournalVoucher
inner join [INDIGO035].[GeneralLedger].[LegalBook] lb on lb.Id = jv.LegalBookId
where lb.Id = 1
...
left join [INDIGO035].[GeneralLedger].[HomologationAccount] ha on ha.OfficialMainAccountId = movcol.IdAccount
left join (
