...
from [INDIGO035].[Treasury].[CashReceipts] cr
left join [INDIGO035].[Treasury].[CashRegisters] c on c.Id = cr.IdCashRegister
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.Id = cr.IdBankAccount
left join (
select EntityCode, sum(DebitValue) as Value
from [INDIGO035].[GeneralLedger].[JournalVouchers] jv
inner join [INDIGO035].[GeneralLedger].[JournalVoucherDetails] jvd on jv.Id = jvd.IdAccounting
inner join [INDIGO035].[GeneralLedger].[MainAccounts] ma on ma.Id = jvd.IdMainAccount and ma.LegalBookId = 1
...
from [INDIGO035].[Treasury].[VoucherTransaction] vt
left join [INDIGO035].[Treasury].[CashRegisters] c on c.Id = vt.IdCashRegister
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.Id = vt.IdEntityBankAccount
left join (
select EntityCode, sum(DebitValue) as Value
from [INDIGO035].[GeneralLedger].[JournalVouchers] jv
inner join [INDIGO035].[GeneralLedger].[JournalVoucherDetails] jvd on jv.Id = jvd.IdAccounting
...
from [INDIGO035].[Treasury].[TreasuryNote] tn
left join [INDIGO035].[Treasury].[CashRegisters] c on c.Id = tn.CashRegisterId
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.Id = tn.EntityBankAccountId
left join (
select EntityCode, sum(DebitValue) as Value
from [INDIGO035].[GeneralLedger].[JournalVouchers] jv
inner join [INDIGO035].[GeneralLedger].[JournalVoucherDetails] jvd on jv.Id = jvd.IdAccounting
...
from [INDIGO035].[Treasury].[TreasuryNote] tn
inner join [INDIGO035].[Treasury].[VoucherTransaction] vt on vt.Id = tn.VoucherTransactionId
left join [INDIGO035].[Treasury].[CashRegisters] c on c.Id = tn.CashRegisterId
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.Id = tn.EntityBankAccountId
left join (
select EntityCode, sum(DebitValue) as Value
...
from [INDIGO035].[Treasury].[TreasuryNote] tn
inner join [INDIGO035].[Treasury].[CashReceipts] cr on cr.Id = tn.CashReceiptId
left join [INDIGO035].[Treasury].[CashRegisters] c on c.Id = tn.CashRegisterId
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.Id = tn.EntityBankAccountId
left join (
select EntityCode, sum(DebitValue) as Value
...
from [INDIGO035].[Treasury].[Consignment] cs
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.Id = cs.EntityBankAccountId
left join (
select EntityCode, sum(DebitValue) as Value
from [INDIGO035].[GeneralLedger].[JournalVouchers] jv
inner join [INDIGO035].[GeneralLedger].[JournalVoucherDetails] jvd on jv.Id = jvd.IdAccounting
...
inner join [INDIGO035].[GeneralLedger].[MainAccounts] ma on ma.Id = jvd.IdMainAccount and ma.LegalBookId = 1
left join [INDIGO035].[Treasury].[CashRegisters] cr on cr.IdMainAccount = jvd.IdMainAccount
left join [INDIGO035].[Treasury].[EntityBankAccounts] eba on eba.IdMainAccount = jvd.IdMainAccount
where jvd.IdMainAccount in (select IdMainAccount from [INDIGO035].[Treasury].[CashRegisters] union all select IdMainAccount from [INDIGO035].[Treasury].[EntityBankAccounts])
