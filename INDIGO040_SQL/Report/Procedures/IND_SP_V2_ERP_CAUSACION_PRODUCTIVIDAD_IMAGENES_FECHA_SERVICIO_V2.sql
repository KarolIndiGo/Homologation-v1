...
-- ItemId: SPN
-- Schema: Report
-- Object: IND_SP_V2_ERP_CAUSACION_PRODUCTIVIDAD_IMAGENES_FECHA_SERVICIO_V2
-- Extracted by Fabric SQL Extractor SPN v3.9.0


--SET STATISTICS TIME ON;

CREATE PROCEDURE [Report].[IND_SP_V2_ERP_CAUSACION_PRODUCTIVIDAD_IMAGENES_FECHA_SERVICIO_V2]
...
AS
(
  SELECT distinct SOD.ServiceOrderId,sod.Id ServiceOrderDetailId,SO.code 'NRO ORDEN',ISNULL(RC.PatientCode,SO.PatientCode) AS 'IDENTIFICACION',
  ISNULL(RC.AdmissionNumber,SO.AdmissionNumber) AS 'INGRESO',CAST(SOD.ServiceDate AS DATE) 'FECHA SERVICIO',IPSS.[CODIGO SERVICIO IPS],IPSS.[NOMBRE SERVICIO IPS],
  ...
  CASE TIPOINGRE WHEN 1 THEN 'AMBULATORIO' WHEN 2 THEN 'HOSPITALARIO' END 'AMBITO',sod.RateManualSalePrice [VALOR SERVICIO],
  ...
  SOD.IsDelete,ID.ID Id_DetFac,I.InvoiceNumber 'NRO FACTURA',CAST(I.InvoiceDate AS DATE) 'FECHA FACTURA',CAST(G.[FECHA LECTURA] AS DATE) 'FECHA LECTURA' ,IM.Agrupador
  ...
  LEFT JOIN DBO.SEGusuaru USUA WITH (NOLOCK) ON USUA.CODUSUARI = SO.AnnulmentUser
  ...
  LEFT JOIN MedicalFees.MedicalFeesContract MFC WITH (NOLOCK) ON MFC.Id = HPC.MedicalFeesContractId
  WHERE SOD.RecordType=1 AND IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' AND CUPS.ServiceType  in (3,4,5) 
...
(
 ...
 WHERE SOD.RecordType=1 AND SOD.Packaging=1  and IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' AND CUPS.ServiceType IN (3,4,5) 
 AND CAST(SOD.ServiceDate AS DATE) BETWEEN @FECHAINI AND  @FECHAFIN
 ),
...
(
 ...
 FROM Billing.ServiceOrder AS SO WITH (NOLOCK)
  ...
  WHERE SOD.RecordType=1 AND SOD.SettlementType IN (2,3) and IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' and CUPS.ServiceType IN (3,4,5) 
  AND CAST(SOD.ServiceDate AS DATE) BETWEEN @FECHAINI AND  @FECHAFIN
)

SELECT 'VIE' 'FUENTE',IMG.[TIPO SERVICIO],IMG.IDENTIFICACION,IMG.[FECHA SERVICIO],
...
IMG.[UNIDAD FUNCIONAL SERVICIO],IMG.[CODIGO CC],IMG.[CENTRO DE COSTO],
...
IMG.[PORCENTAJE CAUSACION], IMG.[TIPO] 'AGRUPADOR CAUSACION',CAST(IIF(IMG.[VALOR TOTAL]=0,IMG.[VALOR SERVICIO],IMG.[VALOR TOTAL])*(IMG.[PORCENTAJE CAUSACION]/100) AS NUMERIC(18,2)) 'VALOR CAUSADO',
...
INC.[DESCRIPCION INCLUIDO],IMG.[CODIGO CONCEPTO],IMG.[CONCEPTO FACTURACION],IMG.[OBTENER CC],IMG.[NRO ORDEN],IMG.[AUTO],
IMG.[AMBITO], IMG.[NIT],IMG.[ENTIDAD],IMG.[GRUPO ATENCION],IMG.[USUARIO FACTURO],IMG.[USUARIO ANULO],IMG.[FECHA LECTURA],
IMG.[AGRUPADOR]
