...
-- ItemId: SPN
-- Schema: Report
-- Object: IND_SP_V2_EHR_AGENDAMIENTO_CITAS_TOTAL_V4
...
CREATE PROCEDURE [Report].[IND_SP_V2_EHR_AGENDAMIENTO_CITAS_TOTAL_V4]
@FECHAINI DATE,
@FECHAFIN DATE
AS

--DECLARE @FECHAINI AS DATE='2025-03-03';
--DECLARE @FECHAFIN AS DATE='2025-03-05';
...
( --29684
 ...
 RTRIM(C.CODIPSSEC) AS [CODIGO IPS],C.CODCENATE,CASE TIPSOLICITU WHEN 1 THEN 'CITA MEDICA CONSULTORIO' ELSE 'OTROS' END [TIPO CITA],
...
( ---21006
 ...
 RTRIM(HEA.HealthEntityCode) 'CODIGO ENTIDAD ADMISION',RTRIM(HEA.Name) 'ENTIDAD ADMISION',CG.Name AS [GRUPO ATENCION ADMISION],A.CODSERIPS,UNI.UFUDESCRI,UNI.UFUTIPUNI,
 IIF(D.IPSEXOPAC=1,'N/A',CASE RIE.GESTACION WHEN 0 THEN 'N/A' when 1 then 'SI' when 2 then 'NO' when 21 then 'Riesgo no evaluado' ELSE 'Riesgo no evaluado' end) 'GESTACION'
 ...
 (
  ...
  LEFT JOIN DBO.HCRIESGOSP AS RIE WITH (NOLOCK) ON A.NUMINGRES=RIE.NUMINGRCES AND A.IPCODPACI=RIE.IPCODPACI
  LEFT JOIN dbo.INUNIFUNC AS UNI ON A.UFUCODIGO=UNI.UFUCODIGO
),
...
AS
(
 SELECT DISTINCT A.ID CODCONCEC,A.IPCODPACI,A.FECHAPRO,A.CODPROSAL,A.NUMINGRES,A.IDCITA,A.GENCAREGROUP,A.GENCONENTITY,A.FECHAREG,A.CODSERIPS,UNI.UFUDESCRI,UNI.UFUTIPUNI,
 ...
 IIF(D.IPSEXOPAC=1,'N/A',CASE RIE.GESTACION WHEN 0 THEN 'N/A' when 1 then 'SI' when 2 then 'NO' when 21 then 'Riesgo no evaluado' ELSE 'Riesgo no evaluado' end) 'GESTACION',
 'CITA PROCEDIMIENTOS CONSULTORIO' [TIPO CITA_SI]
...
SELECT DISTINCT  A.ID CODCONCEC,A.IPCODPACI,A.FECHAPRO,A.CODPROSAL,A.NUMINGRES,A.IDCITA,A.GENCAREGROUP,A.GENCONENTITY,A.FECHAREG,A.CODSERIPS,UNI.UFUDESCRI,UNI.UFUTIPUNI,
 RTRIM(HEA.HealthEntityCode) 'CODIGO ENTIDAD ADMISION',RTRIM(HEA.Name) 'ENTIDAD ADMISION',CG.Name AS [GRUPO ATENCION ADMISION],G2.IDHCHISPACA,A.CODESPECI,
 IIF(D.IPSEXOPAC=1,'N/A',CASE RIE.GESTACION WHEN 0 THEN 'N/A' when 1 then 'SI' when 2 then 'NO' when 21 then 'Riesgo no evaluado' ELSE 'Riesgo no evaluado' end) 'GESTACION',
 'CITA PROCEDIMIENTOS CONSULTORIO' [TIPO CITA_NO]
 FROM dbo.AMBORDOTROSPRO A WITH (NOLOCK)
 INNER JOIN dbo.INPACIENT AS D WITH (NOLOCK) ON A.IPCODPACI = D.IPCODPACI
 INNER JOIN
 (
...
AS
(
 SELECT HIS.IPCODPACI,HIS.NUMINGRES,HIS.FECHISPAC,HIS.ID,HIS.CODESPTRA,HIS.CODDIAGNO FROM 
  DBO.HCHISPACA HIS
  INNER JOIN
  (
...
(
SELECT 
CAST(DB_NAME() AS VARCHAR(9)) AS ID_COMPANY,CTE.[CENTRO ATENCION],CTE.[CODIGO IPS],
...
RTRIM(ISNULL(UNICE.UFUDESCRI,ISNULL(UNIID.UFUDESCRI,ISNULL(CTESI.UFUDESCRI,ISNULL(CTENO.UFUDESCRI,ISNULL(CTEOTROSI.UFUDESCRI,CTEOTRONO.UFUDESCRI)))))) AS [UNIDAD FUNCIONAL], 
CASE RTRIM(ISNULL(UNICE.UFUTIPUNI,ISNULL(UNIID.UFUTIPUNI,ISNULL(CTESI.UFUTIPUNI,ISNULL(CTENO.UFUTIPUNI,ISNULL(CTEOTROSI.UFUTIPUNI,CTEOTRONO.UFUTIPUNI))))))
	  ...
	  WHEN 24 THEN 'CONSULTA EXTERNA GINOCO OBSTETRICIA' WHEN 30 THEN 'AMBULATORIO' WHEN 31 THEN 'CONSULTA PRIORITARIA' ELSE 'AMBULATORIO' END  [AMBITO],
...
CASE WHEN A.CODTIPSOL = '0' THEN 'TELEFONICA' WHEN A.CODTIPSOL = '1' THEN 'PRESENCIAL' END AS [FORMA DE SOLICITUD],
ISNULL(IM.Agrupador,IIF(A.TIPSOLICITU=1,CASE A.CODTIPCIT WHEN 0 THEN 'PRIMERA VEZ' WHEN 1 THEN 'CONTROL' WHEN 2 THEN 'POS OPERATORIO' ELSE 'CONTROL' END,'CONTROL')) [TIPO DE CITA],
--FN V2 
CASE A.MODALIDAD WHEN 0 THEN 'PRESENCIAL' WHEN 1 THEN 'TELECONSULTA' ELSE 'PRESENCIAL' END AS [MODALIDAD],
...
CASE HEA.EntityType WHEN 1 THEN 'Contributivo' WHEN 2 THEN 'Subsidiado' WHEN 3 THEN 'ET Vinculados Municipios' WHEN 4 THEN 'ET Vinculados Departamentos'
...
WHEN 10 THEN 'Accidentes de transito' WHEN 11 THEN 'Fosyga' WHEN 12 THEN 'Particulares' ELSE 'Particulares' end [REGIMEN CITA], 
...
DIA.CODDIAGNO 'CIE10',DIA.NOMDIAGNO 'DIAGNOSTICO',CTE.[TIPO CITA],CTEOTROSI.[TIPO CITA_SI],CTEOTRONO.[TIPO CITA_NO],
ISNULL(HIS.FECHISPAC,HIS2.FECHISPAC) 'FECHA HISTORIA CLINICA'
FROM dbo.AGASICITA A WITH (NOLOCK)
INNER JOIN dbo.INPACIENT AS D WITH (NOLOCK) ON A.IPCODPACI = D.IPCODPACI
...
LEFT JOIN dbo.INDIAGNOS AS DIA WITH (NOLOCK) ON DIA.CODDIAGNO=ISNULL(HIS.CODDIAGNO,ISNULL(HIS2.CODDIAGNO,ISNULL(CODDIAEGR,CODDIAING)))
)

SELECT 
ID_COMPANY,[CENTRO ATENCION],[CODIGO IPS],CASE [TIPO AGENDAMIENTO] WHEN 'Cita Medica' THEN 'Cita Medica' WHEN 'Apoyo Diagnostico' THEN 'Cita Procedimientos' 
ELSE 'Otros' end [TIPO AGENDAMIENTO],[UNIDAD FUNCIONAL],[AMBITO],[CODIGO ESPECIALIDAD],[ESPECIALIDAD],[CODIGO MEDICO],[MEDICO],[ACTIVIDAD AGENDAMIENTO],
...
[GRUPO ATENCION CITA],[CODIGO CUPS],[DESCRIPCION CUPS],
...
IIF([CITA EXTRA]='Si',[FECHA DE SOLICITUD],ISNULL(CONVERT(VARCHAR(16),CAST(G3.FECHA_HORA_ASIGNACION AS DATETIME),20),[FECHA DE SOLICITUD])) [FECHA DE SOLICITUD SERVINTE],
IIF([CITA EXTRA]='Si',[FECHA SOLICITA PACIENTE],ISNULL(CONVERT(VARCHAR(16),CAST(G3.FECHA_HORA_USR_SOLICTA AS DATETIME),20),[FECHA SOLICITA PACIENTE])) [FECHA SOLICITA PACIENTE SERVINTE],
IIF([CITA EXTRA]='Si',[FECHA DE CITA],ISNULL(CONVERT(VARCHAR(16),CAST(G3.FECHA_HORA_CITA AS DATETIME),20),[FECHA DE CITA])) [FECHA DE CITA SERVINTE],
DATEDIFF(DAY,IIF([CITA EXTRA]='Si',[FECHA DE SOLICITUD],ISNULL(CONVERT(VARCHAR(16),CAST(G3.FECHA_HORA_ASIGNACION AS DATETIME),20),[FECHA DE SOLICITUD])),
IIF([CITA EXTRA]='Si',[FECHA DE CITA],ISNULL(CONVERT(VARCHAR(16),CAST(G3.FECHA_HORA_CITA AS DATETIME),20),[FECHA DE CITA]))) AS [OPORTUNIDAD DE CITA F_ASIGNACION VS F_CITA],
CIT.[CIE10],CIT.[DIAGNOSTICO],CONVERT(VARCHAR(16),CAST(CIT.[FECHA HISTORIA CLINICA] AS DATETIME),20) [FECHA HISTORIA CLINICA],G3.ID_CITA_SERVINTE,
CIT.[FECHA DE SOLICITUD],CIT.[FECHA SOLICITA PACIENTE],CIT.[FECHA DE CITA], CIT.[OPORTUNIDAD DE CITA FS VS FAC]
...
LEFT JOIN 
 ...
 (
  ...
  WHERE ([TIPO CITA]='CITA MEDICA CONSULTORIO' OR [TIPO CITA_SI]='CITA PROCEDIMIENTOS CONSULTORIO' OR [TIPO CITA_NO]='CITA PROCEDIMIENTOS CONSULTORIO')
  AND [UNIDAD FUNCIONAL] NOT IN ('IMAGENOLOGIA APOYO DIAGNOSTICO','BANCO DE SANGRE APOYO TERAPEUTICO','GINECOBSTETRICIA URGENCIAS','HEMODINAMIA')
  AND [ACTIVIDAD AGENDAMIENTO] NOT IN ('TRANSFUSION HEMOCOMPONENTES')
  --[ESPECIALIDAD] NOT IN ('RADIOLOGIA INTERVENCIONISTA','RADIOLOGIA E IMAGENES DIAGNOSTICAS')
  --and [NRO IDENTIFICACION]='1000128563'
