...
-- ItemId: SPN
-- Schema: Report
-- Object: IND_SP_V2_EHR_AGENDAMIENTO_CITAS_TOTAL_V2_ELIMINAR
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE PROCEDURE [Report].[IND_SP_V2_EHR_AGENDAMIENTO_CITAS_TOTAL_V2]
@FECHAINI DATE,
@FECHAFIN DATE
AS
...
( --29684
 ...
 WHERE CAST(A.FECHORAIN AS DATE) BETWEEN @FECHAINI AND @FECHAFIN --AND NUMINGRES IN ('E47A9F0ECB','72217A5C2C')
),
...
AS
( ---21006
 SELECT A.CODCONCEC,A.IPCODPACI,A.IPFECHCIT,A.CODPROSAL,A.NUMINGRES,A.NUMCONCIT,A.GENCAREGROUP,A.GENCONENTITY,A.FECREGSIS,A.IDHCHISPACA,A.FECCITACUMPLIDA,A.IdCitaSync,
...
AS
(
 SELECT A.ID CODCONCEC,A.IPCODPACI,A.FECHAPRO,A.CODPROSAL,A.NUMINGRES,A.IDCITA,A.GENCAREGROUP,A.GENCONENTITY,A.FECHAREG,A.CODSERIPS,
 RTRIM(HEA.HealthEntityCode) 'CODIGO ENTIDAD ADMISION',RTRIM(HEA.Name) 'ENTIDAD ADMISION',CG.Name AS [GRUPO ATENCION ADMISION],G2.IDHCHISPACA,
 IIF(D.IPSEXOPAC=1,'N/A',CASE RIE.GESTACION WHEN 0 THEN 'N/A' when 1 then 'SI' when 2 then 'NO' when 21 then 'Riesgo no evaluado' ELSE 'Riesgo no evaluado' end) 'GESTACION'
 ...
 (
  ...
  ) AS G2 ON G2.NUMINGRES=A.NUMINGRES AND G2.IPCODPACI=A.IPCODPACI AND CAST(G2.FECHAREGISTRO AS DATE)=CAST(A.FECHAPRO AS DATE)
  LEFT JOIN Contract.HealthAdministrator AS HEA WITH (NOLOCK) ON A.GENCONENTITY = HEA.Id
  LEFT JOIN Contract.CareGroup AS CG WITH (NOLOCK) ON A.GENCAREGROUP=CG.Id
  LEFT JOIN DBO.HCRIESGOSP AS RIE WITH (NOLOCK) ON A.NUMINGRES=RIE.NUMINGRCES AND A.IPCODPACI=RIE.IPCODPACI
),
...
--SELECT * FROM CTE_OTROS_PROCEDIMENTOS_NO
--SELECT * FROM CTE_OTROS_PROCEDIMENTOS_SI

--SELECT * FROM CTE_CONTROL_NO
...
CG.Name AS [GRUPO ATENCION CITA],--HEA2.HealthEntityCode [CODIGO ENTIDAD ADMISION],HEA2.Name [ENTIDAD ADMISION],CG2.Name [GRUPO ATENCION ADMISION],
...
CASE A.CODESTCIT WHEN 1 THEN 'CUMPLIDA'WHEN 2 THEN 'INCUMPLIDA'WHEN 3 THEN 'PREASIGNADA'WHEN 4 THEN 'CANCELADA' WHEN 5 THEN 'INATENCION' END AS [ESTADO ACTUAL],
A.CODCAUCAN AS [CODIGO CANCELACION],CAN.DESCAUCAN AS [DESCRIPCION CANCELACION],ING.NUMINGRES AS [INGRESO DE ATENCION],
ing.IAUTORIZA 'NUMERO AUTORIZACION',IIF(HIS.ID IS NOT NULL OR CTEOTROSI.IDHCHISPACA IS NOT NULL,'SI','NO') 'REGISTRO HISTORIA',
...
A.codautonu 'IDCITA' --A.NUMAUTORI AS [NUMERO AUTORIZACION],
FROM dbo.AGASICITA A WITH (NOLOCK)
INNER JOIN dbo.INPACIENT AS D WITH (NOLOCK) ON A.IPCODPACI = D.IPCODPACI
...
LEFT JOIN CTE_OTROS_PROCEDIMENTOS_NO CTEOTRONO ON CTEOTRONO.IPCODPACI=A.IPCODPACI AND CTEOTRONO.NUMINGRES=A.NUMINGRES AND CTEOTRONO.CODPROSAL=A.CODPROSAL
...
LEFT JOIN HCHISPACA AS HIS WITH (NOLOCK) ON HIS.ID=ISNULL(CTESI.IDHCHISPACA,CTENO.IDHCHISPACA) -- /*ING.NUMINGRES = HIS.NUMINGRES AND HIS.GENCONEXT = 1 AND  HIS.TIPHISPAC IN ('I','N') AND*/
