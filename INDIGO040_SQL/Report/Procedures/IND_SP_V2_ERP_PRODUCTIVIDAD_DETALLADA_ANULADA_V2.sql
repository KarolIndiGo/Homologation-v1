...
-- ItemId: SPN
-- Schema: Report
-- Object: IND_SP_V2_ERP_PRODUCTIVIDAD_DETALLADA_ANULADA_V2
...
CREATE PROCEDURE [Report].[IND_SP_V2_ERP_PRODUCTIVIDAD_DETALLADA_ANULADA_V2]
...
--****************CTE PARA SACAR LAS FACTURAS ANULADAS DEL MES DESEADO, DEBEN SER REGISTROS UNICOS DESDE CONTABILIDAD POR EL TIPO DE DOCUMENTO IPO FACTURA******************************--
--SET STATISTICS TIME ON;

WITH CTE_FACTURADOS_ANULADOS_TOTAL_UNICOS
AS
(
	...
	AND JV.LegalBookId =1 AND jv.EntityName ='Invoice'  AND IdJournalVoucher=@TIPOANULACION --AND F.Status=1
...
(
 ...
 FROM dbo.HCORDIMAG A
  INNER JOIN CTE_FACTURADOS_ANULADOS_TOTAL_UNICOS AS UNI ON CAST(UNI.AdmissionNumber AS CHAR)=CAST(A.NUMINGRES AS CHAR)
   INNER JOIN
   (
    SELECT A.IPCODPACI,A.NUMINGRES,A.CODSERIPS,A.GENSERVICEORDER,MAX(A.AUTO) AUTO
     FROM dbo.ADINGRESO ing with (nolock)
	  ...
	  INNER JOIN CTE_FACTURADOS_ANULADOS_TOTAL_UNICOS AS UNI ON CAST(UNI.AdmissionNumber AS CHAR)=CAST(ING.NUMINGRES AS CHAR)
...
(  --CA.CODIPSSEC 'CODIGO HABILITACION'
 ...
 I.InvoiceNumber AS [NRO FACTURA],CAST(I.InvoiceDate AS DATE) AS [FECHA FACTURA],UNI.[CIUDAD],
 UNI.[FECHA ANULACION],CAST(UNI.[FECHA ANULACION] AS DATE) AS 'FECHA BUSQUEDA',
 ...
 I.TotalInvoice AS [TOTAL FACTURA], TP.Nit AS NIT,TP.Name AS ENTIDAD,CG.Code AS [CODIGO GRUPO ATENCION],CG.Name AS [GRUPO ATENCION]
  FROM Billing.Invoice AS I WITH (NOLOCK)
   INNER JOIN CTE_FACTURADOS_ANULADOS_TOTAL_UNICOS AS UNI WITH (NOLOCK) ON UNI.EntityId =I.Id
...
CAST(I.InitialDate AS DATE) 'FECHA INGRESO/CORTE FACTURA',CAST(I.OutputDate AS DATE) 'FECHA EGRESO/CORTE FACTURA',
  ...
  ISNULL(IMA.[AGREMIACION],MFC.ContractName) 'AGREMIACION',CAST(UNI.VoucherDate AS DATE) AS 'FECHA BUSQUEDA',
...
CTE_SERVICIOS_PAQUETE_DETALLADO_ANULADO
AS
...
UNI.[NRO FACTURA],UNI.[FECHA FACTURA],UNI.[TOTAL FACTURA],UNI.[NIT],UNI.[ENTIDAD],UNI.[CODIGO GRUPO ATENCION],UNI.[GRUPO ATENCION],
	...
	RTRIM(ISNULL(MEDQX.NOMMEDICO,ISNULL(MED.NOMMEDICO,TPT.Name)))) 'PROFESIONAL REALIZA',ISNULL(IMA.[ESPECIALIDA REALIZO],
	ISNULL(ESPQX.DESESPECI,ESPMED.DESESPECI)) 'ESPECIALIDAD REALIZO',ISNULL(CUPS.[GRUPO CUPS],IPR.[GRUPO PRODUCTO]) 'GRUPO CUPS/PRODUCTO',
	ISNULL(CUPS.[SUBGRUPO CUPS],IPR.[SUBGRUPO PRODUCTO]) 'SUBGRUPO CUPS/PRODUCTO',ISNULL(CUPS.[CODIGO CONCEPTO],IPR.[CODIGO PRODUCTO]) 'CODIGO CONCEPTO FAC CUPS',
   ...
   INNER JOIN CTE_PAQUETE_UNICO_ANULADO AS UNI WITH (NOLOCK) ON UNI.ID=SOD.PackageServiceOrderDetailId
   INNER JOIN Billing.ServiceOrder AS SO WITH (NOLOCK) ON SO.Id =SOD.ServiceOrderId
...
SELECT * 
FROM CTE_FACTURA_ANULADA_DETALLADA --where NIT='900156264'
UNION ALL
SELECT *
FROM CTE_SERVICIOS_PAQUETE_DETALLADO_ANULADO --where NIT='900156264'
