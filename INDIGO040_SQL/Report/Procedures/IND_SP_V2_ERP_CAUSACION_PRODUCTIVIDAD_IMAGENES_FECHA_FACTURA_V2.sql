...
-- ItemId: SPN
-- Schema: Report
-- Object: IND_SP_V2_ERP_CAUSACION_PRODUCTIVIDAD_IMAGENES_FECHA_FACTURA_V2
-- Extracted by Fabric SQL Extractor SPN v3.9.0


--SET STATISTICS TIME ON;

CREATE PROCEDURE [Report].[IND_SP_V2_ERP_CAUSACION_PRODUCTIVIDAD_IMAGENES_FECHA_FACTURA_V2]
...
(  --CA.CODIPSSEC 'CODIGO HABILITACION'
 ...
 I.TotalInvoice AS [TOTAL FACTURA]
  FROM Billing.Invoice AS I WITH (NOLOCK)
   ...
   WHERE I.STATUS=1 AND CAST(I.InvoiceDate AS DATE) BETWEEN @FECHAINI AND @FECHAFIN AND SOD.RecordType=1 AND SOD.Packaging=1 and IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' 
...
(  --CA.CODIPSSEC 'CODIGO HABILITACION'
 ...
 CAST(I.OutputDate AS DATE) 'FECHA EGRESO/CORTE FACTURA',I.TotalInvoice AS [TOTAL FACTURA]
  FROM Billing.Invoice AS I WITH (NOLOCK)
   ...
   and IM.Agrupador='Imagenes'AND IM.ReportType='Causacion' and CUPS.ServiceType IN (3,4,5,6,7,8)
),
...
(
 ...
 FROM 
  ...
  LEFT JOIN MedicalFees.MedicalFeesContract MFC WITH (NOLOCK) ON MFC.Id = HPC.MedicalFeesContractId
   WHERE I.STATUS=1 AND SOD.RecordType=1 AND IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' AND CUPS.ServiceType  in (3,4,5,6,7,8) 
...
(
 ...
 WHERE SOD.RecordType=1 AND SOD.Packaging=1  and IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' AND CUPS.ServiceType IN (3,4,5,6,7,8) 
),
...
(
 ...
 FROM Billing.ServiceOrder AS SO WITH (NOLOCK)
  ...
  LEFT JOIN CTE_CUPS AS CUPS2 WITH (NOLOCK) ON CUPS2.Id=SODI.CUPSEntityId
   WHERE SOD.RecordType=1 AND SOD.SettlementType IN (2,3) and IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' and CUPS.ServiceType IN (3,4,5,6,7,8) 
),
-----*******************************************************************************************************************************************************************-----
-----************************************************************* SEGMENTO ANULADOS ***********************************************************************************-----
-----*******************************************************************************************************************************************************************-----
...
( 
 ...
 CAST(I.OutputDate AS DATE) 'FECHA EGRESO/CORTE FACTURA', I.TotalInvoice AS [TOTAL FACTURA]
  FROM Billing.Invoice AS I WITH (NOLOCK)
   ...
   INNER JOIN Contract.CUPSEntity AS CUPS WITH (NOLOCK) ON CUPS.Id=SOD.CUPSEntityId
   INNER JOIN REPORT.TableSocieties as IM on IM.Code=CUPS.code
   WHERE I.STATUS=2 AND CAST(I.AnnulmentDate AS DATE) BETWEEN @FECHAINI AND  @FECHAFIN AND SOD.RecordType=1 AND SOD.Packaging=1 and IM.Agrupador='Imagenes' 
   AND CUPS.ServiceType IN (3,4,5,6,7,8)
),
...
(
 ...
 FROM 
  ...
  AND HPC.LiquidateDefault = 1
  LEFT JOIN MedicalFees.MedicalFeesContract MFC WITH (NOLOCK) ON MFC.Id = HPC.MedicalFeesContractId
   WHERE I.STATUS=2 AND SOD.RecordType=1 AND IM.Agrupador='Imagenes' AND IM.ReportType='Causacion' AND CUPS.ServiceType  in (3,4,5,6,7,8) 
   AND CAST(I.AnnulmentDate AS DATE) BETWEEN @FECHAINI AND  @FECHAFIN
),

CTE_DATOS_IMAGENES_EMPAQUETADO_ANULADOS
AS
(
 SELECT SOD.ServiceOrderId,sod.Id ServiceOrderDetailId,so.status,SOD.CUPSEntityId,SOD.IPSServiceId,sod.PackageServiceOrderDetailId,ID.ID,I.InvoiceNumber 'NRO FACTURA',
...
CTE_DATOS_IMAGENES_INCLUIDO_ANULADOS
AS
(
 SELECT SOD.ServiceOrderId,sod.Id ServiceOrderDetailId,so.status,SOD.CUPSEntityId,SOD.IPSServiceId,SOD.SettlementType,SOD.IncludeServiceOrderDetailId,ID.ID 'Id_DetFacInc',
 I.InvoiceNumber 'NRO FACTURA',CAST(I.InvoiceDate AS DATE) 'FECHA FACTURA',CUPS2.[CUPS] 'CUPS INCLUIDO',CUPS2.[DESCRIPCION CUPS] 'DESCRIPCION INCLUIDO',
...
IMG.[GRUPO ATENCION],IMG.[USUARIO FACTURO],IMG.[USUARIO ANULO],ISNULL(IMG.[FECHA ANULACION],ISNULL(PAQ.[FECHA ANULACION],INC.[FECHA ANULACION])) 'FECHA ANULACION',
IMG.[FECHA LECTURA],IMG.[AGRUPADOR CAUSACION],IMG.[AGRUPADOR] 
FROM CTE_DATOS_IMAGENES_ANULADOS IMG
LEFT JOIN CTE_DATOS_IMAGENES_EMPAQUETADO_ANULADOS PAQ ON IMG.ServiceOrderDetailId=PAQ.ServiceOrderDetailId
LEFT JOIN CTE_DATOS_IMAGENES_INCLUIDO_ANULADOS INC ON IMG.ServiceOrderDetailId=INC.ServiceOrderDetailId
