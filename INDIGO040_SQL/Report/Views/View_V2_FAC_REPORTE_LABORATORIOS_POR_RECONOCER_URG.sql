...
-- ItemId: SPN
-- Schema: Report
-- Object: View_V2_FAC_REPORTE_LABORATORIOS_POR_RECONOCER_URG
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE VIEW [Report].[View_V2_FAC_REPORTE_LABORATORIOS_POR_RECONOCER_URG]
AS

WITH CTE_LABORATORIOS_UNICOS
----CTE PARA IDENTIFICAR LOS INGRESOS UNICOS DE LABORATORIOS QUE ESTAN PENDIENTES DE RECONOCER - URGENCIAS
AS
(
SELECT DISTINCT ING.NUMINGRES ,ING.IPCODPACI,ING.IESTADOIN
  ...
  WHERE ing.IESTADOIN in (' ','P') AND A.GENSERVICEORDER  IS NULL AND A.ESTSERIPS IN ('3','4') AND A.MANEXTPRO ='0' AND FUN.UFUTIPUNI =1
UNION ALL
   SELECT DISTINCT ING.NUMINGRES ,ING.IPCODPACI,ING.IESTADOIN
...
AS
(
    SELECT EGR.IPCODPACI ,EGR.NUMINGRES,EGR.FECINIEST ,EGR.FECFINEST ,EGR.CODICAMAS,CAM.NUMCAMHOS ,FUN.UFUDESCRI  ,FUN.UFUTIPUNI ,FUN.UFUCODIGO  
	FROM CHREGESTA EGR with (nolock)
	INNER JOIN 
	(
	   SELECT EGR.IPCODPACI ,EGR.NUMINGRES,MAX(EGR.ID) ID  FROM CHREGESTA EGR with (nolock)
	   INNER JOIN CTE_LABORATORIOS_UNICOS AS UNI with (nolock) ON UNI.NUMINGRES =EGR.NUMINGRES
...
INNER JOIN
 (
  ...
  INNER JOIN CTE_LABORATORIOS_UNICOS AS UNI with (nolock) ON UNI.NUMINGRES =RC.AdmissionNumber  
  INNER JOIN Contract .CUPSEntity AS CUPS WITH (NOLOCK) ON CUPS.Id =SOD.CUPSEntityId AND CUPS.ServiceType ='1'
...
---CTE QUE ME TRAE LOS DATOS DE LOS LABORATORIOS DE FORMA AGRUPADA
AS
(
  ...
  WHEN ISNULL(E.FECHISPAC, '') <> '' THEN E.FECHISPAC ELSE A.FECHARESULT END, 126), 'T00:00:00', 'T23:59:59.998') AS DATETIME)) AS 'FECHA REALIZADO MAXIMA'
	 ...
	 (
	    SELECT ING.NUMINGRES,A.IPCODPACI,P.IPNOMCOMP ,RTRIM(A.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(A.CANSERIPS ) AS 'CANTIDAD ORDENADA',UNI.IESTADOIN 
		    FROM dbo.ADINGRESO ing
	         ...
	         INNER JOIN DBO.INPACIENT AS P with (nolock) ON P.IPCODPACI =A.IPCODPACI
			 INNER JOIN DBO.INUNIFUNC AS FUN ON FUN.UFUCODIGO =A.UFUCODIGO 
			 WHERE A.ESTSERIPS IN ('3','4') AND A.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1'
	...
	WHERE A.GENSERVICEORDER  IS NULL AND A.ESTSERIPS IN ('3','4') AND A.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1'
	GROUP BY ING.NUMINGRES,A.NUMEFOLIO,A.IPCODPACI,P.IPNOMCOMP ,A.CODSERIPS,B.DESSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],CAM.NUMCAMHOS,HA.Name , CG.Name ,
	ING.IFECHAING ,FUN.UFUDESCRI,FUN.UFUTIPUNI
UNION ALL
   SELECT ING.NUMINGRES,A.NUMEFOLIO,A.IPCODPACI,P.IPNOMCOMP ,RTRIM(A.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(A.CANSERIPS ) AS CANSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],CAM.NUMCAMHOS,
   HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',FUN.UFUDESCRI,FUN.UFUTIPUNI,
   MIN(CAST(REPLACE(CONVERT(VARCHAR,CASE WHEN ISNULL(I.FECGENERA, '') <> '' THEN TRY_PARSE(I.FECGENERA AS DATETIME USING 'es-co')
...
(
  ...
  ING.NUMCAMHOS,EGR.FECEGRESO,CAR.CUPS 'CUPS CARGADO',CAR.CANTIDAD 'CANTIDAD CARGADA'  ,ING.[FECHA REALIZADO MINIMA] ,ING.[FECHA REALIZADO MAXIMA]  
  FROM CTE_DATOS_LABORATORIOS ING
  ...
  --AND  ING.UFUTIPUNI ='1' --ING.NUMCAMHOS LIKE 'U%' OR ING.NUMCAMHOS LIKE 'O%' OR ING.NUMCAMHOS LIKE 'SRE%'
),
...
WHERE JUS.Code IS NULL and lab.UFUDESCRI in ('PEDIATRIA URGENCIAS','URGENCIAS ADULTOS','PABELLON URGENCIAS ADULTOS','GINECOBSTETRICIA URGENCIAS')
--FN V2
