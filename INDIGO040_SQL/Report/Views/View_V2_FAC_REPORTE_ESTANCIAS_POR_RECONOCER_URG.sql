...
-- ItemId: SPN
-- Schema: Report
-- Object: View_V2_FAC_REPORTE_ESTANCIAS_POR_RECONOCER_URG
...
CREATE VIEW [Report].[View_V2_FAC_REPORTE_ESTANCIAS_POR_RECONOCER_URG]
AS
...
AS
(
 SELECT DISTINCT EST.IPCODPACI ,EST.NUMINGRES,ING.IESTADOIN,EST.ID,ACS.StayId
 FROM   dbo.CHREGESTA EST
 INNER JOIN DBO.ADINGRESO AS ING WITH (NOLOCK) ON ING.NUMINGRES = EST.NUMINGRES 
 INNER JOIN DBO.INUNIFUNC AS FUN with (nolock) ON ING.UFUCODIGO =FUN.UFUCODIGO  
 LEFT JOIN Billing.AccountControlStays ACS with (nolock) ON ACS.AdmissionCode=EST.NUMINGRES AND ACS.StayId=EST.ID
 WHERE (GENESTLIQ IN (1, 2)) AND ING.TIPOINGRE=2 and  ing.IESTADOIN in (' ','P') AND FUN.UFUTIPUNI ='1' AND EST.REGDIAEST>0 and ACS.StayId IS NULL
),
...
INNER JOIN Contract .CUPSEntity AS CUPS WITH (NOLOCK) ON CUPS.Id =SOD.CUPSEntityId AND CUPS.Code ='5DSA01' --ServiceType IN (7,4)
...
CTE_DATOS_ESTANCIAS
---CTE QUE ME TRAE LOS DATOS DE LAS ESTANCIAS DE FORMA AGRUPADA
AS
(
...
HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',MIN(A.FECINIEST) 'FECHA REALIZADO MINIMA' ,MAX(A.FECFINEST) 'FECHA REALIZADO MAXIMA'
 FROM dbo.CHREGESTA A WITH (NOLOCK)
   ...
   WHERE (GENESTLIQ IN (1, 2))  AND FUN.UFUTIPUNI ='1' AND ACS.StayId IS NULL AND A.REGDIAEST>0
...
(
  ...
  WHERE CAR.CANTIDAD IS NULL --and (ING.CANSERIPS <>ISNULL(CAR.CANTIDAD,0)  AND ING.CANSERIPS >ISNULL(CAR.CANTIDAD,0))
--AND (CAM.NUMCAMHOS LIKE 'U%' OR CAM.NUMCAMHOS LIKE 'O%' OR CAM.NUMCAMHOS LIKE 'SRE%')
)

SELECT * FROM CTE_CAMAS_URGENCIAS 
