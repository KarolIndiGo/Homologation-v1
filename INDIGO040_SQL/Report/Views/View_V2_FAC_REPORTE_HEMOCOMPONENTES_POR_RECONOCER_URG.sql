...
-- ItemId: SPN
-- Schema: Report
-- Object: View_V2_FAC_REPORTE_HEMOCOMPONENTES_POR_RECONOCER_URG
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE VIEW [Report].[View_V2_FAC_REPORTE_HEMOCOMPONENTES_POR_RECONOCER_URG]
AS

WITH CTE_HEMOCOMPONENTES_UNICOS
----CTE PARA IDENTIFICAR LOS INGRESOS UNICOS DE HEMOCOMPONENTES QUE ESTAN PENDIENTES DE RECONOCER - URGENCIAS
AS
(
	SELECT  DISTINCT ING.NUMINGRES ,ING.IPCODPACI,ING.IESTADOIN FROM HCORHEMSER SER with (nolock) 
	  ...
	  INNER JOIN DBO.INUNIFUNC FUN with (nolock)  ON FUN.UFUCODIGO =ING.UFUCODIGO 
	 WHERE ing.IESTADOIN in (' ','P') AND SER.ORDSERVICIOID IS NOT NULL AND SOL.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1'
	UNION ALL
	SELECT DISTINCT ING.NUMINGRES,ING.IPCODPACI,ING.IESTADOIN FROM  HCORHEMSER SER
...
WHERE RCD.STATUS IN ('1','2', '3') AND CUPS.ServiceType ='9'
...
SELECT ING.NUMINGRES,SOL.NUMEFOLIO,SOL.IPCODPACI,P.IPNOMCOMP ,RTRIM(SER.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(1) AS CANSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA] ,
	...
	(
	    ...
	    WHERE ser.ESTADO='2' AND SER.TIPOSERVICIO IN ( 2, 3) AND SOL.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1' --ING.UFUCODIGO in ('1111001','1113008','1113009')
	...
	WHERE ser.ESTADO='2' AND SER.TIPOSERVICIO IN ( 2, 3) AND SER.ORDSERVICIOID IS NULL AND SOL.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1'
	GROUP BY ING.NUMINGRES,SOL.NUMEFOLIO,SOL.IPCODPACI,P.IPNOMCOMP ,SER.CODSERIPS,B.DESSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],HA.Name,CG.Name,ING.IFECHAING ,FUN.UFUDESCRI ,FUN.UFUTIPUNI
UNION ALL
	SELECT ING.NUMINGRES,SOL.NUMEFOLIO,SOL.IPCODPACI,P.IPNOMCOMP ,RTRIM(SER.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(1) AS CANSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],
	HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',FUN.UFUDESCRI ,FUN.UFUTIPUNI,
    MIN(ISNULL( CAST(REPLACE(CONVERT(VARCHAR, COALESCE(BOL.FECAPLICENF, BOL.FECAPLICMED), 126), 'T00:00:00', 'T23:59:59.998') AS DATETIME),SOL.FECORDMED)) AS 'FECHA REALIZADO MINIMA',
	...
	INNER JOIN CTE_HEMOCOMPONENTES_UNICOS UNI ON UNI.NUMINGRES=ING.NUMINGRES
	INNER JOIN DBO.INUNIFUNC FUN ON FUN.UFUCODIGO =ING.UFUCODIGO 
	WHERE ser.ESTADO='2' AND SER.TIPOSERVICIO IN ( 2, 3)  AND SOL.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1'
	GROUP BY ING.NUMINGRES,SOL.IPCODPACI,P.IPNOMCOMP ,SER.CODSERIPS,B.DESSERIPS,UNI.IESTADOIN
	) AS G ON G.NUMINGRES =ING.NUMINGRES AND G.CODSERIPS =SER.CODSERIPS
		LEFT JOIN 
	(
	...
	WHERE ser.ESTADO='2' AND SER.TIPOSERVICIO IN ( 2, 3) AND SER.ORDSERVICIOID IS NULL AND SOL.MANEXTPRO ='0' AND fun.UFUTIPUNI ='1'--ING.UFUCODIGO in ('1111001','1113008','1113009')
...
--SELECT * FROM CTE_RECONOCIMIENTOS_HEMOCOMPONENTES 
...
WHERE JUS.Code IS NULL and HEM.UFUDESCRI in ('PEDIATRIA URGENCIAS','URGENCIAS ADULTOS','PABELLON URGENCIAS ADULTOS','GINECOBSTETRICIA URGENCIAS')
--FN V2
