...
-- ItemId: SPN
-- Schema: Report
-- Object: View_V2_FAC_REPORTE_PATOLOGIAS_POR_RECONOCER_URG
-- Extracted by Fabric SQL Extractor SPN v3.9.0



CREATE VIEW [Report].[View_V2_FAC_REPORTE_PATOLOGIAS_POR_RECONOCER_URG]
AS
...
(
   SELECT DISTINCT  ING.NUMINGRES ,ING.IPCODPACI,ING.IESTADOIN
	...
	WHERE ing.IESTADOIN in (' ','P') AND A.GENSERVICEORDER IS NULL and A.ESTSERIPS IN ('2','3','4') AND A.MANEXTPRO ='0' AND FUN.UFUTIPUNI ='1'
...
INNER JOIN Contract .CUPSEntity AS CUPS WITH (NOLOCK) ON CUPS.Id =SOD.CUPSEntityId AND CUPS.ServiceType ='2'
...
(
	...
	HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',D.UFUDESCRI ,D.UFUTIPUNI ,
	 ...
	 (
	  ...
	  INNER JOIN CTE_PATOLOGIAS_UNICOS AS UNI with (nolock) ON UNI.NUMINGRES =ING.NUMINGRES 
	    WHERE ESTSERIPS IN ('2','3','4')  AND A.MANEXTPRO ='0' AND D.UFUTIPUNI ='1'--ING.UFUCODIGO in ('1111001','1113008','1113009')
	...
	WHERE A.ESTSERIPS IN ('2','3','4')  AND A.GENSERVICEORDER IS NULL  AND A.MANEXTPRO ='0' AND D.UFUTIPUNI ='1'--ING.UFUCODIGO in ('1111001','1113008','1113009')
  ...
  UNION ALL
   ...
   HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',D.UFUDESCRI ,D.UFUTIPUNI,
	...
	WHERE A.ESTSERIPS IN ('2','3','4')  AND A.GENSERVICEORDER IS NULL  AND A.MANEXTPRO ='0' AND ING.UFUCODIGO in ('1111001','1113008','1113009')
	GROUP BY ING.NUMINGRES,A.NUMEFOLIO,A.IPCODPACI,P.IPNOMCOMP ,A.CODSERIPS,B.DESSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],HA.Name, CG.Name ,ING.IFECHAING ,D.UFUDESCRI ,D.UFUTIPUNI
 ),
...
WHERE JUS.Code IS NULL AND PA.UFUDESCRI in ('PEDIATRIA URGENCIAS','URGENCIAS ADULTOS','PABELLON URGENCIAS ADULTOS','GINECOBSTETRICIA URGENCIAS')
--FN V2
