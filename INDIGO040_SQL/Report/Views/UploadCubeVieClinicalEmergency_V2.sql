...
-- ItemId: SPN
-- Schema: Report
-- Object: UploadCubeVieClinicalEmergency_V2
...
Fecha:04-02-2025
Observaciones:Los valores comparativos de tiempo ahora se calcula en segundos.
***********************************************************************************************************************************/
CREATE view [Report].[UploadCubeVieClinicalEmergency_V2]
as
...
CTE_ORDENES_CON_ESTANCIA_RECONOCIDAS AS
 ( 
	...
	Contract .CUPSEntity AS CUPS  ON SOD.CUPSEntityId=CUPS.Id AND CUPS.BillingGroupId IN (4)
	WHERE
               ...
               FROM
			    (SELECT
				 ...
				 FROM 
				  ...
				  INNER JOIN Contract .CUPSEntity AS CUPS  ON CUPS.Id =SOD.CUPSEntityId AND CUPS.BillingGroupId IN  (4)
			   ...
			   FROM 
			    (SELECT RC.AdmissionNumber ,CUPS.Code CUPS,CUPS.Description 'DESCRIPCION CUPS' 
				 FROM 
				  ...
				  INNER JOIN Billing .SERVICEORDERDETAILDISTRIBUTION AS SODD  ON RCD.ID = SODD.REVENUECONTROLDETAILID AND RCD.STATUS IN ('1','2', '3')
				  INNER JOIN BILLING.SERVICEORDERDETAIL AS SOD  ON SODD.SERVICEORDERDETAILID = SOD.ID
				  INNER JOIN Contract .CUPSEntity AS CUPS  ON CUPS.Id =SOD.CUPSEntityId AND CUPS.BillingGroupId IN  (3)
				  INNER JOIN Contract .IPSService AS IPS  ON IPS.Id =SOD.IPSServiceId
				 WHERE 
...
SELECT
 ...
 IIF(C.IPFECLLEGA IS NULL,'0',FORMAT(CAST(DATEDIFF(SECOND,C.IPFECLLEGA, A.TRIAFECHA) AS FLOAT) / 60,'N2')) AS [MINUTOS ESPERA LLEGADA VS TRIAGE],--OPORTUNIDAD FL vs FT (MIN)
 IIF(convert(char(8), A.TRIAFECHA, 108) BETWEEN '07:00:00' AND '12:59:59','MAÑANA',IIF(convert(char(8), A.TRIAFECHA, 108) BETWEEN '13:00:00' AND '18:59:59','TARDE',
...
DAY(A.TRIAFECHA)%2=0,'NOCHE DÍA PAR',
 ...
 IIF(k.FECHINIHI IS NULL OR HC.FECHISPAC IS NULL,NULL,FORMAT(CAST(DATEDIFF(SECOND,A.TRIAFECHA,K.FECHINIHI) AS FLOAT) / 60,'N2')) AS [MINUTOS ESPERA TRIAGE VS CONSULTA],
 ...
 IIF(C.IPFECLLEGA IS NULL,FORMAT(CAST(DATEDIFF(SECOND,I.IFECHAING,A.TRIAFECHA) AS FLOAT) / 60,'N2'),FORMAT(CAST(DATEDIFF(SECOND,C.IPFECLLEGA, k.FECHINIHI) AS FLOAT) / 60,'N2')) AS [MINUTOS LLEGADA VS ATENCION],--OPORTUNIDAD FL vs FA (MIN)
 ...
 A.ID AS [ID TRIAGE],
 1 AS CANTIDAD,
 CAST(ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING)) AS date) AS [FECHA BUSQUEDA],
 YEAR(ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING))) AS [AÑO BUSQUEDA],
 MONTH(ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING))) AS [MES BUSQUEDA],
CONCAT(FORMAT(MONTH(ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING))), '00') ,' - ', 
CASE MONTH(ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING))) WHEN 1 THEN 'ENE'
   						WHEN 2 THEN 'FEB'
						WHEN 3 THEN 'MAR'
						WHEN 4 THEN 'ABR'
						WHEN 5 THEN 'MAY'
...
CASE DATENAME(dw,ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING))) WHEN 'Monday' THEN 'LUN' 
...
WHERE 
UNF.UFUDESCRI LIKE '%URGENCIA%' --AND CAST(ISNULL(C.IPFECLLEGA,ISNULL(A.TRIAFECHA,I.IFECHAING)) AS DATE) between @FECHAINI AND @FECHAFIN
--and pac.IPCODPACI='24095711122163'
