...
-- ItemId: SPN
-- Schema: Report
-- Object: ViewPathologiesRequests
...
Fecha: 17-01-2024
Observaciones: Se cambia la logica del reporte para rendimiento y se organizan todas las columnas.
--------------------------------------
Version 2
...
CREATE VIEW [Report].[ViewPathologiesRequests] AS
...
SELECT 
CG.CODE AS CODGRUPO, 
--CG.NAME AS GRUPOATENCION,CON.CODE AS CODCONTRATO,CON.CONTRACTNAME AS NOMBRECONTRATO, 
--PT.CODE AS CODPLANTIPROCE,PT.NAME AS PLANTILLAPROCEDIMIENTO, CGR.CODE AS CODIGOGRUPO, 
--CGR.NAME AS GRUPO, CSG.CODE AS CODSUBGRUPO,CSG.NAME AS SUBGRUPO, 
CE.CODE AS CUPS, 
--CE.DESCRIPTION AS DESCRIPCIONCUPS,CD.CODE AS CODRELACION,CD.NAME AS DESCRIPCIONRELACIONADA,
...
FROM 
CONTRACT.CAREGROUP AS CG WITH(NOLOCK)
...
LEFT OUTER JOIN CONTRACT.CUPSENTITYCONTRACTDESCRIPTIONS AS CECD WITH(NOLOCK) ON PC.CUPSENTITYCONTRACTDESCRIPTIONID = CECD.ID
--LEFT JOIN CONTRACT.CONTRACTDESCRIPTIONS AS CD WITH(NOLOCK) ON CECD.CONTRACTDESCRIPTIONID = CD.ID
--LEFT OUTER JOIN CONTRACT.CUPSSUBGROUP CSG WITH(NOLOCK) ON CE.CUPSSUBGROUPID = CSG.ID
--LEFT OUTER JOIN CONTRACT.CUPSGROUP CGR WITH(NOLOCK) ON CSG.CUPSGROUPID = CGR.ID
)
--CTE_ORDENAMIENTO_PATOLOGIA
--AS (
-----****PATOLOGIAS******
...
--CASE WHEN SERREASIT = '1' THEN 'ESTUDIOS REALIZADOS EN SITIO'
--	 WHEN ESTSERIPS IN('2', '3', '4')THEN 'ESTUDIOS REALIZADOS'
--	 WHEN ESTSERIPS = '6' THEN 'ANULADO' ELSE 'ESTUDIOS NO REALIZADOS' END AS ESTADO, 
...
--JOIN DBO.HCORDPATO A WITH(NOLOCK) ON ING.NUMINGRES = A.NUMINGRES
...
--LEFT JOIN CONTRACT.CONTRACTDESCRIPTIONS CD WITH(NOLOCK) ON CD.ID = CECD.CONTRACTDESCRIPTIONID LEFT JOIN
...
--Billing.Invoice FAC ON FACD.InvoiceId=FAC.Id
...
--CTE_GRUPO AS G ON G.CODGRUPO = GA.CODE
...
--GROUP BY A.CODSERIPS, B.DESSERIPS, SERREASIT, ESTSERIPS, A.IPCODPACI, A.NUMINGRES, I.IPNOMCOMP, CECD.ID, CD.ID, CD.CODE, CD.NAME, E.NOMCENATE, 
...
--JOIN CONTRACT.CUPSSUBGROUP AS CSG WITH(NOLOCK) ON CSG.ID = CUPS.CUPSSUBGROUPID
--JOIN CONTRACT.CUPSGROUP AS CG WITH(NOLOCK) ON CG.ID = CSG.CUPSGROUPID
--JOIN DBO.HCHISPACA HIS WITH(NOLOCK) ON A.NUMINGRES = HIS.NUMINGRES AND A.NUMEFOLIO = HIS.NUMEFOLIO
--JOIN DBO.INESPECIA AS ESPMED WITH(NOLOCK) ON ESPMED.CODESPECI = HIS.CODESPTRA
--JOIN DBO.INDIAGNOS AS DIAG WITH(NOLOCK) ON DIAG.CODDIAGNO = HIS.CODDIAGNO
--JOIN CONTRACT.CAREGROUP AS GA WITH(NOLOCK) ON GA.ID = ING.GENCAREGROUP
--JOIN CONTRACT.HEALTHADMINISTRATOR AS EA WITH(NOLOCK) ON EA.ID = ING.GENCONENTITY LEFT JOIN 
--CONTRACT.CUPSENTITYCONTRACTDESCRIPTIONS CECD WITH(NOLOCK) ON CECD.ID = A.IDDESCRIPCIONRELACIONADA LEFT JOIN 
--CONTRACT.CONTRACTDESCRIPTIONS CD WITH(NOLOCK) ON CD.ID = CECD.CONTRACTDESCRIPTIONID LEFT JOIN
--Billing.ServiceOrder ORD ON ORD.Id=A.GENSERVICEORDER LEFT JOIN
--Billing.ServiceOrderDetail ORDD ON ORD.Id=ORDD.ServiceOrderId LEFT JOIN
--Billing.InvoiceDetail FACD ON ORDD.ID=FACD.ServiceOrderDetailId LEFT JOIN
--Billing.Invoice FAC ON FACD.InvoiceId=FAC.Id
--LEFT JOIN DBO.INDIAGNOP AS EST WITH(NOLOCK) ON A.IPCODPACI = EST.IPCODPACI AND A.NUMINGRES = EST.NUMINGRES
--																		   AND EST.NUMEFOLIO = A.NUMEFOLIO
--																		   AND EST.ESTADIO IS NOT NULL LEFT JOIN
--CTE_GRUPO AS G ON G.CODGRUPO = GA.CODE
--AND G.CUPS = A.CODSERIPS
...
--E.NOMCENATE, A.UFUCODIGO, D.UFUDESCRI, A.NUMEFOLIO, PRO.CODPROSAL, PRO.NOMMEDICO, MANEXTPRO, A.FECORDMED, CG.CODE + '-' + CG.NAME, 
--CSG.CODE + '-' + CSG.NAME, ESPMED.DESESPECI, DIAG.CODDIAGNO, DIAG.NOMDIAGNO, EA.CODE + ' - ' + EA.NAME, GA.CODE + ' - ' + GA.NAME, 
--GA.LIQUIDATIONTYPE, EST.ESTADIO, GA.CODE, A.IDDESCRIPCIONRELACIONADA, G.CONTRATADO, G.COTIZADO, IIF(G.CODGRUPO IS NULL, 'NO', 'SI'), 
--ESTA.ESTADIO, I.IPTIPODOC, I.IPTELEFON, I.IPTELMOVI,FAC.InvoiceNumber
--)
...
A.NUMINGRES AS [INGRESO],
A.NUMEFOLIO AS FOLIO, 
RTRIM(A.UFUCODIGO) + ' - ' +D.UFUDESCRI AS [UNIDAD FUNCIONAL], 
PRO.CODPROSAL [ID PROFESIONAL], 
PRO.NOMMEDICO AS PROFESIONAL, 
ESPMED.DESESPECI ESPECIALIDAD, 
A.CODSERIPS [CODIGO CUPS], 
...
CASE A.ESTSERIPS WHEN 1 THEN 'Solicitado'
				 ...
				 WHEN 6 THEN 'Anulado'
				 WHEN 7 THEN 'Extramural' ELSE CASE B.SERREASIT WHEN 1 THEN 'Estudio Realizado' END END AS ESTADO, 
SUM(A.CANSERIPS) AS CANTIDAD, 
DIAG.CODDIAGNO CIE10, 
DIAG.NOMDIAGNO DIAGNOSTICO,
CASE ESTA.ESTADIO WHEN 0 THEN 'ESTADIO CLÍNICO (EC) 0 (TUMOR IN SITU)'
