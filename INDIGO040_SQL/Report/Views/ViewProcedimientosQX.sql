...
CREATE VIEW  [Report].[ViewProcedimientosQX] AS
...
SELECT
 ...
 CASE WHEN ING.TIPOINGRE=1 THEN 'ORIGEN AMBULATORIO' 
	  WHEN ING.TIPOINGRE=2 THEN 'ORIGEN HOSPITALARIO'
	  WHEN PR.ORIGENQX=1 THEN 'ORIGEN AMBULATORIO' 
	  WHEN PR.ORIGENQX=2 THEN 'ORIGEN HOSPITALARIO'  END AS AMBITO,
...
FROM 
 ...
 INNER JOIN dbo.ADINGRESO ING ON QX.NUMINGRES=ING.NUMINGRES
 ...
 INNER JOIN CONTRACT.CAREGROUP AS GA ON GA.ID = PAC.GENCAREGROUP 
 INNER JOIN Contract.HealthAdministrator EA ON PAC.CODENTIDA=EA.CODE 
 INNER JOIN dbo.INDIAGNOS DIG1 ON DIG1.CODDIAGNO=QX.CODDIAPRE 
 INNER JOIN dbo.INDIAGNOS DIG2 ON DIG2.CODDIAGNO=QX.CODDIAPOS  
 /*in v6*/INNER JOIN dbo.HCHISPACA HC ON QX.NUMINGRES=HC.NUMINGRES AND QX.NUMEFOLIO=HC.NUMEFOLIO AND HC.ESTAFOLIO=1-- se valida que el folio no este anulado
 INNER JOIN dbo.HCQXEQUIP CX ON QX.NUMINGRES=CX.NUMINGRES AND QX.NUMEFOLIO=CX.NUMEFOLIO AND CX.QXPRINCIP=1/*fn v6*/
 INNER JOIN dbo.INPROFSAL PRO ON CX.CODPROSAL=PRO.CODPROSAL
 LEFT JOIN dbo.INUNIFUNC UNI ON QX.UFUCODIGO = UNI.UFUCODIGO
 ...
 LEFT JOIN dbo.HCORDPROQ SOL ON SOL.AUTO=(SELECT MAX(A.AUTO) FROM dbo.HCORDPROQ A WHERE A.NUMINGRES=QX.NUMINGRES AND QX.CODSERIPS=A.CODSERIPS)
 LEFT JOIN dbo.HCORDPRON NONQX ON QX.NUMINGRES=NONQX.NUMINGRES AND NONQX.AUTO=(SELECT MAX(A.AUTO) FROM dbo.HCORDPRON A WHERE A.NUMINGRES=QX.NUMINGRES AND QX.CODSERIPS=A.CODSERIPS)
 LEFT JOIN dbo.AGEPROGQX PR ON SOL.AUTOCITA=PR.CODAUTONU --FN V8
 ...
 LEFT JOIN Report.GruposQX GRU ON QX.CODSERIPS=GRU.Code /*IN V5*/
 LEFT JOIN AGENSALAC SAL ON PR.AGENSALAC=SAL.CODCONCEC /*FN V5*/
