...
-- ItemId: SPN
-- Schema: Report
-- Object: View_V2_FAC_REPORTE_GASES_POR_RECONOCER_HOS
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE VIEW [Report].[View_V2_FAC_REPORTE_GASES_POR_RECONOCER_HOS]
AS
...
INNER JOIN Contract .CUPSEntity AS CUPS WITH (NOLOCK) ON CUPS.Id =SOD.CUPSEntityId AND CUPS.ServiceType IN (4)--CUPS.BillingConceptId IN (5)
...
AS
(
SELECT ING.NUMINGRES,A.IPCODPACI,P.IPNOMCOMP ,RTRIM(E.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(a.TOTLITADM ) AS CANSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA] ,
HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',MIN(A.HORAINICIA) 'FECHA REALIZADO MINIMA',MAX(A.HORAINICIA) 'FECHA REALIZADO MAXIMA'
  ...
  INNER JOIN
	...
	GROUP BY ING.NUMINGRES,A.IPCODPACI,P.IPNOMCOMP ,E.CODSERIPS,B.DESSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],HA.Name , CG.Name ,ING.IFECHAING 
  UNION ALL
   SELECT ING.NUMINGRES,A.IPCODPACI,P.IPNOMCOMP ,RTRIM(E.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(A.TOTLITADM ) AS CANSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA] ,
   HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',MIN(A.HORAINICIA) 'FECHA REALIZADO MINIMA',MAX(A.HORAINICIA) 'FECHA REALIZADO MAXIMA'
   FROM dbo.HCCONOXIG AS A 
