...
-- ItemId: SPN
-- Schema: Report
-- Object: View_V2_FAC_REPORTE_IMAGENES_POR_RECONOCER_URG
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [Report].[View_V2_FAC_REPORTE_IMAGENES_POR_RECONOCER_URG]
AS

WITH CTE_IMAGENES_UNICOS
----CTE PARA IDENTIFICAR LOS INGRESOS UNICOS DE IMAGENES QUE ESTAN PENDIENTES DE RECONOCER - URGENCIAS
AS
(
SELECT DISTINCT  ING.NUMINGRES ,ING.IPCODPACI,ING.IESTADOIN
  FROM dbo.ADINGRESO ing with (nolock)
	...
	INNER JOIN DBO.INUNIFUNC AS FUN with (nolock) ON  FUN.UFUCODIGO =A.UFUCODIGO 
	WHERE ing.IESTADOIN in (' ','P') AND A.GENSERVICEORDER IS NULL and (A.ESTSERIPS IN ('2','3','4') OR B.SERREASIT='1') AND A.MANEXTPRO ='0'
	AND FUN.UFUTIPUNI ='1'
   UNION ALL
	SELECT DISTINCT ING.NUMINGRES,ING.IPCODPACI,ING.IESTADOIN 
...
( 
 ...
 INNER JOIN
  (
   ...
   INNER JOIN Contract .CUPSEntity AS CUPS WITH (NOLOCK) ON CUPS.Id =SOD.CUPSEntityId AND CUPS.ServiceType ='3'
...
MAX(COALESCE(A.FECHLECT, A.FECORDMED)) as 'FECHA REALIZADO MAXIMA',D.UFUDESCRI ,D.UFUTIPUNI 
FROM dbo.ADINGRESO ing with (nolock) 
	INNER JOIN dbo.HCORDIMAG A with (nolock) ON ing.NUMINGRES = A.NUMINGRES
	INNER JOIN dbo.INCUPSIPS B with (nolock) ON A.CODSERIPS=B.CODSERIPS 
	...
	(
	    ...
	    WHERE (A.ESTSERIPS IN ('2','3','4') OR B.SERREASIT='1') AND A.MANEXTPRO ='0' AND D.UFUTIPUNI ='1'
	...
	WHERE (A.ESTSERIPS IN ('2','3','4') OR B.SERREASIT='1') AND A.GENSERVICEORDER IS NULL AND A.MANEXTPRO ='0' AND D.UFUTIPUNI ='1'
	GROUP BY ING.NUMINGRES,A.NUMEFOLIO,A.IPCODPACI,P.IPNOMCOMP ,A.CODSERIPS,B.DESSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA],HA.Name, CG.Name ,ING.IFECHAING ,D.UFUDESCRI ,D.UFUTIPUNI 
  UNION ALL
   SELECT ING.NUMINGRES,A.NUMEFOLIO,A.IPCODPACI,P.IPNOMCOMP ,RTRIM(A.CODSERIPS) AS CODSERIPS,B.DESSERIPS,SUM(A.CANSERIPS ) AS CANSERIPS,G.IESTADOIN,G.[CANTIDAD ORDENADA] ,
   HA.Name ENTIDAD, CG.Name 'GRUPO ATENCION',CAST(ING.IFECHAING AS DATE) 'FECHA INGRESO',MIN(COALESCE(A.FECHLECT, A.FECORDMED)) as 'FECHA REALIZADO MINIMA',
MAX(COALESCE(A.FECHLECT, A.FECORDMED)) as 'FECHA REALIZADO MAXIMA',D.UFUDESCRI ,D.UFUTIPUNI 
...
WHERE JUS.Code IS NULL AND IM.UFUDESCRI in ('PEDIATRIA URGENCIAS','URGENCIAS ADULTOS','PABELLON URGENCIAS ADULTOS','GINECOBSTETRICIA URGENCIAS')
--FN V2
