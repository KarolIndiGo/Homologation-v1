...
WITH CTE_FACTURACION_TOTAL AS
 (
  SELECT 
   F.Id ID_FACTURA ,CASE F.DocumentType WHEN '1' THEN 'FACTURA EAPB CON CONTRATO'  WHEN '2' THEN 'FACTURA EAPB SIN CONTRATO'  WHEN '3' THEN 'FACTURA PARTICULAR'
   WHEN '4' THEN 'FACTURA CAPITA' WHEN '5' THEN 'CONTROL CAPITACION' WHEN '6' THEN 'NO OPERACIONAL' WHEN '7' THEN 'FACTURA VENTA PRODUCTOS' END 'TIPO REGISTRO',
   ...
   ISNULL(RTRIM(F.PatientCode), 0) AS 'IDENTIFICACION' ,CASE WHEN RTRIM(PAC.IPNOMCOMP) IS NULL AND F.DOCUMENTTYPE LIKE '4' THEN 'N/A' WHEN PAC.IPNOMCOMP IS NULL AND F.DOCUMENTTYPE LIKE '6' THEN 'N/A'
   ...
   F.InvoiceNumber 'NRO FACTURA' ,IIF(F.DOCUMENTTYPE=6, cast(f.InitialDate AS date), CAST(F.InvoiceDate AS DATE)) AS 'FECHA FACTURA',cast(F.InvoiceExpirationDate as date) AS 'FECHA VENCIMIENTO',
   ...
   CASE F.STATUS WHEN '1' THEN 'FACTURADO' WHEN '2' THEN 'ANULADO' END AS 'ESTADO FACTURA',RTRIM(T.Nit) 'NIT ENTIDAD' ,
   ...
   iif(F.DOCUMENTTYPE=6, cast(f.InitialDate AS date), CAST(F.InvoiceDate AS DATE)) AS 'FECHA BUSQUEDA', USU2.NOMUSUARI 'USUARIO ANULO',BRR.Description 'RAZONES DE ANULACION'
  FROM 
   ...
   LEFT JOIN Common.ThirdParty AS T WITH (NOLOCK) ON T.Id = F.ThirdPartyId
   LEFT JOIN DBO.ADINGRESO AS ING WITH (NOLOCK) ON ING.NUMINGRES =F.AdmissionNumber 
   LEFT JOIN DBO.INPACIENT AS PAC WITH (NOLOCK) ON PAC.IPCODPACI =F.PatientCode 
   ...
   LEFT JOIN dbo.SEGusuaru AS USU2 WITH (NOLOCK) ON USU2.CODUSUARI = F.AnnulmentUser
   LEFT JOIN Billing .BillingReversalReason AS BRR WITH (NOLOCK) ON BRR.Id =F.ReversalReasonId
   where iif(F.DOCUMENTTYPE=6, cast(f.InitialDate AS date), CAST(F.InvoiceDate AS DATE)) between @FECINI and @FECFIN
 ),
...
CTE_FACTURACION_TOTAL_ANULADA AS
 ( 
  SELECT 
   ...
   WHEN '9' THEN 'CERTIFICADO NACIDO VIVO' WHEN '10' THEN 'CARNET DIPLOMATICO'WHEN '11' THEN 'SALVOCONDUCTO' WHEN '12' THEN 'PERMISO ESPECIAL DE PERMANENCIA' 
   WHEN '13' THEN 'PERMISO TEMPORAL DE PERMANENCIA' WHEN '14' THEN 'DOCUMENTO EXTRANJERO' WHEN '15' THEN 'SIN IDENTIFICACION' END AS 'TIPO IDENTIFICACION',
   ISNULL(F.PatientCode, 0) AS 'IDENTIFICACION' ,CASE WHEN RTRIM(PAC.IPNOMCOMP) IS NULL AND F.DOCUMENTTYPE LIKE '4' THEN 'N/A' WHEN PAC.IPNOMCOMP IS NULL AND F.DOCUMENTTYPE LIKE '6' THEN 'N/A'
   ELSE RTRIM(PAC.IPNOMCOMP) END AS  'PACIENTE' ,ISNULL(F.AdmissionNumber,'000') 'INGRESO',CAST(ISNULL(ING.IFECHAING,f.InitialDate) AS DATE)'FECHA INGRESO',CAST(ISNULL(ING.FECHEGRESO,ISNULL(ING.IFECHAING,f.InitialDate)) AS DATE) 'FECHA EGRESO' ,
   F.InvoiceNumber 'NRO FACTURA' ,CAST(F.AnnulmentDate AS DATE) AS 'FECHA FACTURA',cast(F.InvoiceExpirationDate as date) AS 'FECHA VENCIMIENTO',
   IIF(F.DOCUMENTTYPE=6, -f.InvoiceValue, -F.TotalInvoice) AS 'TOTAL FACTURA',-F.ValueTax AS 'VALOR IMPUESTO',-F.ThirdPartySalesValue AS 'VR TOTAL ENTIDAD',-F.ThirdPartyDiscountValue AS 'VR TOTAL DESCUENTO',
   -F.TotalPatientSalesPrice AS 'VR TOTAL CUOTA RECUPERACION',-F.PatientDiscount AS 'DESCUENTO CUOTA RECUPERACION',-F.PatientPaidValue AS 'VR PAGADO PACIENTE',
   -F.ThirdPartyAccountReceivableValue AS 'VR CXC',-F.PatientAccountReceivableValue AS 'VR CXC GENERADA A PACIENTE',
   CASE F.STATUS WHEN '1' THEN 'FACTURADO' WHEN '2' THEN 'ANULADO' END AS 'ESTADO FACTURA',RTRIM(T.Nit) 'NIT ENTIDAD' ,
   ...
   ELSE ING.IAUTORIZA END AS 'AUTORIZACION',  ISNULL(IIF (F.DOCUMENTTYPE=6, LTRIM(FU2.CODE) + ' - ' + FU2.NAME, 
   ISNULL(FU.CODE + ' - ' +FU.Name, LTRIM(ING.UFUCODIGO) + ' - ' + UF.NAME)), 'N/A') AS [UNIDAD FUNCIONAL EGRESO],IIF (F.DOCUMENTTYPE=6, FU2.NAME, C.Name) AS  'CATEGORIA FACTURA',F.AnnulmentDate 'FECHA ANULACION',
   CAST(F.AnnulmentDate AS DATE) AS 'FECHA BUSQUEDA', USU2.NOMUSUARI 'USUARIO ANULO',BRR.Description 'RAZONES DE ANULACION'
 FROM 
  Billing.Invoice AS F WITH (NOLOCK)
  LEFT JOIN Contract .HealthAdministrator AS HA WITH (NOLOCK) ON HA.Id =F.HealthAdministratorId 
  LEFT JOIN Contract .CareGroup AS CG WITH (NOLOCK) ON CG.Id =F.CareGroupId 
  ...
  LEFT JOIN Billing .BillingReversalReason AS BRR WITH (NOLOCK) ON BRR.Id =F.ReversalReasonId 
  WHERE 
   F.Status = 2 and CAST(F.AnnulmentDate AS DATE)  between @FECINI and @FECFIN
)
...
FROM
 CTE_FACTURACION_TOTAL CABF --where cast([FECHA BUSQUEDA] as date) between @FECINI AND @FECFIN
UNION ALL
SELECT
 CAST(DB_NAME() AS VARCHAR(9)) AS ID_COMPANY, 
 CABFA.*,
 ...
 CONVERT(DATETIME,GETDATE() AT TIME ZONE 'Pakistan Standard Time',1) AS ULT_ACTUAL
FROM
 CTE_FACTURACION_TOTAL_ANULADA CABFA --where cast([FECHA BUSQUEDA] as date) between @FECINI AND @FECFIN
