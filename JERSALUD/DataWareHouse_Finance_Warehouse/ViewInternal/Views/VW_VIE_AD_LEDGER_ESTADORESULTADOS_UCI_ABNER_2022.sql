...
-- Schema: ViewInternal
-- Object: VW_VIE_AD_LEDGER_ESTADORESULTADOS_UCI_ABNER_2022
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.VW_VIE_AD_LEDGER_ESTADORESULTADOS_UCI_ABNER_2022
AS
         ...
         WHERE(Number = LEFT(c.Number, 6))
            ...
            CASE
                WHEN sc.Month = '11'
                     AND sc.Year = '2022'
                THEN CAST((SUM(sc.DebitValue) - SUM(sc.CreditValue)) AS DECIMAL(19,4))
            END AS Noviembre2022,
            CASE
                WHEN sc.Month = '12'
                     AND sc.Year = '2022'
                THEN CAST((SUM(sc.DebitValue) - SUM(sc.CreditValue)) AS DECIMAL(19,4))
            END AS Diciembre2022
