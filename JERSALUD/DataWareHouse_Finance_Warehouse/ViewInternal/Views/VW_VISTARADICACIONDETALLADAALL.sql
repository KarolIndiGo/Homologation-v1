...
CREATE VIEW ViewInternal.VW_VISTARADICACIONDETALLADAALL AS
...
FROM            INDIGO031.Portfolio.RadicateInvoiceC AS rc INNER JOIN
                         ...
                         INDIGO031.GeneralLedger.MainAccounts AS MA ON MA.Id = ar.AccountWithoutRadicateId LEFT OUTER JOIN
                         INDIGO031.Billing.Invoice AS i ON i.InvoiceNumber = ar.InvoiceNumber LEFT OUTER JOIN
                         INDIGO031.Billing.InvoiceCategories AS ic ON ic.Id = i.InvoiceCategoryId LEFT OUTER JOIN
                             (SELECT        MAX(gdrc.Id) AS id, gdrd.InvoiceNumber
                               FROM            INDIGO031.Glosas.GlosaDevolutionsReceptionD AS gdrd INNER JOIN
                                                         INDIGO031.Glosas.GlosaDevolutionsReceptionC AS gdrc ON gdrc.Id = gdrd.GlosaDevolutionsReceptionCId
                               GROUP BY gdrd.InvoiceNumber) AS dev ON ar.InvoiceNumber = dev.InvoiceNumber LEFT OUTER JOIN
                         INDIGO031.Glosas.GlosaDevolutionsReceptionC AS gdrc ON dev.id = gdrc.Id LEFT OUTER JOIN
                         INDIGO031.Security.[UserInt] AS u ON gdrc.CreationUser = u.UserCode LEFT OUTER JOIN
                         INDIGO031.Security.PersonInt AS p ON u.IdPerson = p.Id