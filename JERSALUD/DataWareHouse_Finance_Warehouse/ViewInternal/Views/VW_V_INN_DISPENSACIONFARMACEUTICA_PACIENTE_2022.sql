...
-- Schema: ViewInternal
-- Object: VW_V_INN_DISPENSACIONFARMACEUTICA_PACIENTE_2022
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.VW_V_INN_DISPENSACIONFARMACEUTICA_PACIENTE_2022 AS
...
JOIN INDIGO031.[Security].PermissionCompanyInt AS Perm ON Perm.IdUser=U.Id AND Perm.Permission=1 AND Perm.IdContainer=2002
...
WHERE F.IsManual=0 AND DATEPART(YYYY,F.CreationDate) ='2022'
-------------------------------
UNION
-------------------------------
SELECT DISTINCT 
...
JOIN INDIGO031.[Security].[UserInt] AS U ON F.CreationUser=U.UserCode
JOIN INDIGO031.[Security].PersonInt AS Per ON U.IdPerson=Per.Id
JOIN INDIGO031.[Security].PermissionCompanyInt AS Perm ON Perm.IdUser=U.Id AND Perm.Permission=1 AND Perm.IdContainer=72
JOIN INDIGO031.Common.OperatingUnit AS UO ON UO.Id=Perm.IdOperatingUnitDefault
JOIN INDIGO031.Common.City AS C ON C.Id=UO.IdCity
JOIN INDIGO031.Common.Department AS D ON D.Id=C.DepartamentId
LEFT JOIN INDIGO031.dbo.INPACIENT AS pa on pa.IPCODPACI=F.PatientCode
...
WHERE F.IsManual=1 AND DATEPART(YYYY,F.CreationDate)='2022'