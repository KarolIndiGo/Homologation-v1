...
-- Schema: ViewInternal
-- Object: VW_VIE_SERVICIOS_FACTURADOS_2025
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.VW_VIE_SERVICIOS_FACTURADOS_2025
AS
SELECT DISTINCT ING.IFECHAING AS FECHA_INGRESO, ING.NUMINGRES AS INGRESO, ING.CODUSUCRE AS USUARIO_CREA, us.NOMUSUARI as UsuarioIngreso,CA.NOMCENATE AS CENTRO_ATENCION, U.UFUDESCRI AS UNIDAD_FUNCIONAL,
  ...
  FROM INDIGO031.Billing.Invoice AS F 
	   ...
	   LEFT JOIN INDIGO031.Common.OperatingUnit AS OP ON F.OperatingUnitId = OP.Id
	   LEFT JOIN INDIGO031.Billing.BillingGroup AS GF ON CUPS.BillingGroupId = GF.Id
	   LEFT JOIN INDIGO031.Contract.CupsSubgroup AS SCUP ON CUPS.CUPSSubGroupId = SCUP.Id
	   LEFT JOIN INDIGO031.Contract.RIPSServices AS SR on SR.Id = CUPS.RIPSServiceId 


	   --LEFT JOIN .INDIAGNOP AS DIAG ON DIAG.NUMINGRES=ING.NUMINGRES AND DIAG.CODDIAPRI=1
	   --INNER JOIN .INDIAGNOS AS 
WHERE  ING.IFECHAING >= '09-01-2025 00:00:00' and F.InvoiceDate >= '09-01-2025 00:00:00' 