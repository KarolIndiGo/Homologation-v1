...
-- Schema: ViewInternal
-- Object: VW_V_INN_DISPENSACIONFARMACEUTICA_PACIENTE
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.VW_V_INN_DISPENSACIONFARMACEUTICA_PACIENTE AS
...
SELECT DISTINCT 
    ...
    ISNULL(
        (
            SELECT SUM(MFDIF.PendingQuantity)
            FROM INDIGO031.Inventory.MedicalFormulaDetailDeferred AS MFDIF
            WHERE MFDIF.DeliveryDate <= GETDATE()
    ...
    (
        ...
        GROUP BY SQ.MedicalFormulaDetailId
    ) AS 'Cantidad de Entregas',
    ...
    case
        WHEN CEN.CODCENATE IN (002, 003, 004, 005, 006, 007, 008, 009, 017, 021) THEN 'BOYACA'
        WHEN CEN.CODCENATE IN (010, 011, 012, 013, 014, 018) THEN 'META'
        WHEN CEN.CODCENATE IN (015, 016, 019, 020) THEN 'CASANARE'
    END AS Dpto,
    '' AS Municipio,
...
JOIN INDIGO031.Inventory.MedicalFormulaPharmaceuticalDispensing AS DIFI ON DIFI.MedicalFormulaId = F.Id
...
WHERE F.IsManual = 0
  AND DATEPART(YYYY, F.CreationDate) >= '2023'
...
WHERE F.IsManual = 1
  AND DATEPART(YYYY, F.CreationDate) >= '2025'