...
CREATE view ViewInternal.VW_VREPORTECONSOLIDADO
as (
SELECT        i.InvoiceDate AS ff, ad.IPCODPACI AS Cedula, pa.IPNOMCOMP AS Nombre, ad.NUMINGRES AS Ingreso, ic.Code + ' - ' + ic.Name as Categoria, cg.Code + ' - ' + cg.Name AS GrupoAtencion, 
                         ...
                         i.AnnulmentDate AS AnulaFecha, BillingReversalReason.Name AS AnulaRazon, BillingReversalReason.Description AS AnulaDescripcion
FROM            INDIGO031.Billing.Invoice AS i INNER JOIN
				INDIGO031.Billing.InvoiceCategories ic on ic.Id = i.InvoiceCategoryId inner join
                         ...
                         INDIGO031.Security.[UserInt] AS u ON u.UserCode = i.InvoicedUser inner JOIN
						 INDIGO031.Security.PersonInt pe ON pe.Id = u.IdPerson left join
                         INDIGO031.Contract.HealthAdministrator AS ha ON ha.Id = i.HealthAdministratorId left JOIN
                         INDIGO031.Billing.BillingReversalReason ON i.ReversalReasonId = BillingReversalReason.Id
						 )