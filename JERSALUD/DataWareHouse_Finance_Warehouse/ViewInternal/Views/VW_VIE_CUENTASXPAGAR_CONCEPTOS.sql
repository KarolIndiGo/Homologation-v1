...
-- Schema: ViewInternal
-- Object: VW_VIE_CUENTASXPAGAR_CONCEPTOS
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.VW_VIE_CUENTASXPAGAR_CONCEPTOS
AS
     SELECT e.Code AS Identificación, 
            ...
            CASE
                WHEN c1.Number LIKE '2%'
                THEN-(DCxp.Value)
                WHEN c1.Number NOT LIKE '2%'
                THEN(DCxp.Value)
            END AS VlrDetalle, 
            cc.Name AS CentroCosto, 
            c1.Number AS CuentaDetalle, 
     ...
     FROM INDIGO031.Payments.AccountPayable AS p 
          ...
          LEFT OUTER JOIN INDIGO031.Payments.AccountPayableDetailConcept AS DCxp  ON DCxp.IdAccountPayable = p.Id
          LEFT OUTER JOIN INDIGO031.Payroll.CostCenter AS cc ON DCxp.IdCostCenter = cc.Id
          INNER JOIN INDIGO031.GeneralLedger.MainAccounts AS c1 ON DCxp.IdAccount = c1.Id
          INNER JOIN INDIGO031.Payments.AccountPayableConcepts AS Con ON DCxp.IdConceptAccountPayable = Con.Id
          LEFT OUTER JOIN INDIGO031.Common.SupplierBankAccount AS cb  ON cb.SupplierId = e.Id
          ...
          LEFT OUTER JOIN INDIGO031.Common.Email AS m  ON m.IdPerson = t.PersonId
