...
-- Schema: ViewInternal
-- Object: VW_METAS_PRIMERAINFANCIA
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE   VIEW [ViewInternal].[VW_METAS_PRIMERAINFANCIA]
AS
--SELECT COUNT(*) FROM [DataWareHouse_Clinical].[ViewInternal].[VW_METAS_PRIMERAINFANCIA]
SELECT DISTINCT  CASE
            ...
            END AS Regional,  ING.IFECHAING AS FECHA_INGRESO, ING.NUMINGRES AS INGRESO, LTRIM(RTRIM(CA.NOMCENATE)) AS CENTRO_ATENCION, U.UFUDESCRI AS UNIDAD_FUNCIONAL,
				...
				--RTRIM(LTRIM([INDIGO031].[ViewInternal].Edad(P.IPFECNACI, ING.IFECHAING)))  AS EdadDetalle,
                -- >>> Sustitución de UDF por cálculo inline (años + meses):
...
case when F.OutputDiagnosis in ('Z001','Z002','Z761','Z762') and CUPS.Code in ('890283','890263','890201') then 'Nroatenciones_Infancia' 
	 when F.OutputDiagnosis in ('Z001','Z002','Z761','Z762','Z008','Z108') and CUPS.Code in ('890205','890305') then 'Nroatenciones_Enfermeria' 
...
'K119','K120','K121','K122','K123','K130','K131','K132','K133','K134','K135','K136','K137','K140','K141','K142',
'K143','K144','K145','K146','K148','K149','Z012'/*,'Z001','Z002','Z761','Z762'*/) 
and CUPS.Code in ('890203') then 'Atenciones_Bucal' 

-- when F.OutputDiagnosis in ('Z001','Z002','Z761','Z762') and CUPS.Code in ('890205','890305') then 'AtencionEnfermeria'  
 --when F.OutputDiagnosis in ('Z001','Z002','Z761','Z762') and CUPS.Code in ('902211','902213','902210') then 'Anemia'  
 when F.OutputDiagnosis in ('Z391') and CUPS.Code in ('890205','890201','890305','890301','890206','890306') then 'Lactancia'  
 --when  CUPS.Code in ('P02CA03','P02CA01') then 'Desparasita'  
 --when  CUPS.Code in ('V06DX') then 'Micronutrientes'   
				
when F.OutputDiagnosis in ('K000','K001','K002','K003','K004','K005','K006','K007','K008','K009','K010','K011','K020','K021','K022','K023','K024','K025','K028','K029','K030','K031','K032','K033','K034','K035','K036',
'K037','K038','K039','K040','K041','K042','K043','K044','K045','K046','K047','K048','K049','K050','K051','K052','K053','K054','K055','K056','K060','K061','K062','K068','K069','K070','K071','K072','K073','K074',
'K075','K076','K078','K079','K080','K081','K082','K083','K088','K089','K090','K091','K092','K098','K099','K100','K101','K102','K103','K108','K109','K110','K111','K112','K113','K114','K115','K116','K117','K118',
'K119','K120','K121','K122','K123','K130','K131','K132','K133','K134','K135','K136','K137','K140','K141',
'K142','K143','K144','K145','K146','K148','K149','Z012','Z001','Z002','Z761','Z762') 
and CUPS.Code in ('997106','997103','997104') then 'Barniz'
...
'K119','K120','K121','K122','K123','K130','K131','K132','K133','K134','K135','K136','K137','K140','K141',
'K142','K143','K144','K145','K146','K148','K149','Z012','Z001','Z002','Z761','Z762') 
and CUPS.Code in ('997002') then 'Profilaxis'
...
'K119','K120','K121','K122','K123','K130','K131','K132','K133','K134','K135','K136','K137','K140','K141',
'K142','K143','K144','K145','K146','K148','K149','Z012','Z001','Z002','Z761','Z762') 
and CUPS.Code in ('997107') then 'Sellantes'  
...
when  CUPS.Code in ('990101','990102','990103','990104','990105','990106','990107','990108','990109','990110','990111','990112','990113') then 'Educacion_GrupalAdolescencia'
					 ...
					 end as Indicador , 'PrimeraInfancia' as Grupo
...
WHERE  (SOD.ServiceDate) >= '2024-01-01' and CUPS.Code in ('990101', '990102','990103','990104','990105','990106','990107','990108','990109','990110','990111','990112','990113',
...
'906611','906612','906610', '908890','890263','890363','890201','890301','890363','890350','890301','890363','890350','890301','903841','990201','890263','890363','892901','997104',
'903841','903895','903815','903816','903817','903818','903868','907106','890283','890263','890201','890205','890305',
'890203','890303','902213','902211','902210','997106','997103','997107','997002','997301','890205','890305','997101','997102','997107','P02CA03','P02CA01','890305','890301','890206',
'890306','V06DX','890205','890201','997104') 
and DATEDIFF(year, P.IPFECNACI, ING.IFECHAING) >= 0  and  DATEDIFF(year, P.IPFECNACI, ING.IFECHAING) <= 49  and 
...
,'Z123','Z124','Z000','Z008','Z003','Z001','Z002','Z761','Z762','Z012','Z001','Z002','Z761','Z762','Z391',
'Z008','Z108')
...
SELECT DISTINCT  CASE
            ...
            END AS Regional,  ING.IFECHAING AS FECHA_INGRESO, ING.NUMINGRES AS INGRESO, LTRIM(RTRIM(CA.NOMCENATE)) AS CENTRO_ATENCION, U.UFUDESCRI AS UNIDAD_FUNCIONAL,
                ...
                -- >>> Sustitución de UDF por cálculo inline (años + meses):
                CONCAT(
                  CASE WHEN DATEDIFF(YEAR, P.IPFECNACI, ING.IFECHAING) > 0
                       THEN CAST(DATEDIFF(YEAR, P.IPFECNACI, ING.IFECHAING) AS VARCHAR(10)) + ' año ' ELSE '' END,
                  CASE
                       WHEN DATEDIFF(MONTH, DATEADD(YEAR, DATEDIFF(YEAR, P.IPFECNACI, ING.IFECHAING), P.IPFECNACI), ING.IFECHAING) > 0
                       THEN CAST(DATEDIFF(MONTH, DATEADD(YEAR, DATEDIFF(YEAR, P.IPFECNACI, ING.IFECHAING), P.IPFECNACI), ING.IFECHAING) AS VARCHAR(10)) + ' mes(es) '
                       ELSE '' END
                ) AS EdadDetalle,
				CASE
					WHEN cast(DATEDIFF(year, P.IPFECNACI, ING.IFECHAING) as int) BETWEEN 0 AND 5 THEN 'Primera Infancia'
				...
				END AS Ciclo_Vida,
				SOD.ServiceDate as FechaPrestacionServicio,  CASE WHEN IPSEXOPAC = '1' THEN 'M' WHEN IPSEXOPAC = '2' THEN 'F' END  as [Género (F/M)],
				case  when ATC.Code in (select Codigo from [INDIGO031].[ViewInternal].[Desparasitaciones]) then 'Desparasitaciones'
					  when ATC.Code in (select Codigo from [INDIGO031].[ViewInternal].[Micronutrientes]) then 'Micronutrientes'
					  when ATC.Code in (select Codigo from [INDIGO031].[ViewInternal].[FortiMicronutrientes]) then 'FortiMicronutrientes' 
...
WHERE FECINIDOS>='2024-01-01' ) AS DX ON DX.IPCODPACI=F.PatientCode AND DX.Fecha=concat(MONTH(SO.OrderDate),'/',YEAR(SO.OrderDate)) AND DX.CODPRODUC=ATC.Code
WHERE  (SOD.ServiceDate) >= '2024-01-01' and ATC.Code in (select Codigo from [INDIGO031].[ViewInternal].[Desparasitaciones]
										union all 
										select Codigo from [INDIGO031].[ViewInternal].[Micronutrientes]
										union all 
										select Codigo from [INDIGO031].[ViewInternal].[FortiMicronutrientes]) 
--order by ING.IFECHAING DESC;
--ING.IPCODPACI IN ('20851115')
--ING.IFECHAING BETWEEN '2019-01-01' AND '2019-12-31' AND  GA.Code in ('BOG037', 'BOG038', 'BOG075')--ING.NUMINGRES='215531' O 88643')

--select *
--from INDIGO025.dbo.INDIAGNOS
