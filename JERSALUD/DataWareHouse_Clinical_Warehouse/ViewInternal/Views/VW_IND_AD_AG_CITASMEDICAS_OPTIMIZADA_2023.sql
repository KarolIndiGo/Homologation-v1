...
-- Schema: ViewInternal
-- Object: VW_IND_AD_AG_CITASMEDICAS_OPTIMIZADA_2023
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.VW_IND_AD_AG_CITASMEDICAS_OPTIMIZADA_2023
AS
...
INNER JOIN [INDIGO031].[dbo].[INPACIENT] AS PA
        ON PA.IPCODPACI = C.IPCODPACI
INNER JOIN [INDIGO031].[dbo].[INPROFSAL] AS P
        ON P.CODPROSAL = C.CODPROSAL
INNER JOIN [INDIGO031].[dbo].[INESPECIA] AS E
        ON P.CODESPEC1 = E.CODESPECI
INNER JOIN [INDIGO031].[dbo].[AGACTIMED] AS A
        ON A.CODACTMED = C.CODACTMED
INNER JOIN [INDIGO031].[dbo].[SEGusuaru] AS US
        ON US.CODUSUARI = C.CODUSUASI
LEFT OUTER JOIN [INDIGO031].[dbo].[ADCONCOEX] AS CE
        ON C.CODAUTONU = CE.NUMCONCIT
LEFT OUTER JOIN [INDIGO031].[Contract].[HealthAdministrator] AS EA
        ON PA.GENCONENTITY = EA.Id
INNER JOIN [INDIGO031].[dbo].[ADCENATEN] AS CA
        ON C.CODCENATE = CA.CODCENATE
LEFT OUTER JOIN [INDIGO031].[dbo].[RIASCUPS] AS RIC
        ON A.IDRIASCUPS = RIC.ID
LEFT OUTER JOIN [INDIGO031].[dbo].[RIAS] AS R
        ON RIC.IDRIAS = R.ID
LEFT JOIN [INDIGO031].[dbo].[SEGusuaru] AS UC
