...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_V_FACT_VentasJuridicas_2023
-- Extracted by Fabric SQL Extractor SPN v3.9.0




CREATE view [ViewInternal].[FQ45_V_FACT_VentasJuridicas_2023]
AS
SELECT 'FarmaQx' as Sede,t.Nit,
       ...
       END AS [Enero 2023],
       CASE
          WHEN sc.Month = '2' AND sc.Year = '2023'
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Febrero 2023],
       CASE
       ...
       END AS [Junio 2023],
       CASE
          WHEN sc.Month = '7' AND sc.Year = '2023'
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Julio 2023],
       CASE
       ...
       CASE
          ...
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Noviembre 2023],
       CASE
          WHEN sc.Month = '12' AND sc.Year = '2023'
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Diciembre 2023],
...
FROM GeneralLedger.GeneralLedgerBalance AS sc 
     INNER JOIN Common.ThirdParty AS t
        ON t.Id = sc.IdThirdParty AND sc.Year = '2023'
