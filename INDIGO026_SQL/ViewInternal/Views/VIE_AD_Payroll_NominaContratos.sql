...
TE.Name AS TipoEmpleado, 
           CASE p.IdentificationType WHEN 0 THEN 'Cédula de Ciudadanía' WHEN 1 THEN 'Cédula de Extranjería' WHEN 2 THEN 'Tarjeta de Identidad' WHEN 3 THEN 'Registro Civil' WHEN 4 THEN 'Pasporte' WHEN 5 THEN 'Adulto Sin Identificación' WHEN 6 THEN 'Menor Sin Identificación' WHEN 7 THEN 'Nit' END AS TipoIdentificacion, 
           TP.Nit AS NumeroIdentificacion, p.FirstName + ' ' + 
		   p.SecondName AS Nombres, p.FirstLastName + ' ' + 
		   p.SecondLastName AS Apellidos, co.BasicSalary AS Salario, 
		   ca.Code as Cod_Cargo, ca.Name AS Cargo, 
		   cc.Code as Cod_CentroCosto, cc.Name AS [Centro de Costo], 
		   fu.Code as Cod_UF, fu.Name AS UnidadFuncional, 
		   ...
		   -- co.ContractEndingDate AS [Fecha de Finalización], 
			...
			CASE cO.Status WHEN 1 THEN 'Activo' WHEN 2 THEN 'Liquidado' WHEN 3 THEN 'Anulado' WHEN 4 THEN 'Reemplazado' ELSE '' END AS Estado, 
		   cmr.name as RazonModificaciónContrato,
		   '' 
           AS [Fecha Retiro], '' AS [Razón del Retiro], ct.Name AS [Tipo de Contrato], g.Code + ' - ' + g.Name AS Grupo, wc.Name AS [Centro Trabajo], ba.Name AS Banco, co.BankAccountNumber AS [Cuenta Bancaria], p.BirthDate AS FechaNaciomiento, c.Name AS LugarExpedicionID, 
           p.IdentificationExpeditionDate AS FechaExpedicionID, c1.Name AS CiudadNacimiento, CASE p.Gender WHEN 1 THEN 'Masculino' WHEN 2 THEN 'Femenino' WHEN 3 THEN 'Otro' END AS Genero, p.BloodGroup AS [Grupo Sanguineo], p.RH, p.Dependents AS [Personas a cargo], 
           ...
           CASE cO.RowType WHEN 1 THEN 'Contrato Base' WHEN 2 THEN 'Novedad Contrato' ELSE '' END AS [Tipo de Registro], co.InitialContractNumber AS [Número de Contrato], CASE co.TrialPeriod WHEN 0 THEN 'NO' WHEN 1 THEN 'SI' END AS [Periodo Prueba], co.TrialPeriodTime AS [Tiempo Periodo Prueba (Dias)], 
           ...
           MAX(TEL.Phone) AS TelFijo, MAX(mov.Phone) AS TelMovil, salud.Name AS FondoSalud, Pension.Name AS FondoPension, Cesantia.Name AS Cesantia, Riesgo.Name AS Riesgos, Caja.Name AS [Caja Compensacion]--,Ciudad.Name AS Ciudad, ff.Name AS Departamento
FROM   Payroll.Employee AS E INNER JOIN
           ...
           Common.City AS c  ON c.Id = p.IdentificacionCityId INNER JOIN
           Common.City AS c1  ON c1.Id = p.BirthCityId LEFT OUTER JOIN
           Common.Address AS d ON d.IdPerson = p.Id LEFT OUTER JOIN
           --Common.City AS Ciudad  ON Ciudad.Id = d.CityId LEFT OUTER JOIN
           --Common.Department AS ff  ON ff.Id = d.DepartmentId LEFT OUTER JOIN
           Payroll.Contract AS co  ON co.EmployeeId = E.Id LEFT OUTER JOIN
           Payroll.Position AS ca  ON ca.Id = co.PositionId LEFT OUTER JOIN
           Payroll.FunctionalUnit AS fu  ON fu.Id = co.FunctionalUnitId INNER JOIN
           Payroll.ContractType AS ct  ON co.ContractTypeId = ct.Id INNER JOIN
           Payroll.[Group] AS g  ON co.GroupId = g.Id INNER JOIN
           ...
           Payroll.EmployeeType AS TE  ON TE.Id = E.EmployeeTypeId LEFT OUTER JOIN
           Payroll.ProfessionalRisk AS PSR  ON PSR.Id = ca.ProfessionalRiskLevelId
WHERE --(co.Status <> '4') and 
  co.valid='1' and co.status='1' --and  p.SecondName  like '%ashley%'
GROUP BY TE.Name, p.IdentificationType, TP.Nit, p.FirstName, p.SecondName, p.FirstLastName, p.SecondLastName, co.BasicSalary, ca.Code, ca.Name, cc.Code, cc.Name, fu.Code, fu.Name, co.JobBondingDate, co.ContractInitialDate, co.ContractEndingDate, co.Status, co.RetirementDate, rr.Name, ct.Name, g.Code, g.Name, wc.Name, 
           ba.Name, co.BankAccountNumber, p.BirthDate, c.Name, p.IdentificationExpeditionDate, c1.Name, p.Gender, p.BloodGroup, p.RH, p.Dependents, p.MaritalStatus, co.RowType, co.InitialContractNumber, co.TrialPeriod, co.TrialPeriodTime, E.DeclarantType, co.Status, cmr.Name, E.ProfessionalRiskPercentage, salud.Name, 
           Pension.Name, Cesantia.Name, Riesgo.Name, Caja.Name, --Ciudad.Name, ff.Name,
		   e.id,  RetirementReasonId
    ...
    cast( co.JobBondingDate as datetime) AS [Fecha de Contratación], 
	 ...
	 --      co.JobBondingDate AS [Fecha de Contratación], 
		  ...
		  -- cast( co.ContractEndingDate as datetime) AS [Fecha Retiro],
		   ...
		   ISNULL(rr.Name, '') AS [Razón del Retiro], ct.Name AS [Tipo de Contrato], g.Code + ' - ' + g.Name AS Grupo, wc.Name AS [Centro Trabajo], ba.Name AS Banco, co.BankAccountNumber AS [Cuenta Bancaria], p.BirthDate AS FechaNaciomiento, c.Name AS LugarExpedicionID, 
          p.IdentificationExpeditionDate AS FechaExpedicionID, c1.Name AS CiudadNacimiento, CASE p.Gender WHEN 1 THEN 'Masculino' WHEN 2 THEN 'Femenino' WHEN 3 THEN 'Otro' END AS Genero, p.BloodGroup AS [Grupo Sanguineo], p.RH, p.Dependents AS [Personas a cargo], 
           CASE p.MaritalStatus WHEN 0 THEN 'Soltero' WHEN 1 THEN 'Casado' WHEN 2 THEN 'Divorciado' WHEN 3 THEN 'Viudo' WHEN 4 THEN 'Union libre' END AS EstadoCivil, MAX(d.Addresss) AS Dirección, MAX(ema.Email) AS Email, 
           CASE cO.RowType WHEN 1 THEN 'Contrato Base' WHEN 2 THEN 'Novedad Contrato' ELSE '' END AS [Tipo de Registro], co.InitialContractNumber AS [Número de Contrato], '' AS [Periodo Prueba], '' AS [Tiempo Periodo Prueba (Dias)], 
           CASE DeclarantType WHEN 1 THEN 'No Declarante' WHEN 2 THEN 'Declarante' END AS Declarante, CASE cO.Status WHEN 1 THEN 'Mensual' WHEN 2 THEN 'Quincenal' ELSE '' END AS [Perido de Pago], ISNULL(cmr.Name, '') AS [Razón de la Modificación del Contrato], 
...
FROM   Payroll.Employee AS E INNER JOIN
          ...
          -- Common.City AS Ciudad  ON Ciudad.Id = d.CityId LEFT OUTER JOIN
           ...
           Common.Phone AS mov ON mov.IdPerson = p.Id AND mov.IdPhoneType = '2' LEFT OUTER JOIN
           Payroll.EmployeeType AS TE  ON TE.Id = E.EmployeeTypeId LEFT OUTER JOIN
           Payroll.ProfessionalRisk AS PSR  ON PSR.Id = co.ProfessionalRiskLevelId
WHERE (co.Status = '2') and 
  co.valid='0' --and  tp.nit='1013622116'
