...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_V_CXP_Extracto2024
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE view [ViewInternal].[FQ45_V_CXP_Extracto2024]
AS
SELECT        e.Code AS Identificación, t.Nit, t.DigitVerification AS [Digito Verificacion], t.Name AS [Nombre Tercero], tp.Code,
                         ...
                         CASE p.EntityName WHEN 'EntranceVoucher' THEN 'Comprobante Entrada' WHEN 'AccountPayable' THEN 'Cuentas X Pagar' WHEN 'InitialBalance' THEN 'Saldo Inicial' END AS Módulo, c.Number AS [Cuenta contable], 
                         c.Name AS [Descripción Cuenta Contable], p.Id AS [Id C x P], p.Code AS [Consecutivo C x P], 
...
WHERE        (p.Status <> '3') AND (p.CreationDate BETWEEN '2024-01-01' AND '2024-12-31')
