...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_V_Billing_FacturacionElectronica
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE view [ViewInternal].[FQ45_V_Billing_FacturacionElectronica]
AS
...
FROM INDIGO026.Billing.Invoice AS fa WITH (NOLOCK)
INNER JOIN INDIGO026.Billing.ElectronicDocument AS FE WITH (NOLOCK) 
...
LEFT JOIN (
    ...
    FROM INDIGO026.Billing.ElectronicDocumentDetail ed WITH (NOLOCK)

    JOIN (
        SELECT ElectronicDocumentID,
               MAX(CASE WHEN Status = 1 THEN Id ELSE NULL END) AS Estado1ID,
               MAX(Id) AS UltimoEstadoID
        FROM INDIGO026.Billing.ElectronicDocumentDetail WITH (NOLOCK)
...
LEFT JOIN INDIGO026.Common.OperatingUnit AS U WITH (NOLOCK) 
    ON U.Id = FE.OperatingUnitId
LEFT JOIN INDIGO026.Common.ThirdParty AS C WITH (NOLOCK) 
    ON C.Id = FE.CustomerPartyId
LEFT JOIN INDIGO026.Common.Person AS P WITH (NOLOCK) 
    ON P.IdentificationNumber = C.Nit
LEFT JOIN (
    SELECT IdPerson, MAX(Phone) AS Phone
    FROM INDIGO026.Common.Phone WITH (NOLOCK)
    WHERE IdPhoneType = 1
    GROUP BY IdPerson
) AS Tel ON Tel.IdPerson = P.Id
LEFT JOIN (
    SELECT IdPerson, MAX(Id) AS MAX_ID
    FROM INDIGO026.Common.Address WITH (NOLOCK)
    GROUP BY IdPerson
) AS Dir ON Dir.IdPerson = P.Id
LEFT JOIN INDIGO026.Common.Address AS Dir1 WITH (NOLOCK) 
    ON Dir1.Id = Dir.MAX_ID
LEFT JOIN (
    SELECT IdPerson, MAX(Email) AS Email
    FROM INDIGO026.Common.Email WITH (NOLOCK)
    GROUP BY IdPerson
) AS Email ON Email.IdPerson = P.Id
LEFT JOIN (
    SELECT InvoiceNumber, MAX(BillingNoteId) AS BillingNoteId
    FROM INDIGO026.Billing.BillingNoteDetail WITH (NOLOCK)
    GROUP BY InvoiceNumber
) AS ND ON ND.InvoiceNumber = fa.InvoiceNumber
LEFT JOIN INDIGO026.Billing.BillingNote AS nt WITH (NOLOCK) 
    ON nt.Id = ND.BillingNoteId
LEFT JOIN INDIGO026.dbo.Segusuaru AS us WITH (NOLOCK) 
