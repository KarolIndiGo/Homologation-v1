...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_V_FACT_VentasJuridicas_2022
-- Extracted by Fabric SQL Extractor SPN v3.9.0



CREATE view [ViewInternal].[FQ45_V_FACT_VentasJuridicas_2022]
AS
SELECT 'FarmaQx' as Sede,t.Nit,
       ...
       END AS [Enero 2022],
       CASE
          WHEN sc.Month = '2' AND sc.Year = '2022'
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Febrero 2022],
       CASE
       ...
       END AS [Junio 2022],
       CASE
          WHEN sc.Month = '7' AND sc.Year = '2022'
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Julio 2022],
       CASE
       ...
       CASE
          ...
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Noviembre 2022],
       CASE
          WHEN sc.Month = '12' AND sc.Year = '2022'
          THEN
             ((SUM (sc.DebitValue) - SUM (sc.CreditValue))) * -1
       END AS [Diciembre 2022],
...
FROM GeneralLedger.GeneralLedgerBalance AS sc 
     INNER JOIN Common.ThirdParty AS t
        ON t.Id = sc.IdThirdParty AND sc.Year = '2022'
