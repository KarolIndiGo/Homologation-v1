...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_AF_CuentaActivos
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE VIEW [ViewInternal].[FQ45_AF_CuentaActivos]
AS
SELECT pa.Id, pa.AdquisitionType, CASE pa.AdquisitionType WHEN '1' THEN (rtrim(ma1.number) + ' - ' + rtrim(ma1.name)) WHEN '3' THEN (rtrim(ma3.number) + ' - ' + rtrim(ma3.name)) WHEN '4' THEN (rtrim(ma1.number) + ' - ' + rtrim(ma1.name)) WHEN '7' THEN (rtrim(ma7.number) + ' - ' + rtrim(ma7.name)) WHEN '9' THEN (rtrim(ma9.number) 
           + ' - ' + rtrim(ma9.name)) END AS CuentaContable, pa.Plate, it.Code, it.Description, fit.Code AS CodeType, fit.Name AS NameType, pa.HistoricalValue, ISNULL(pad.Valorization, 0) AS Valorization, ISNULL(pad.Devaluation, 0) AS Devaluation, ISNULL(pad.DepreciatedValue, 0) AS DepreciatedValue, ISNULL(pad.ResidualValue, 0) 
