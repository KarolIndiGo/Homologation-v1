...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_V_INN_Comprobante_entrada_devolucion_2020
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[FQ45_V_INN_Comprobante_entrada_devolucion_2020]
AS
SELECT pr.Code AS Nit,
        pr.Name AS Proveedor,
        cce.Code AS Comprobante,
        CASE cce.OperatingUnitId WHEN '3' THEN 'BOGOTA' END AS Sede,
 ...
 FROM Inventory.EntranceVoucher AS cce 
      ...
      INNER JOIN Security.[User] AS u  ON u.UserCode = cce.CreationUser INNER JOIN
                 Security.Person AS per  ON per.Id = u.IdPerson INNER JOIN
                 Inventory.Warehouse AS al 
 ...
 WHERE (cce.Status = '2') AND (cce.InvoiceDate between '01/01/2018 00:00:00' and '31/12/2020 23:59:59')
