...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: FQ45_V_INN_ComprobanteEntradaDevolucion_Jersalud
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[FQ45_V_INN_ComprobanteEntradaDevolucion_Jersalud] AS
SELECT pr.Code AS Nit,
       pr.Name AS Proveedor,
       cce.Code AS Comprobante, cce.OperatingUnitId,
	   UO.UnitName AS Sede,
       ...
       p.Name AS Producto,
       ATC.Code AS [Codigo Medicamento],
       ...
       gff.Name AS [Grupo facturación],
       CASE p.ProductWithPriceControl
          WHEN '0' THEN 'No'
          WHEN '1' THEN 'Control precios'
       END AS [Maneja control precios],
       CASE p.POSProduct WHEN '0' THEN 'No' WHEN '1' THEN 'Si' END AS POS,
       ...
       dce.RTFValue AS Retefuente,                  per.Fullname AS Usuario,
       dev.Code AS [Cod Devolucion],
       dev.DocumentDate AS [Fecha Devolucion],
       dev.Description AS Detalle
FROM Inventory.EntranceVoucher AS cce 
     INNER JOIN Inventory.EntranceVoucherDetail AS dce ON dce.EntranceVoucherId = cce.Id AND cce.Status = '2'
     ...
     LEFT OUTER JOIN Billing.BillingGroup AS gff ON gff.Id = p.BillingGroupId
     ...
     LEFT OUTER JOIN Inventory.EntranceVoucherDevolution AS dev ON dev.EntranceVoucherId = cce.Id
	 LEFT OUTER JOIN Common.OperatingUnit as UO on UO.IdUnit = CCE.OperatingUnitId
WHERE (cce.Status = '2') AND YEAR(cce.InvoiceDate)>='2022' and AL.Code in ('0037','0071','0072')
