...
CREATE VIEW [ViewInternal].[VIE_AD_Payments_TrasladoCuentas] AS
...
FROM   Payments.AccountPayable AS A  INNER JOIN
           ...
           Security.Person AS P  ON U.IdPerson = P.Id LEFT JOIN
		   [Treasury].[SchedulePaymentDetail] AS spd on spd.AccountPayableId = a.id left join
		   Treasury.VoucherTransaction as vt on vt.Id = spd.VoucherTransactionId
