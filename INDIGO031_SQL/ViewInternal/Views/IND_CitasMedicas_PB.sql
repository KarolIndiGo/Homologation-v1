...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: IND_CitasMedicas_PB
-- Extracted by Fabric SQL Extractor SPN v3.9.0



CREATE VIEW [ViewInternal].[IND_CitasMedicas_PB] AS	
	SELECT DISTINCT *
	FROM (
	SELECT               
...
'Z345','Z346','Z347','Z348','Z349','Z350','Z351','Z352','Z353','Z354','Z355','Z356','Z357','Z358','Z359','Z391', 'Z392', 'Z718', 'Z719') then 'RPYM'  --se agrega RPYM  --Pertenece a RPYMM
 ...
 C.CODAUTONU AS IdCita, A.DESACTMED AS Actividad, ce.*
     ...
     FROM dbo.AGASICITA AS C  
          ...
          LEFT OUTER JOIN dbo.ADCONCOEX as ce on c.IPCODPACI=ce.IPCODPACI AND CE.CONESTADO = '3' AND CE.IPFECHACO>='2023' and cee.CODCONCEC = ce.CODCONCEC
          ...
          LEFT JOIN dbo.HCHISPACA AS H   ON H.NUMINGRES = CE.NUMINGRES and h.id=ce.IDHCHISPACA AND YEAR(H.FECHISPAC) >='2023'
        
     WHERE --C.FECHORAIN BETWEEN '2020-12-01' AND '2021-02-01'
     year(C.FECHORAIN)>=2023)  --and  month(C.FECHORAIN)>=7) 
	  AS A 
