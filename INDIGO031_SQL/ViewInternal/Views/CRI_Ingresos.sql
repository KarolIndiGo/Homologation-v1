...
CREATE VIEW [ViewInternal].[CRI_Ingresos]  AS  
SELECT 
--CASE
--     WHEN i.codcenate IN ( '001', '00101','00102', '00103' ) THEN 'Neiva'
--WHEN i.codcenate IN ( '00104' ) THEN 'Abner'
--WHEN i.codcenate IN ( '00105' ) THEN 'Materno'
--         WHEN i.codcenate IN ( '002', '00201' ) THEN 'Florencia'
--         WHEN i.codcenate IN ( '004','00402','00403', '00401' ) THEN 'Tunja'
--WHEN i.codcenate IN ( '007' ) THEN 'Facatativa'
--  WHEN i.codcenate IN ( '003' ) THEN 'Pitalito'
--END AS Sucursal,
...
FROM            dbo.ADINGRESO AS i WITH (NOLOCK) INNER JOIN
                         ...
                         --[Security].[User] AS u WITH (nolock) ON u.UserCode = i.CODUSUCRE LEFT OUTER JOIN
                         --[Security].[User] AS um WITH (nolock) ON uM.UserCode = i.codusumod LEFT OUTER JOIN
                         --[Security].Person AS per WITH (nolock) ON per.Id = u.IdPerson LEFT OUTER JOIN
                         --[Security].Person AS PERM WITH (nolock) ON PERM .Id = um.IdPerson LEFT OUTER JOIN
           ...
           Common.ThirdParty AS t ON t.Id = ea.ThirdPartyId LEFT OUTER JOIN
                         ...
                         dbo.INMUNICIP AS EE WITH (NOLOCK) ON EE.DEPMUNCOD = BB.DEPMUNCOD LEFT OUTER JOIN
						  dbo.INDEPARTA AS DEP WITH (NOLOCK) ON DEP.depcodigo = ee.DEPCODIGO 
left outer join .adcenaten as caa with (nolock) on caa.codcenate=I.codcenate
LEFT OUTER JOIN .INPROFSAL AS PRO WITH (NOLOCK) ON PRO.CODPROSAL=em.CODPROSAL
 --LEFT OUTER JOIN  ReportesFinancieros.[DF].[Contract_EntidadesAdministradoras] as a on a.id=I.GENCONENTITY--LEFT OUTER JOIN
--Billing.RevenueControl AS RC WITH (NOLOCK) ON RC.AdmissionNumber = I.NUMINGRES LEFT OUTER JOIN
--Billing.RevenueControlDetail AS rcd WITH (nolock) ON rcd.RevenueControlId = RC.Id AND I.GENCONENTITY = rcd.HealthAdministratorId
WHERE       -- (i.CODCENATE IN ('001', '00101','00102','00103','00104','002','00201','004','00401','00402','00403','003','00105')) and  
(i.IESTADOIN <> 'A') and i.IFECHAING>='01-02-2025'
