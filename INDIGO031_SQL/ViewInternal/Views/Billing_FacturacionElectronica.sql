...
FROM INDIGO031.Billing.Invoice AS fa WITH (NOLOCK)
INNER JOIN INDIGO031.Billing.ElectronicDocument AS FE WITH (NOLOCK) 
...
LEFT JOIN (
    ...
    FROM INDIGO031.Billing.ElectronicDocumentDetail ed WITH (NOLOCK)

    JOIN (
        SELECT ElectronicDocumentID,
               MAX(CASE WHEN Status = 1 THEN Id ELSE NULL END) AS Estado1ID,
               MAX(Id) AS UltimoEstadoID
        FROM INDIGO031.Billing.ElectronicDocumentDetail WITH (NOLOCK)
...
LEFT JOIN INDIGO031.Common.OperatingUnit AS U WITH (NOLOCK) 
    ON U.Id = FE.OperatingUnitId
LEFT JOIN INDIGO031.Common.ThirdParty AS C WITH (NOLOCK) 
    ON C.Id = FE.CustomerPartyId
LEFT JOIN INDIGO031.Common.Person AS P WITH (NOLOCK) 
    ON P.IdentificationNumber = C.Nit
LEFT JOIN (
    SELECT IdPerson, MAX(Phone) AS Phone
    FROM INDIGO031.Common.Phone WITH (NOLOCK)
    WHERE IdPhoneType = 1
    GROUP BY IdPerson
) AS Tel ON Tel.IdPerson = P.Id
LEFT JOIN (
    SELECT IdPerson, MAX(Id) AS MAX_ID
    FROM INDIGO031.Common.Address WITH (NOLOCK)
    GROUP BY IdPerson
) AS Dir ON Dir.IdPerson = P.Id
LEFT JOIN INDIGO031.Common.Address AS Dir1 WITH (NOLOCK) 
    ON Dir1.Id = Dir.MAX_ID
LEFT JOIN (
    SELECT IdPerson, MAX(Email) AS Email
    FROM INDIGO031.Common.Email WITH (NOLOCK)
    GROUP BY IdPerson
) AS Email ON Email.IdPerson = P.Id
LEFT JOIN (
    SELECT InvoiceNumber, MAX(BillingNoteId) AS BillingNoteId
    FROM INDIGO031.Billing.BillingNoteDetail WITH (NOLOCK)
    GROUP BY InvoiceNumber
) AS ND ON ND.InvoiceNumber = fa.InvoiceNumber
LEFT JOIN INDIGO031.Billing.BillingNote AS nt WITH (NOLOCK) 
    ON nt.Id = ND.BillingNoteId
LEFT JOIN INDIGO031.dbo.Segusuaru AS us WITH (NOLOCK) 
