...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: V_INN_DispensacionFacturacion
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[V_INN_DispensacionFacturacion] AS

SELECT   UO.UnitName AS Sede, D.Code AS Documento, 'Dispensacion' AS [Tipo documento], D.DocumentDate AS Fecha, CASE WHEN (MONTH(D .DocumentDate)) = '1' AND (YEAR(D .DocumentDate)) = '2025' THEN 'Ene25' WHEN (MONTH(D .DocumentDate)) = '2' AND (YEAR(D .DocumentDate)) 
           ...
           (YEAR(D .DocumentDate)) = '2025' THEN 'Dic25' END AS Mes, P.IPCODPACI AS Identificacion, P.IPNOMCOMP AS Paciente, pr.Code AS Cod_Producto, pr.Name AS Producto, pr.CodeAlternative AS [Código alterno], pr.CodeAlternativeTwo AS [Código alterno 2], sg.Code + ' - ' + sg.Name AS [Código Subgrupo], 
...
WHERE (D.DocumentDate >= '01/01/2025 00:00:00') AND (D.Status = '2') AND (DI.Quantity - DI.ReturnedQuantity <> '0') 
