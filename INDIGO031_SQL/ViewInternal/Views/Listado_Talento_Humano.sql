...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: Listado_Talento_Humano
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE VIEW [ViewInternal].[Listado_Talento_Humano] AS
...
FROM Payroll.Employee AS E
		...
		LEFT OUTER JOIN	(SELECT IDPERSON,/* MAX(Addresss) as Addresss, departmentid,*/ max(id) as idd
					FROM Common.Address --where IdPerson='81599'
					group by idperson) AS de ON de.IdPerson = p.Id 
		LEFT OUTER JOIN Common.Address AS d ON d.IdPerson = de.IdPerson and de.idd = d.id
		LEFT OUTER JOIN Common.City AS Ciudad ON Ciudad.Id = d.CityId
		--LEFT OUTER JOIN	(SELECT IDPERSON,/* MAX(Addresss) as Addresss departmentid,*/ max(id) as idd
		--			FROM Common.Address where IdPerson='81599'  --select * from Common.Address
		--			group by idperson) AS de ON de.IdPerson = p.Id 
		...
		left outer join Payroll.StudyType as es on pp.StudyTypeId = es.id
