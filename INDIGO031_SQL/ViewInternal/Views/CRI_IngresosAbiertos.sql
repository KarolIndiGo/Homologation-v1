...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: CRI_IngresosAbiertos
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[CRI_IngresosAbiertos]  
AS  
     SELECT DISTINCT   
     ...
     FROM dbo.INPACIENT AS A   
          INNER JOIN dbo.ADINGRESO AS I  ON A.IPCODPACI = I.IPCODPACI  
                                                                          AND I.IESTADOIN = ''  
          LEFT JOIN Billing.ServiceOrder AS O  ON O.AdmissionNumber = I.NUMINGRES  
          LEFT JOIN Billing.ServiceOrderDetail AS OD  ON OD.ServiceOrderId = O.Id  
          INNER JOIN Contract.CareGroup AS GA  ON GA.Id = OD.CareGroupId  
          ...
          LEFT JOIN dbo.HCHISPACA AS H  ON H.NUMINGRES = I.NUMINGRES
		  LEFT JOIN dbo.INDIAGNOS AS CIE10  ON CIE10.CODDIAGNO = H.CODDIAGNO
	WHERE GA.Name NOT LIKE '%MEDICAMENTOS%'
