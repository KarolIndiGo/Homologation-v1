...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: IND_Indicador_Laboratorios_PB
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW [ViewInternal].[IND_Indicador_Laboratorios_PB] AS
...
'Z345','Z346','Z347','Z348','Z349','Z350','Z351','Z352','Z353','Z354','Z355','Z356','Z357','Z358','Z359','Z391', 'Z392', 'Z718', 'Z719') then 'RPYM'  --se agrega RPYM  --Pertenece a RPYMM
 ...
 END                                   AS CursoVida,
     ...
     FROM dbo.HCORDLABO AS I WITH (NOLOCK)
          INNER JOIN dbo.ADCENATEN AS ca WITH (NOLOCK)  ON I.CODCENATE = ca.CODCENATE
          INNER JOIN dbo.INPROFSAL AS M WITH (NOLOCK) ON I.CODPROSAL = M.CODPROSAL
          INNER JOIN dbo.INESPECIA AS EI WITH (NOLOCK) ON M.CODESPEC1 = EI.CODESPECI
		  inner join dbo.INCUPSIPS as cu WITH (NOLOCK) on cu.CODSERIPS=i.CODSERIPS
 		  LEFT OUTER JOIN dbo.INDIAGNOS AS DX WITH (NOLOCK) ON DX.CODDIAGNO = I.CODDIAGNO
          LEFT OUTER JOIN ViewInternal.IND_RedProfesionales_Jer AS Red WITH (NOLOCK) ON m.CODESPEC1 = Red.CODESPECI
...
----INNER JOIN	dbo.ADINGRESO AS IG  ON IG.NUMINGRES = I.NUMINGRES 
