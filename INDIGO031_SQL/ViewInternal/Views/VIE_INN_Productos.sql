...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: VIE_INN_Productos
-- Extracted by Fabric SQL Extractor SPN v3.9.0


CREATE VIEW [ViewInternal].[VIE_INN_Productos] AS
     SELECT DISTINCT 
			pr.Id, 
            pr.Code AS Còdigo, 
            pr.Name AS Nombre, 
     ...
     FROM Inventory.InventoryProduct AS pr 
          INNER JOIN Inventory.ProductType AS tpr  ON tpr.Id = pr.ProductTypeId AND pr.STATUS = 1
          INNER JOIN Inventory.ProductGroup AS gp  ON gp.Id = pr.ProductGroupId
          INNER JOIN Inventory.ProductSubGroup AS sg  ON sg.Id = pr.ProductSubGroupId
          INNER JOIN GeneralLedger.MainAccounts AS cuenta  ON cuenta.Id = gp.IncomeAccountId
          INNER JOIN Inventory.PackagingUnit AS ue  ON ue.Id = pr.PackagingUnitId
          ...
          LEFT OUTER JOIN [Security].[User] AS s ON s.UserCode = pr.CreationUser
          LEFT OUTER JOIN [Security].Person AS sp ON sp.Id = s.IdPerson
          LEFT OUTER JOIN GeneralLedger.GeneralLedgerIVA AS IVA ON IVA.Id = pr.IVAId
          LEFT OUTER JOIN Inventory.ATC AS catc  ON catc.Id = pr.ATCId AND catc.STATUS = 1
		  LEFT OUTER JOIN Inventory.DCI AS dci  ON catc.DCIId = dci.Id AND dci.STATUS = 1
		  LEFT OUTER JOIN Inventory.InventorySupplie AS ins  ON ins.Id = pr.SupplieId 
          LEFT OUTER JOIN Inventory.ATCEntity AS ATC ON catc.ATCEntityId = ATC.Id
          LEFT OUTER JOIN [Security].[User] AS s1 ON s1.UserCode = catc.CreationUser
          LEFT OUTER JOIN [Security].Person AS sp1 ON sp1.Id = s1.IdPerson
          LEFT OUTER JOIN [Security].[User] AS s2 ON s2.UserCode = catc.ModificationUser
          LEFT OUTER JOIN [Security].Person AS sp2 ON sp2.Id = s2.IdPerson
          LEFT OUTER JOIN [Security].[User] AS s3 ON s3.UserCode = pr.ModificationUser
          LEFT OUTER JOIN [Security].Person AS sp3 ON sp3.Id = s3.IdPerson
--where pr.Id='129'
