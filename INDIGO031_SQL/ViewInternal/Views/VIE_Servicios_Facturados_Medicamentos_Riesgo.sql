...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: VIE_Servicios_Facturados_Medicamentos_Riesgo
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW  [ViewInternal].[VIE_Servicios_Facturados_Medicamentos_Riesgo] AS
...
SELECT   -- ROW_NUMBER() OVER(ORDER BY MF.Number ASC) AS Row#, 627.716  627.776
   ...
   --         END AS Tipo_Liquidacion, 
			...
			ISNULL((SELECT SUM(MFDIF.PendingQuantity)FROM Inventory.MedicalFormulaDetailDeferred AS MFDIF WHERE DeliveryDate<=GETDATE()AND MFDIF.MedicalFormulaDetailId=MFD.Id),MFD.PendingQuantity) AS Cantidad_Pendiente_Real--CASO 342646
--SELECT *
   FROM Inventory.MedicalFormula AS MF WITH (NOLOCK)
		  ...
		  INNER JOIN dbo.ADINGRESO AS AI  ON AI.NUMINGRES=O.AdmissionNumber and ai.IFECHAING >= '01-01-2025 00:00:00'
		  ...
		  LEFT JOIN Inventory.MedicalFormulaDetail AS MFD WITH (NOLOCK) ON MFD.MedicalFormulaId=MF.Id AND MFD.ProductCode = M.Code
