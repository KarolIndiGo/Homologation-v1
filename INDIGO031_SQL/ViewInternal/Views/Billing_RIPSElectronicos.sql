...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: Billing_RIPSElectronicos
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW ViewInternal.Billing_RIPSElectronicos AS
SELECT DISTINCT 
    ...
    -- Tipo de cobertura en salud (Resolución 3047)
    CASE
...
FROM 
    ...
    INNER JOIN ViewInternal.Billing_TrazabilidadRIPS_Gral AS NCV 
        ON f.[InvoiceNumber] = NCV.NroFactura AND EstadoRIPS = 'Validado'
    LEFT OUTER JOIN Common.OperatingUnit AS up 
        ON up.Id = f.OperatingUnitId
    LEFT OUTER JOIN ADINGRESO AS ing 
        ON ing.NUMINGRES = f.AdmissionNumber
    LEFT OUTER JOIN Portfolio.AccountReceivable AS cp 
        ON cp.InvoiceNumber = f.InvoiceNumber
    LEFT OUTER JOIN INPACIENT AS pa 
...
--select TOP  (8)  * from Billing.Invoice

------select * from dbo.INPACIENT

----select * from Common.OperatingUnit

--select TOP  (8) * FROM dbo.ADINGRESO

--select TOP  (8) * from Portfolio.AccountReceivable

--select * from dbo.HCJUNOPMH
--select * from dbo.HCJUNOPMI