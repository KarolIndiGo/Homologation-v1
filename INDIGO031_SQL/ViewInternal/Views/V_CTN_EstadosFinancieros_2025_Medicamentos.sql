...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: V_CTN_EstadosFinancieros_2025_Medicamentos
...
CREATE VIEW [ViewInternal].[V_CTN_EstadosFinancieros_2025_Medicamentos]
AS
...
FROM   GeneralLedger.GeneralLedgerBalance AS B 
	   INNER JOIN GeneralLedger.MainAccounts AS C ON B.IdMainAccount = C.Id AND B.Year = '2025'
	   INNER JOIN GeneralLedger.MainAccountClasses AS cl ON C.IdAccountClass = cl.Id AND C.LegalBookId = 1 AND cl.Code BETWEEN '4' AND '6' 
	   LEFT JOIN GeneralLedger.MainAccounts AS C1 ON C.IdParent = C1.Id AND C1.LegalBookId =  1 
	   LEFT JOIN GeneralLedger.MainAccounts AS C2 ON C1.IdParent = C2.Id AND C2.LegalBookId = 1 -- se ajusto para que trajera el libro NIIF a peticion del caso 299286
	   LEFT JOIN GeneralLedger.MainAccounts AS C3 ON C2.IdParent = C3.Id AND C3.LegalBookId = 1 
	   LEFT JOIN GeneralLedger.MainAccounts AS C4 ON C3.IdParent = C4.Id AND C4.LegalBookId = 1 
