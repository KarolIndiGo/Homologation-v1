...
-- ItemId: SPN
-- Schema: Report
-- Object: View_HDSAP_IMAGENOLOGIA
...
CREATE VIEW [Report].[View_HDSAP_IMAGENOLOGIA]
AS
...
SELECT                CASE P.IPTIPODOC
                      ...
                      I.IPCODPACI AS Cedula, P.IPNOMCOMP AS Nombre, I.NUMINGRES AS Ingreso, U.UFUDESCRI AS UnidadSolicita, I.FECORDMED AS FechaOrden, 
                      ...
                      CASE WHEN ESTSERIPS = '1' THEN 'Solicitado' WHEN ESTSERIPS = '2' THEN 'Muestra Recolectada' WHEN ESTSERIPS = '3' THEN 'Examen Tomado' WHEN ESTSERIPS
                       ...
                       EstadoExamen, I.INTERPRET AS Interpretación, I.CODPROINT AS CodigoProfesionalInterpreto, I.NUMFOLINT AS NroFolioInterpreto, 
                      CASE WHEN SERTRANSC = 0 THEN 'Sin Lectura' WHEN SERTRANSC = 1 THEN 'Con Lectura' END AS LecturaRadiologo, 
					  ...
					  ha.name as Entidad,
						 ...
						 END as Régimen
FROM                  HCORDIMAG AS I INNER JOIN
