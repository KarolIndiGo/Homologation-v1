...
-- ItemId: SPN
-- Schema: Report
-- Object: View_HDSAP_CONSUMO_PROMEDIO_FARMACIA
...
CREATE VIEW [Report].[View_HDSAP_CONSUMO_PROMEDIO_FARMACIA]
AS
...
WITH ConsumosMensuales AS (
	...
	JOIN Inventory.PhysicalInventory ph on ph.productid = pro.id
	JOIN Inventory.Warehouse w on w.id = ph.WarehouseId
    LEFT JOIN Inventory.atc atc ON atc.id = pro.ATCId
    WHERE YEAR(k.DocumentDate) in( '2024')  and k.MovementType = 2 and k.EntityName = 'PharmaceuticalDispensing'-- and pro.code = 'DM-00226'
    GROUP BY pro.code, pro.name, atc.code, atc.Name, pro.CodeCUM, pro.ProductTypeId, MONTH(k.DocumentDate),	w.Name
)
SELECT 
    CodigoProducto,
    NombreProducto,
