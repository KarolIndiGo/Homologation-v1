...
INGRESO_INICIAL AS
 (
  ...
  FROM 
   CHREGESTA AS ES
   INNER JOIN (SELECT EST.IPCODPACI,  EST.NUMINGRES,  MIN(ID) ID FROM CHREGESTA AS EST GROUP BY EST.IPCODPACI, EST.NUMINGRES) AS G ON G.ID = ES.ID
...
SELECT  --COUNT(*) 
 ...
 NIV.NIVDESCRI AS [NIVEL O ESTRACTO],
 CASE PAC.IPTIPOAFI WHEN 0 THEN 'No Aplica'
					...
					WHEN 5 THEN 'Pensionado' END AS [TIPO DE AFILIACIÓN],
...
TIP.NOMBRE AS [DESCRIPCION IDENTIFICACION], 
 ...
 CASE WHEN FLOOR((CAST(CONVERT(VARCHAR(8), HC.FECALTPAC, 112) AS INT) - CAST(CONVERT(VARCHAR(8), PAC.IPFECNACI, 112) AS INT)) / 10000)<=5 THEN '0-5 Años' 
	  WHEN FLOOR((CAST(CONVERT(VARCHAR(8), HC.FECALTPAC, 112) AS INT) - CAST(CONVERT(VARCHAR(8), PAC.IPFECNACI, 112) AS INT)) / 10000)>=6 AND FLOOR((CAST(CONVERT(VARCHAR(8), HC.FECALTPAC, 112) AS INT) - CAST(CONVERT(VARCHAR(8), PAC.IPFECNACI, 112) AS INT)) / 10000)<=11 THEN '6-11 Años' 
	  WHEN FLOOR((CAST(CONVERT(VARCHAR(8), HC.FECALTPAC, 112) AS INT) - CAST(CONVERT(VARCHAR(8), PAC.IPFECNACI, 112) AS INT)) / 10000)>=12 AND FLOOR((CAST(CONVERT(VARCHAR(8), HC.FECALTPAC, 112) AS INT) - CAST(CONVERT(VARCHAR(8), PAC.IPFECNACI, 112) AS INT)) / 10000)<=17 THEN '12-17 Años' 
 ...
 CASE UNI.UFUTIPUNI WHEN 1 THEN 'URGENCIAS' 
					...
					WHEN 30 THEN 'OTRAS'
					WHEN 31 THEN 'CONSULTA PRIORITARIA' END AS [TIPO UNIDAD], 
 ...
 ING.CODDIAING AS [CODIGO DX INGRESO], 
 RTRIM(DIA.NOMDIAGNO) AS [DX INGRESO],  
 CASE EI.CODTIPEST WHEN '001' THEN 'GENERAL' 
				   WHEN '002' THEN 'CUIDADO BASICO' 
				   WHEN '003' THEN 'CUIDADO INTERMEDIO' ELSE 'CUIDADO INTENSIVO' END AS [TIPO ESTANCIA],
 ...
 ING.CODDIAEGR AS [CODIGO DX EGRESO], 
 RTRIM(DIAE.NOMDIAGNO) AS [DX EGRESO], 
 ...
 CAST(CTUE.[FECHA EGRESO] as date) AS [FECHA EGRESO], 
 CONVERT(NVARCHAR, CAST(CTUE.[FECHA EGRESO] as time), 8) [HORA EGRESO], 
 --CONVERT(VARCHAR,DATEDIFF(MINUTE,CING.[FECHA INGRESO],HC.FECALTPAC)/60)+','+CONVERT(VARCHAR,DATEDIFF(MINUTE,CING.[FECHA INGRESO],HC.FECALTPAC)%60) AS [FECHA INGRESO VS FECHA DE ALTA EN HORAS],
 CONVERT(VARCHAR,DATEDIFF(MINUTE,CING.[FECHA INGRESO],HC.FECALTPAC)/60)+','+REPLACE(CONVERT(VARCHAR,CONVERT(INT,ROUND((DATEDIFF(MINUTE,CING.[FECHA INGRESO],HC.FECALTPAC)%60)/0.60,0))),'-','') AS [FECHA INGRESO VS FECHA DE ALTA EN HORAS],
 CONVERT(VARCHAR,DATEDIFF(HOUR,CING.[FECHA INGRESO],HC.FECALTPAC)/24)+','+REPLACE(CONVERT(VARCHAR,CONVERT(INT,ROUND((DATEDIFF(HOUR,CING.[FECHA INGRESO],HC.FECALTPAC)%24)/0.24,0))),'-','') AS [FECHA INGRESO VS FECHA DE ALTA EN DIAS],
 CONVERT(VARCHAR,DATEDIFF(MINUTE,HC.FECALTPAC,CTUE.[FECHA EGRESO])/60)+','+REPLACE(CONVERT(VARCHAR,CONVERT(INT,ROUND((DATEDIFF(MINUTE,HC.FECALTPAC,CTUE.[FECHA EGRESO])%60)/0.60,0))),'-','') AS [FECHA ALTA MEDICA VS FECHA DE EGRESO EN HORAS],
...
LEFT JOIN Billing.SlipOut SAL ON HC.NUMINGRES=SAL.AdmissionNumber
...
--WHERE HC.NUMINGRES='119556'
--where hc.IPCODPACI='79749386' and hc.NUMINGRES='105634'
