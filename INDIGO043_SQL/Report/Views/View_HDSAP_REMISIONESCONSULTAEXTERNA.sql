...
-- ItemId: SPN
-- Schema: Report
-- Object: View_HDSAP_REMISIONESCONSULTAEXTERNA
...
CREATE VIEW [Report].[View_HDSAP_REMISIONESCONSULTAEXTERNA]
AS
...
select                   distinct
                         pa.IPPRINOMB PrimerNombre,
						 pa.IPSEGNOMB SegundoNombre,
						 pa.IPPRIAPEL PrimerApellido,
						 pa.IPSEGAPEL SegundoApellido,
						 CASE pa.IPTIPODOC
                             WHEN '1'
                             THEN 'CC'
                             WHEN '2'
                      ...
                      -- CONVERT(CHAR(10), i.IFECHAING,103) AS FechaIngr,
                       --CONVERT(CHAR(5), i.IFECHAING,108) AS Hora,
						...
						e1.DESESPECI EspecialidadTratante,
						 E.DESESPECI AS EspecialidadRemite,
						 I.CODDIAING AS 'DIAGNOSTICO INGRESO',
						 d.NOMDIAGNO as 'Nombre Diagnostico',
...
from HCORDINTE hc
JOIN INPACIENT AS Pa ON Pa.IPCODPACI = hc.IPCODPACI 
join ADINGRESO AS I on i.NUMINGRES = hc.NUMINGRES
JOIN INUBICACI AS M ON M.AUUBICACI = Pa.AUUBICACI 
JOIN INENTIDAD AS A ON A.CODENTIDA = i.CODENTIDA
JOIN INUNIFUNC p on p.UFUCODIGO = hc.UFUCODIGO
JOIN INPROFSAL PRO ON PRO.CODPROSAL = HC.CODPROSAL
LEFT OUTER JOIN INMUNICIP AS L ON L.DEPMUNCOD = M.DEPMUNCOD 
LEFT OUTER JOIN INDIAGNOS AS D ON D.CODDIAGNO = I.CODDIAING 
LEFT OUTER JOIN INDIAGNOS AS D2 ON D2.CODDIAGNO = I.CODDIAEGR 
LEFT OUTER JOIN INESPECIA AS E ON E.CODESPECI = HC.CODESPECI
LEFT OUTER JOIN INESPECIA AS E1 ON E1.CODESPECI = PRO.CODESPEC1
 where hc.ESTSERIPS = 4 and p.UFUDESCRI like '%ext%'--  and hc.IPCODPACI = '36270479' and  i.IFECHAING between '2024-02-01' and '2024-02-29'
