...
CREATE VIEW [Report].[View_HDSAP_EUTRIAGE]
AS
...
SELECT        T.IPCODPACI AS Documento, C.IPCODPACI AS IdControl, T.NUMINGRES AS Ingreso, C.IPNOMCOMP AS NombrePaciente, C.IPFECLLEGA AS [ArriboUrgencias(F1)], T.TRIAFECHA AS [Triage(F2)], 
                         HCURGING1.FECHINIHI AS [LlamadoConsulta(F3)], H.FECHISPAC AS [FinConsulta(F4)], t2.fecha AS [Revaloracion(F5)], DATEDIFF(MINUTE, C.IPFECLLEGA, T.TRIAFECHA) AS [F1-F2], DATEDIFF(MINUTE, 
                         T.TRIAFECHA, H.FECHISPAC) AS [F2-F4], DATEDIFF(MINUTE, T.TRIAFECHA, HCURGING1.FECHINIHI) AS [F2-F3], DATEDIFF(MINUTE, H.FECHISPAC, HCURGING1.FECHINIHI) AS [F4-F3], 
                         DATEDIFF(MINUTE, C.IPFECLLEGA, H.FECHISPAC) AS [F1-F4], DATEDIFF(MINUTE, C.IPFECLLEGA, HCURGING1.FECHINIHI) AS [F1-F3], DATEDIFF(MINUTE, H.FECHISPAC, t2.fecha) AS [F4-F5], 
                         T.TRIAGEDAD AS Edad, 
                         ...
                         UF.UFUDESCRI AS UnidadFuncional, HCURGING1.FECINIATE AS FechaInicioAten, T.TRIACAUIN AS CausaIngreso
FROM            ADTRIAGEU AS T LEFT OUTER JOIN
                         HCHISPACA AS H ON T.IPCODPACI = H.IPCODPACI AND T.NUMINGRES = H.NUMINGRES INNER JOIN
                         ...
                         INPACIENT ON T.IPCODPACI = INPACIENT.IPCODPACI INNER JOIN
                         HCURGING1 ON H.NUMEFOLIO = HCURGING1.NUMEFOLIO AND H.NUMINGRES = HCURGING1.NUMINGRES AND 
                         H.IPCODPACI = HCURGING1.IPCODPACI and HCURGING1.CODPROSAL = P.CODPROSAL LEFT OUTER JOIN
					     ...
					     INPROFSAL AS P2 ON t2.CODPROSAL = P2.CODPROSAL
WHERE        (C.UFUCODIGO IN ('01', '038', '0352')) AND (H.TIPHISPAC = 'I') AND (HCURGING1.FECINIATE > CONVERT(DATETIME, '2015-12-31 00:00:00', 102))
