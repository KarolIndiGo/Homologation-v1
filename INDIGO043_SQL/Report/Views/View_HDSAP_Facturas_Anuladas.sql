...
-- ItemId: SPN
-- Schema: Report
-- Object: View_HDSAP_Facturas_Anuladas
...
CREATE VIEW [Report].[View_HDSAP_Facturas_Anuladas]
AS
...
SELECT        i.InvoiceDate AS ff, ad.IPCODPACI AS Cedula, pa.IPNOMCOMP AS Nombre, ad.NUMINGRES AS Ingreso, ic.Code + ' - ' + ic.Name as Categoria, cg.Code + ' - ' + cg.Name AS GrupoAtencion, 
						 ...
						 ,j.Consecutive as 'Numero Comprobate',SUM(jd.DebitValue) as 'Valor Afectado'
FROM            Billing.Invoice AS i INNER JOIN
				Billing.InvoiceCategories ic on ic.Id = i.InvoiceCategoryId inner join
						 ...
						 LEFT JOIN GeneralLedger.JournalVouchers j on j.EntityCode= i.InvoiceNumber and j.IdJournalVoucher='26'
						 LEFT join GeneralLedger.JournalVoucherDetails jd on jd.IdAccounting = j.Id 
where i.Status='2' --and  i.InvoiceNumber='HSP1875749'
GROUP by  i.InvoiceDate,i.InvoiceDate,ad.IPCODPACI , pa.IPNOMCOMP, ad.NUMINGRES,ic.Code, ic.Name, cg.Code,cg.Name,ha.Code,cg.Name
      ,cg.EntityType 
	   ,ha.Name,cg.Name, i.InvoiceNumber, i.InvoicedUser, 
                         Pe.Fullname,i.InvoiceDate, i.TotalInvoice , i.ThirdPartySalesValue, 
                         i.Status, i.AnnulmentUser, 
                         i.AnnulmentDate, Billing.BillingReversalReason.Name, Billing.BillingReversalReason.Description
						,j.Consecutive,i.OutputDate  
