...
create VIEW [ViewInternal].[Listado_Talento_Humano]
AS
...
FROM Payroll.Employee AS E
		...
		--left outer join Common.PersonStudy as pp with (nolock) on pp.personid=p.id 
		--left outer join Payroll.StudyCenter as ps with (nolock) on ps.id=pp.studycenterid
WHERE (E.State = '1')
		AND (p.State = '1')
		AND (co.Status = '1')
		AND (co.Valid = '1')
