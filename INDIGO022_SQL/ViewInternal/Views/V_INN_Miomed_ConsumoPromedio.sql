...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: V_INN_Miomed_ConsumoPromedio
-- Extracted by Fabric SQL Extractor SPN v3.9.0


--SELECT * FROM VIE10.Inventory.Warehouse;
create VIEW [ViewInternal].[V_INN_Miomed_ConsumoPromedio]
AS
...
WHERE  CONVERT(NVARCHAR(10), D.DocumentDate, 20) > CONVERT(VARCHAR(10), GETDATE() - 367, 20) 
		AND D .STATUS = '2' 
...
GROUP BY UO.UnitName, (MONTH(D.DocumentDate)), YEAR(D.DocumentDate), pr.Code, pr.Name, sg.Code + ' - ' + sg.Name, 
		 ATC.Code, ATC.Name, pr.CodeCUM, pr.Status, pr.ProductCost, Med.Code, Med.Name, ins.Code, ins.SupplieName, 
		 gf.Name, pr.ProductControl,
		 ue.Name, ue.Abbreviation, pr.Abbreviation, Med.HighCost, F.Name, TP.Name
UNION ALL
SELECT --UO.UnitName AS Sede, 
	   CASE DO.SourceWarehouseId
...
FROM    Inventory.TransferOrder AS DO INNER JOIN
		...
		INNER JOIN Inventory.Manufacturer AS F ON F.Id = pr.ManufacturerId
	    INNER JOIN Inventory.ProductType AS TP ON TP.id = pr.ProductTypeId
WHERE do.documentdate >= '01-01-2021' --and pr.code='700003079'
...
--where Cod_Producto='19934992-07'
