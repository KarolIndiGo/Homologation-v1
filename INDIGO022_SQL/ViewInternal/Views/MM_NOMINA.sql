...
CREATE VIEW [ViewInternal].[MM_NOMINA] AS
...
L.VacationInitialDate as VacacionesDesde,
L.VacationEndDate as VacacionesHasta,
L.VacationValueEnjoy as ValorVacaciones,
L.DaysWorked as DiasLaborados,
CASE WHEN YEAR(CONT.JobBondingDate) = YEAR('11/30/2018') AND MONTH(CONT.JobBondingDate) = MONTH('12/31/2023') THEN 'X' ELSE '' END AS ING, 
CASE WHEN YEAR(CONT.RetirementDate) = YEAR('11/30/2018') AND MONTH(CONT.RetirementDate) = MONTH('12/31/2023') THEN 'X' ELSE '' END AS RET, 
'' AS TDE,
'' AS TAE,
'' AS TDP,
'' AS TAP,
...
CASE WHEN L.VacationInitialDate <> '0001-01-01' THEN 'X' ELSE '' END AS VAC,
...
(SELECT sum(AccruedValue) FROM Payroll.LiquidationDetail where PayrollId = L.Id and ConceptCode in ('001','027','924')) as SueldoPagado, 
...
(SELECT ISNULL(sum(AccruedValue),0) FROM Payroll.LiquidationDetail where PayrollId = L.Id and ConceptClass in('010') and ConceptCode <> '924' ) as OtrosDevengados, 
...
(SELECT ISNULL(sum(ConceptTotalValue),0) FROM Payroll.LiquidationDetail where PayrollId = L.Id and ConceptCode = '923') as EmbargosJudiciales,
...
L.PayrollDateLiquidated AS FechaNominaLiquidada
...
--AND CONT.Valid = 1
--AND TP.Nit IN ('1053818082')
--AND L.PayrollDateLiquidated >= '01/31/2022'
--order by L.PayrollDateLiquidated desc
--and G.Code in (05,06,09)

--select * from Payroll.[Group]
--where Code in (05,06,09)

--select * from Payroll.Concept
--where name like '%soste%'

--code 027 integral
--code 924 apoyo sostenimiento
