...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: VIE_AD_Treasury_RecibosCaja2021
-- Extracted by Fabric SQL Extractor SPN v3.9.0

create VIEW [ViewInternal].[VIE_AD_Treasury_RecibosCaja2021] as

SELECT uo.UnitName AS Sede, cr.Code AS [Nro Recibo],

CASE when T.PersonType=1 then '999' else T .Nit end as Nit , 
case when T.PersonType=1 then 'PARTICULARES' else T .Name end AS [NombreTercero], 

--t.Nit, 
t.DigitVerification AS [Dígito verificación], 
--t.Nit AS Tercero, t.Name AS NombreTercero, 
CASE cr.CollectType WHEN '1' THEN 'Caja' WHEN '2' THEN 'Bancos' END AS [Tipo de recaudo], c.Number AS Cuenta, c.Name AS [Nombre cuenta], cr.Detail AS Detalle, 
		   ...
		   --+ ' - ' + p.Fullname 
		   AS Crea, 
		   cr.CreationDate AS [Fecha crea], 
		   cr.ModificationUser 
		  -- + '-' + pm.Fullname 
		   AS Modifica, 
		   cr.ModificationDate AS [Fecha mofica], 
		   cr.AnnulmentUser
		   --+ ' - ' + pa.Fullname 
		   AS Anula, 
		   cr.AnnulmentDate AS [Fecha anula], 
		   cr.ConfirmationUser 
		   --+ ' - ' + pc.Fullname 
		   AS Confirma, 
		   ...
		   crar.InvoiceNumber as Factura, crar.value as ValorFactura
FROM   Treasury.CashReceipts AS cr  INNER JOIN
           Common.ThirdParty AS t  ON t.Id = cr.IdThirdParty INNER JOIN
           GeneralLedger.MainAccounts AS c  ON c.Id = cr.IdMainAccount LEFT OUTER JOIN
           ...
           Contract.HealthAdministrator AS harc ON harc.Id = pac.GENCONENTITY left outer join
		   Treasury.CashReceiptAccountReceivable  as  CRAR on crar.CashReceiptDetailId=dr.id
WHERE year(cr.CreationDate) >='2020'
