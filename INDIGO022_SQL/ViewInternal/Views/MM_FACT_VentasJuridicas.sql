...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: MM_FACT_VentasJuridicas
-- Extracted by Fabric SQL Extractor SPN v3.9.0



 CREATE VIEW [ViewInternal].[MM_FACT_VentasJuridicas]
AS
...
SELECT t.Nit, 
		...
		END AS sede,
            ...
            'Examenes Especializados' AS Tipo
     ...
     WHERE(c.Number in ('41250103','41250121'))
          -- OR c.Number LIKE '4170%')
         -- AND (t.PersonType = '2')
          AND (c.LegalBookId = '1')-- and t.nit='900215983'
     GROUP by t.Nit, 
              t.Name, 
              cco.Code, 
              sc.Month, 
     ...
     SELECT t.Nit, 
		...
		END AS sede,
            ...
            'Ingresos por Servicios' AS Tipo
     FROM GeneralLedger.GeneralLedgerBalance AS sc 
          INNER JOIN Common.ThirdParty AS t ON t.Id = sc.IdThirdParty
                                                                 AND sc.Year in ( '2024', '2025')
          JOIN Payroll.CostCenter AS cco ON cco.Id = sc.IdCostCenter
          JOIN GeneralLedger.MainAccounts AS c ON c.Id = sc.IdMainAccount
     WHERE (c.Number in ('41250101', '41250106','41250109','41250112', '41250115', '41250118', '41250125' )) 
        --  AND (t.PersonType = '2')
          AND (c.LegalBookId = '1') --and t.nit='900215983'
     GROUP BY t.Nit, 
              t.Name, 
