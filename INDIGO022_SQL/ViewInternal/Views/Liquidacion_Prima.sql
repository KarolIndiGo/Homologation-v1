...
--	WHERE LI.InitialContractNumber=CONT.Id AND LI.EmployeeId=E.ID AND LI.PayrollDateLiquidated between '2024-01-01' and '2024-06-30') as Valor_Recargos,
(SELECT  SUM(ConceptTotalValue) 
	FROM Payroll.Liquidation as LI 
	LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id
	WHERE   LI.EmployeeId=E.ID and LI.PayrollDateLiquidated  between '2024-01-01' and '2024-06-30' and DD.ConceptClass IN ('042','043','051','052')) as Valor_Recargos,
--(SELECT SUM(CASE WHEN DD.ConceptClass IN ('001','012','013','050') THEN ISNULL(DD.AccruedValue,0) ELSE 0 END) 
--	FROM Payroll.Liquidation LI LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id 
--	WHERE LI.InitialContractNumber=CONT.Id AND LI.EmployeeId=E.ID AND LI.PayrollDateLiquidated between '2024-01-01' and '2024-06-30') as Valor_HorasExtras,
(SELECT  SUM(ConceptTotalValue) 
	FROM Payroll.Liquidation as LI 
	LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id
	WHERE   LI.EmployeeId=E.ID and LI.PayrollDateLiquidated  between '2024-01-01' and '2024-06-30' and DD.ConceptClass IN ('001','012','013','050')) as Valor_HorasExtras,
--(SELECT SUM(CASE WHEN DD.ConceptClass IN ('010') THEN ISNULL(DD.AccruedValue,0) ELSE 0 END) 
--	FROM Payroll.Liquidation LI LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id 
--	WHERE LI.InitialContractNumber=CONT.Id AND LI.EmployeeId=E.ID AND LI.PayrollDateLiquidated between '2024-01-01' and '2024-06-30') as Valor_OtrosDevengados,
(SELECT  SUM(ConceptTotalValue) 
	FROM Payroll.Liquidation as LI 
	LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id
	WHERE   LI.EmployeeId=E.ID and LI.PayrollDateLiquidated  between '2024-01-01' and '2024-06-30' and DD.ConceptClass IN ('010')) as Valor_OtrosDevengados,
--((SELECT SUM(CASE WHEN DD.ConceptClass IN ('042','043','051','052','001','012','013','050') THEN ISNULL(DD.AccruedValue,0) ELSE 0 END) 
--	FROM Payroll.Liquidation LI LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id 
--	WHERE LI.InitialContractNumber=CONT.Id AND LI.EmployeeId=E.ID AND LI.PayrollDateLiquidated between '2024-01-01' and '2024-06-30')/L.WorkingDays)*30  as Valor_Promedio,
((SELECT  SUM(ConceptTotalValue) 
	FROM Payroll.Liquidation as LI 
	LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id
	WHERE   LI.EmployeeId=E.ID and LI.PayrollDateLiquidated  between '2024-01-01' and '2024-06-30' and DD.ConceptClass IN ('042','043','051','052'))/L.WorkingDays)*30  as Valor_Promedio, l.WorkingDays,
...
--	FROM Payroll.Liquidation as LI 
--	LEFT JOIN Payroll.LiquidationDetail DD ON DD.PayrollId = LI.Id
--	WHERE  LI.PayrollDateLiquidated  between '2024-01-01' and '2024-06-30'
