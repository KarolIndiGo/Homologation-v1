...
CREATE VIEW [ViewInternal].[IMO_NOMINA] AS
SELECT 
...
(SELECT sum(AccruedValue) FROM Payroll.LiquidationDetail where PayrollId = L.Id and ConceptCode in ('001','027','929')) as SueldoPagado, --se agregan el ConceptCode 027 Integral y 929 y APRENDIZ
...
(SELECT ISNULL(sum(AccruedValue),0) FROM Payroll.LiquidationDetail where PayrollId = L.Id and ConceptClass in('010') and ConceptCode <> '929' ) as OtrosDevengados, --se excluye 929 apoyo sostenimiento caso 341824
