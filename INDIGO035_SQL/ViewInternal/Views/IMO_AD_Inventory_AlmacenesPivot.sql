...
-- ItemId: SPN
-- Schema: ViewInternal
-- Object: IMO_AD_Inventory_AlmacenesPivot
...
FROM (SELECT PR.Code
 ...
 WHEN '7'	then 'ACTIVO' 
      ...
      FROM Inventory.InventoryProduct AS PR
           INNER JOIN Billing.BillingGroup AS GF
              ON GF.Id = PR.BillingGroupId
           LEFT OUTER JOIN
           Inventory.PhysicalInventory AS INF
              ON INF.ProductId = PR.Id
           LEFT OUTER JOIN Inventory.Warehouse AS AL
              ON AL.Id = INF.WarehouseId
           LEFT OUTER JOIN Inventory.ATC AS ATC
              ON ATC.Id = PR.ATCId
           LEFT OUTER JOIN
           [Inventory].[InventorySupplie] AS s
              ON s.id = pr.SupplieId
           LEFT OUTER JOIN
           Inventory.ProductSubGroup AS SG
