...
create VIEW [ViewInternal].[IND_AD_AG_OportunidadAgenda_PB]
AS
...
ACT.ESTADOACT, ACT.DESACTMED, ACT.DURAACTIV as DuracionActividad,
 CASE
                WHEN a.codcenate IN(002, 003, 004, 005, 006, 007, 008, 009,017)
                THEN 'BOYACA'
                WHEN a.codcenate IN(010, 011, 012, 013, 014)
                THEN 'META'
                WHEN a.codcenate IN (015,016)
                THEN 'CASANARE'
            END AS Regional
     
from  [dbo].[AGAGEMEDC] AS A
INNER JOIN [dbo].[AGAGEMEDD] AS AD ON AD.CODAUTONU=A.CODAUTONU
INNER JOIN [dbo].[ADCENATEN] AS CA ON CA.CODCENATE=A.CODCENATE
--INNER JOIN [dbo].[INPROFSAL] AS P ON P.CODPROSAL=A.CODPROSAL
INNER JOIN [dbo].[INESPECIA] AS E ON E.CODESPECI=a.codespeci
LEFT OUTER JOIN [dbo].[AGASICITA] AS C ON C.IDAGENDA=A.CODAUTONU AND C.CODESTCIT IN ('0','1')
LEFT OUTER JOIN [dbo].[AGACTIMED] AS ACT ON ACT.CODACTMED=C.CODACTMED
WHERE CONVERT(DATE,A.FECHORAIN)>=CONVERT(DATE,GETDATE()) )
