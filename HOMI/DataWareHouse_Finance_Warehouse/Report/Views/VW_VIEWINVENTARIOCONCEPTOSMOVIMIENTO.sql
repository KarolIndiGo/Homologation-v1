...
(
    ...
    INNER JOIN INDIGO036.Billing.BasicBilling B 
        ON K.EntityId = B.Id AND K.EntityName = 'BasicBilling'
    ...
    FROM INDIGO036.Inventory.EntranceVoucherDetail ED
    INNER JOIN INDIGO036.Inventory.EntranceVoucher E 
        ON ED.EntranceVoucherId = E.Id
    INNER JOIN INDIGO036.Payments.AccountPayable CP 
        ON E.Id = CP.EntityId AND CP.EntityName = 'EntranceVoucher'
    INNER JOIN INDIGO036.Inventory.Kardex K 
        ON K.Id = (
            SELECT MAX(A.Id) 
            FROM INDIGO036.Inventory.Kardex A 
            WHERE E.Id = A.EntityId 
              AND ED.ProductId = A.ProductId 
              AND ED.Quantity = A.Quantity
        )
    LEFT JOIN INDIGO036.Inventory.RemissionEntranceDetailBatchSerial REDBS 
        ON ED.RemissionEntranceDetailBatchSerialId = REDBS.Id
    LEFT JOIN INDIGO036.Inventory.RemissionEntranceDetail RD 
        ON REDBS.RemissionEntranceDetailId = RD.Id
    LEFT JOIN INDIGO036.Inventory.RemissionEntrance R 
        ON RD.RemissionEntranceId = R.Id
    ...
    SELECT 
        ...
        CONVERT(NUMERIC(18,2),
            IIF(DEV.IvaPercentage = 0, DEV.UnitValue * EVDD.Quantity,
    ...
    FROM INDIGO036.Inventory.EntranceVoucherDevolutionDetail EVDD
    INNER JOIN INDIGO036.Inventory.EntranceVoucherDevolution EVD 
        ON EVDD.EntranceVoucherDevolutionId = EVD.Id AND EVDD.Quantity <> 0
    INNER JOIN INDIGO036.Inventory.EntranceVoucherDetailBatchSerial EVDBS 
        ON EVDD.EntranceVoucherDetailBatchSerialId = EVDBS.Id
    INNER JOIN INDIGO036.Inventory.EntranceVoucherDetail DEV 
        ON EVDBS.EntranceVoucherDetailId = DEV.Id
    INNER JOIN INDIGO036.Inventory.EntranceVoucher EV 
        ON DEV.EntranceVoucherId = EV.Id
    INNER JOIN INDIGO036.Inventory.Kardex K 
        ON K.Id = (
            SELECT MAX(A.Id) 
            FROM INDIGO036.Inventory.Kardex A 
            WHERE EVD.Id = A.EntityId 
              AND DEV.ProductId = A.ProductId 
              AND A.EntityName = 'EntranceVoucherDevolution'
        )
    LEFT JOIN INDIGO036.Payments.PaymentNotes PN 
        ON EVD.PaymentNoteId = PN.Id
    ...
    SELECT 
        ...
        NULL AS [VALOR REMISION],
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.InventoryAdjustment AI
    LEFT JOIN INDIGO036.Inventory.InventoryAdjustmentDetail AID 
        ON AI.Id = AID.InventoryAdjustmentId
    LEFT JOIN INDIGO036.Inventory.InventoryAdjustmentControl IAC 
        ON AI.Id = IAC.InventoryAdjustmentId
    LEFT JOIN INDIGO036.Inventory.InventoryControlDetailBatchSerial ICDBS 
        ON IAC.InventoryControlDetailBatchSerialId = ICDBS.Id
    LEFT JOIN INDIGO036.Inventory.InventoryControlDetail ICD 
        ON ICDBS.InventoryControlDetailId = ICD.Id
    LEFT JOIN INDIGO036.Inventory.Kardex K 
        ON K.Id = (
            SELECT MAX(A.Id) 
            FROM INDIGO036.Inventory.Kardex A 
            WHERE (AI.Id = A.EntityId AND AID.ProductId = A.ProductId AND AID.Quantity = A.Quantity AND A.EntityName='InventoryAdjustment')
               OR (AI.Id = A.EntityId AND ICD.ProductId = A.ProductId AND IAC.QuantityAdjustment = A.Quantity AND A.EntityName='InventoryAdjustment')
        )
    LEFT JOIN INDIGO036.Inventory.AdjustmentConcept AC 
        ON ISNULL(AI.AdjustmentConceptId, AID.AdjustmentConceptId) = AC.Id
    ...
    SELECT 
        ...
        NULL AS [VALOR REMISION],
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.Kardex 
    WHERE EntityName = 'InventoryControl'
    ...
    SELECT 
        ...
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.Kardex 
    WHERE EntityName = 'LoanMerchandise'
    ...
    SELECT 
        ...
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.Kardex 
    WHERE EntityName = 'LoanMerchandiseDevolution'
    ...
    SELECT DISTINCT
        ...
        NULL AS [VALOR REMISION],
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Billing.ServiceOrder SO
    INNER JOIN INDIGO036.Billing.ServiceOrderDetail SOD 
        ON SO.Id = SOD.ServiceOrderId 
    ...
    INNER JOIN INDIGO036.Inventory.Kardex K 
        ON K.Id = (
            SELECT MAX(A.Id) 
            FROM INDIGO036.Inventory.Kardex A 
            WHERE SO.EntityId = A.EntityId 
              AND SOD.ProductId = A.ProductId 
              AND SOD.SupplyQuantity = A.Quantity 
              AND A.EntityName = 'PharmaceuticalDispensing'
        )
    LEFT JOIN (
        SELECT A.Id, ROW_NUMBER () OVER (PARTITION BY A.ProductId, C.EntityId ORDER BY A.ProductId, A.Id DESC) AS NUMERO, B.WarehouseId
        FROM INDIGO036.Billing.ServiceOrder C
        INNER JOIN INDIGO036.Billing.ServiceOrderDetail A ON C.Id = A.ServiceOrderId
        INNER JOIN INDIGO036.Inventory.Kardex B ON C.EntityId = B.EntityId AND A.ProductId = B.ProductId AND A.SupplyQuantity = B.Quantity AND B.EntityName='PharmaceuticalDispensing'
    ) ALM1 ON SOD.Id = ALM1.Id AND ALM1.NUMERO = 1
    LEFT JOIN (
        SELECT A.Id, ROW_NUMBER () OVER (PARTITION BY A.ProductId, C.EntityId ORDER BY A.ProductId, A.Id DESC) AS NUMERO, B.WarehouseId
        FROM INDIGO036.Billing.ServiceOrder C
        INNER JOIN INDIGO036.Billing.ServiceOrderDetail A ON C.Id = A.ServiceOrderId
        INNER JOIN INDIGO036.Inventory.Kardex B ON C.EntityId = B.EntityId AND A.ProductId = B.ProductId AND A.SupplyQuantity = B.Quantity AND B.EntityName='PharmaceuticalDispensing'
    ) ALM2 ON SOD.Id = ALM2.Id AND ALM2.NUMERO = 4
    ...
    SELECT 
        ...
        NULL AS [VALOR REMISION],
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.PharmaceuticalDispensingDevolution PDD
    INNER JOIN (
        SELECT DEV.PharmaceuticalDispensingDevolutionId, SUM(DEV.Quantity) AS Quantity, SUM(DEV.TOTAL) AS TOTAL, DEV.FunctionalUnitId, DEV.ProductId
        FROM (
            SELECT 
                ...
                DFD.ProductId
            FROM INDIGO036.Inventory.PharmaceuticalDispensingDevolutionDetail PDDD 
    ...
    SELECT 
        ...
        K.WarehouseId, K.ProductId, K.DocumentDate, K.Quantity, K.[Value], K.AverageCost,
        CONVERT(NUMERIC(18,2),
            IIF(RED.UnitValue IS NOT NULL,
                IIF(RED.IvaPercentage = 0, RDD.Quantity * RED.UnitValue, (RED.UnitValue + (RED.UnitValue * RED.IvaPercentage) / 100) * RDD.Quantity),
        ...
        NULL AS [VALOR REMISION],
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.RemissionDevolutionDetail RDD
    ...
    LEFT JOIN INDIGO036.Inventory.Kardex K 
        ON K.Id = (
            SELECT MAX(A.Id) 
            FROM INDIGO036.Inventory.Kardex A 
            WHERE RDD.RemissionDevolutionId = A.EntityId 
    ...
    SELECT 
        ...
        K.WarehouseId, K.ProductId, K.DocumentDate, K.Quantity, K.[Value], K.AverageCost,
        CONVERT(NUMERIC(18,2),
            IIF(RED.IvaPercentage = 0, RED.Quantity * RED.UnitValue,
        ...
        NULL AS [VALOR REMISION],
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.RemissionEntranceDetail RED 
    INNER JOIN INDIGO036.Inventory.RemissionEntrance RE ON RED.RemissionEntranceId = RE.Id
    INNER JOIN INDIGO036.Inventory.Kardex K 
        ON K.Id = (
            SELECT MAX(A.Id) 
            FROM INDIGO036.Inventory.Kardex A 
            WHERE RED.RemissionEntranceId = A.EntityId 
    ...
    SELECT 
        ...
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.Kardex K 
    INNER JOIN INDIGO036.Inventory.TransferOrder OT 
        ON K.EntityId = OT.Id AND K.EntityName = 'RemissionOutput'
    LEFT JOIN INDIGO036.Inventory.AdjustmentConcept AC 
        ON OT.AdjustmentConceptId = AC.Id
    ...
    FROM INDIGO036.Inventory.Kardex K 
    INNER JOIN INDIGO036.Inventory.TransferOrder OT 
        ON K.EntityId = OT.Id AND K.EntityName = 'TransferOrder'
    LEFT JOIN INDIGO036.Inventory.AdjustmentConcept AC 
        ON OT.AdjustmentConceptId = AC.Id
    LEFT JOIN INDIGO036.Payroll.FunctionalUnit FU 
        ON OT.TargetFunctionalUnitId = FU.Id
    ...
    SELECT 
        ...
        NULL AS [CUENTA X PAGAR / NOTA]
    FROM INDIGO036.Inventory.Kardex K 
    INNER JOIN INDIGO036.Inventory.TransferOrderDevolution TOD 
        ON K.EntityId = TOD.Id AND K.EntityName = 'TransferOrderDevolution'
    INNER JOIN INDIGO036.Inventory.TransferOrder OT 
        ON TOD.TransferOrderId = OT.Id
    LEFT JOIN INDIGO036.Payroll.FunctionalUnit FUN 
        ON OT.TargetFunctionalUnitId = FUN.Id
)
...
SELECT
    ...
    CONCAT(FORMAT(MONTH(CON.DocumentDate), '00'), ' - ', 
           CASE MONTH(CON.DocumentDate) WHEN 1 THEN 'ENERO'
                                        ...
                                        WHEN 10 THEN 'OCTUBRE'
                                        WHEN 11 THEN 'NOVIEMBRE'
                                        WHEN 12 THEN 'DICIEMBRE' END) AS MES_LABEL_INGRESO,
    CONVERT(DATETIME, GETDATE() AT TIME ZONE 'Pakistan Standard Time', 1) AS ULT_ACTUAL
FROM CTE_CONCEPTOS CON
...
LEFT JOIN INDIGO036.Inventory.EntranceVoucherDetail CED 
    ON P.Id = CED.ProductId 
   AND CED.Id = (SELECT MAX(A.Id) FROM INDIGO036.Inventory.EntranceVoucherDetail A WHERE P.Id = A.ProductId)
...
LEFT JOIN INDIGO036.Inventory.ProductGroupFunctionalUnit FU 
    ON GRU.Id = FU.ProductGroupId AND CON.FunctionalUnitId = FU.FunctionalUnitId
