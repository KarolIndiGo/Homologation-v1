...
-- Schema: Report
-- Object: VW_URGENCIAS_CRUE_2
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW Report.VW_URGENCIAS_CRUE_2 AS 
WITH CTE_MUERTOS AS (
    SELECT 
    ...
    FROM 
        ...
        INNER JOIN [INDIGO036].dbo.HCREGEGRE AS EGR ON HIS.NUMINGRES = EGR.NUMINGRES AND HIS.NUMEFOLIO = EGR.NUMEFOLIO
...
CTE_ORDENES_CON_ESTANCIA_RECONOCIDAS AS (
    ...
    FROM 
        ...
        INNER JOIN [INDIGO036].[Billing].[ServiceOrderDetailDistribution] AS SODD ON RCD.Id = SODD.RevenueControlDetailId AND RCD.Status IN ('1','2','3')
        INNER JOIN [INDIGO036].[Billing].[ServiceOrderDetail] AS SOD ON SODD.ServiceOrderDetailId = SOD.Id
        INNER JOIN [INDIGO036].[Contract].[CUPSEntity] AS CUPS ON SOD.CUPSEntityId = CUPS.Id AND CUPS.BillingGroupId IN (2)
    WHERE 
        ...
        INNER JOIN (
            ...
            FROM (
                ...
                FROM 
                    ...
                    INNER JOIN [INDIGO036].[Billing].[ServiceOrderDetailDistribution] AS SODD ON RCD.Id = SODD.RevenueControlDetailId AND RCD.Status IN ('1','2','3')
                    INNER JOIN [INDIGO036].[Billing].[ServiceOrderDetail] AS SOD ON SODD.ServiceOrderDetailId = SOD.Id
                    INNER JOIN [INDIGO036].Contract.CUPSEntity AS CUPS ON CUPS.Id = SOD.CUPSEntityId AND CUPS.BillingGroupId IN (2)
        ...
        INNER JOIN (
            ...
            FROM (
                ...
                FROM 
                    ...
                    INNER JOIN [INDIGO036].[Billing].[ServiceOrderDetailDistribution] AS SODD ON RCD.Id = SODD.RevenueControlDetailId AND RCD.Status IN ('1','2','3')
                    INNER JOIN [INDIGO036].[Billing].[ServiceOrderDetail] AS SOD ON SODD.ServiceOrderDetailId = SOD.Id
                    INNER JOIN [INDIGO036].Contract.CUPSEntity AS CUPS ON CUPS.Id = SOD.CUPSEntityId AND CUPS.BillingGroupId IN (7,8)
                    INNER JOIN [INDIGO036].Contract.IPSService AS IPS ON IPS.Id = SOD.IPSServiceId
                WHERE 
                    CUPS.Code IN ('10A004', '5DSA01')
                GROUP BY 
                    RC.AdmissionNumber, CUPS.Code, CUPS.Description
            ) AS SourceTable
            PIVOT (
                MAX([CUPS]) 
                FOR [DESCRIPCION_CUPS] IN (
...
, CTE_TRIAGE_UNICO_POR_ATENCIÓN AS (
    ...
    WHERE 
        A.ID = (
            SELECT MAX(B.ID)
            FROM [INDIGO036].dbo.ADTRIAGEU B
            WHERE A.IPCODPACI = B.IPCODPACI 
...
CTE_HCURGING1 AS (
    ...
    WHERE 
        K.NUMEFOLIO = (
            SELECT MIN(CAST(FOL.NUMEFOLIO AS INT))
            FROM [INDIGO036].dbo.HCURGING1 FOL
            WHERE K.NUMINGRES = FOL.NUMINGRES
        )
)
SELECT
    CAST(DB_NAME() AS VARCHAR(9)) AS ID_COMPANY,
    CEN.NOMCENATE AS [CENTRO DE ATENCIÃ“N],
    A.ID,
    ...
    CASE 
        WHEN CONVERT(CHAR(8), A.TRIAFECHA, 108) BETWEEN '07:00:00' AND '12:59:59' THEN 'MAÃ‘ANA'
        ...
        WHEN (
            ...
            ) AND DAY(A.TRIAFECHA) % 2 = 0 THEN 'NOCHE DÃA PAR'
        WHEN (
                (CONVERT(CHAR(8), A.TRIAFECHA, 108) BETWEEN '19:00:00' AND '23:59:59')
                OR (CONVERT(CHAR(8), A.TRIAFECHA, 108) BETWEEN '00:00:00' AND '06:59:59')
            ) AND DAY(A.TRIAFECHA) % 2 = 1 THEN 'NOCHE DÃA IMPAR'
        ELSE ''
    END AS JORNADA,
    ...
    CASE 
        WHEN TRA.MEDIOTRASPORTE = 1 THEN 'BÃ¡sica'
    ...
    ISNULL(
        CASE k.INDICAPAC
            ...
            WHEN 2 THEN 'Trasladar a ObservaciÃ³n Urgencias'
            WHEN 3 THEN 'Trasladar a HospitalizaciÃ³n'
            WHEN 7 THEN 'Trasladar a Consulta Externa'
            WHEN 9 THEN 'HospitalizaciÃ³n en Casa'
            WHEN 10 THEN 'Referencia'
            WHEN 11 THEN 'Morgue'
            WHEN 12 THEN 'Salida'
            WHEN 13 THEN 'ContinÃºa en la Unidad'
    ...
    1 AS CANTIDAD,
    CAST(ISNULL(C.IPFECLLEGA, ISNULL(I.IFECHAING, A.TRIAFECHA)) AS DATE) AS FECHA_BUSQUEDA,
    YEAR(ISNULL(C.IPFECLLEGA, ISNULL(I.IFECHAING, A.TRIAFECHA))) AS AÑO_BUSQUEDA,
    MONTH(ISNULL(C.IPFECLLEGA, ISNULL(I.IFECHAING, A.TRIAFECHA))) AS MES_BUSQUEDA,
    CONCAT(
...
FROM 
    ...
    LEFT JOIN [INDIGO036].dbo.HCHISPACA HC ON A.NUMINGRES = HC.NUMINGRES 
        AND HC.FECHISPAC = (SELECT MIN(H.FECHISPAC) FROM [INDIGO036].dbo.HCHISPACA H WHERE A.NUMINGRES = H.NUMINGRES)
    ...
    LEFT JOIN [INDIGO036].dbo.HCREGEGRE EGR ON I.NUMINGRES = EGR.NUMINGRES 
        AND EGR.NUMEFOLIO = (SELECT MAX(E.NUMEFOLIO) FROM [INDIGO036].dbo.HCREGEGRE E WHERE I.NUMINGRES = E.NUMINGRES)
...
WHERE 
    UNF.UFUCODIGO IN ('1111001', '1111002');