...
-- Schema: Report
-- Object: VW_VIEWOPPORTUNITYQX
-- Extracted by Fabric SQL Extractor SPN v3.9.0

CREATE VIEW Report.VW_VIEWOPPORTUNITYQX AS
    ...
    FROM
        ...
        INNER JOIN (
            ...
            GROUP BY
                NUMINGRES,
                RCX.CODCIRUGI
        ) ANE1 ON ANE1.IDANESTES = ANE.IDANESTES
...
CTE_PROCEDIMIENTOS AS (
    SELECT
        ...
        1 AS CANTIDAD
    FROM
        INDIGO036.dbo.HCQXREALI REA
        INNER JOIN INDIGO036.Contract.CUPSEntity CUPS ON REA.CODSERIPS = CUPS.Code
        AND REA.QXPRINCIP = 1
    UNION ALL
    SELECT
        ROW_NUMBER () OVER (
            PARTITION BY REA.NUMINGRES,
            REA.NUMEFOLIO
            order by
                REA.CONSECUQX DESC
        ) AS NUMERO,
...
SELECT
    ...
    END [TIPO CONTRATO],
    CASE
        WHEN PR.ORIGENQX = 1 THEN 'ORIGEN AMBULATORIO'
        WHEN PR.ORIGENQX = 2 THEN 'ORIGEN HOSPITALARIO'
        WHEN ING.TIPOINGRE = 1 THEN 'ORIGEN AMBULATORIO'
        WHEN ING.TIPOINGRE = 2 THEN 'ORIGEN HOSPITALARIO'
    END AS AMBITO,
    ...
    CASE
        ...
        ELSE SOL.PRISERIPS
    END AS [PRIORIDAD SOLICITUD],
    CASE
        QX.URGECIRUG
        WHEN 1 THEN 'SI'
        WHEN 0 THEN 'NO'
    END AS [MARCADO URGENCIA INFORME],
    'PRINCIPAL' AS [TIPO DE PRODCEDIMIENTO QX],
    CASE
        TIPOINFORME
        WHEN 1 THEN 'Informe QX'
        WHEN 2 THEN 'Informe no QX'
    END AS [TIPO DE INFORME],
    ...
    RAD.NUMRADICACION AS [NUMERO RADICACION],
    RAD.FECHARADIC AS [FECHA RADICADO],
    RAD.FECHACONFIRMACION AS [FECHA CONFIRMACION RADICACION],
    ISNULL(SOL.FECORDMED, NONQX.FECORDMED) AS [FECHA SOLICITUD ORDEN],
    ...
    DATEDIFF(
        MINUTE,
        ISNULL(SOL.FECORDMED, NONQX.FECORDMED),
        QX.FECHORINI
    ) AS [ORDEN VS INICIO QX, MINUTOS],
    CASE
        SAL.TIPOSALA
        WHEN 'Q' THEN 'Quirurgica'
        WHEN 'D' THEN 'Apoyo Diagnostico'
        WHEN 'U' THEN 'Urgencias'
        WHEN 'E' THEN 'Tratamientos Especiales'
        ELSE 'NO APLICA'
    END [TIPO SALA],
    ISNULL(
        SAL.DESCRIPSAL,
    ...
    INE.CODESPECI AS [CODIGO ESPECIALIDAD CIRUJANO],
    INE.DESESPECI AS [ESPECIALIDAD CIRUJANO],
    CASE
        QX.TIPHERCIR
        WHEN 1 THEN 'LIMPIA'
    ...
    CONCAT(
        FORMAT(MONTH(ISNULL(QX.FECHORINI, PR.FECHORAIN)), '00'),
        ' - ',
        CASE
            MONTH(ISNULL(QX.FECHORINI, PR.FECHORAIN))
            WHEN 1 THEN 'ENERO'
            WHEN 2 THEN 'FEBRERO'
            WHEN 3 THEN 'MARZO'
            WHEN 4 THEN 'ABRIL'
...
FROM
    ...
    INNER JOIN INDIGO036.dbo.HCHISPACA HC ON QX.NUMINGRES = HC.NUMINGRES
        AND QX.NUMEFOLIO = HC.NUMEFOLIO
        AND HC.ESTAFOLIO = 1
    INNER JOIN INDIGO036.dbo.HCQXEQUIP CX ON QX.NUMINGRES = CX.NUMINGRES
        AND QX.NUMEFOLIO = CX.NUMEFOLIO
        AND CX.QXPRINCIP = 1
    ...
    LEFT JOIN INDIGO036.dbo.HCORDPROQ SOL ON QX.NUMINGRES = SOL.NUMINGRES
        AND SOL.AUTO =(
            SELECT
                MAX(A.AUTO)
            FROM
            ...
            WHERE
                A.NUMINGRES = QX.NUMINGRES
                AND QX.CODSERIPS = A.CODSERIPS
        )
    LEFT JOIN INDIGO036.dbo.HCORDPRON NONQX ON QX.NUMINGRES = NONQX.NUMINGRES
        AND NONQX.AUTO =(
            SELECT
                MAX(A.AUTO)
            FROM
    ...
    LEFT JOIN INDIGO036.dbo.AGEPROGQX PR ON SOL.AUTOCITA = PR.CODAUTONU
    LEFT JOIN INDIGO036.dbo.INESPECIA INE ON PRO.CODESPEC1 = INE.CODESPECI
    LEFT JOIN INDIGO036.dbo.ADRADICACIONQX RAD ON PR.IDRADICACIONQX = RAD.ID
        AND RAD.ESTADO != 3
    LEFT JOIN CTE_PROCEDIMIENTOS PRI ON QX.NUMINGRES = PRI.NUMINGRES
        AND QX.NUMEFOLIO = PRI.NUMEFOLIO
        AND PRI.TIPO = 1
    LEFT JOIN CTE_PROCEDIMIENTOS SEC1 ON QX.NUMINGRES = SEC1.NUMINGRES
        AND QX.NUMEFOLIO = SEC1.NUMEFOLIO
        AND SEC1.TIPO = 2
        AND SEC1.NUMERO = 1
    LEFT JOIN CTE_PROCEDIMIENTOS SEC2 ON QX.NUMINGRES = SEC2.NUMINGRES
        AND QX.NUMEFOLIO = SEC2.NUMEFOLIO
        AND SEC2.TIPO = 2
        AND SEC2.NUMERO = 2
    LEFT JOIN CTE_PROCEDIMIENTOS SEC3 ON QX.NUMINGRES = SEC3.NUMINGRES
        AND QX.NUMEFOLIO = SEC3.NUMEFOLIO
        AND SEC3.TIPO = 2
        AND SEC3.NUMERO = 3
    LEFT JOIN CTE_PROCEDIMIENTOS SEC4 ON QX.NUMINGRES = SEC4.NUMINGRES
        AND QX.NUMEFOLIO = SEC4.NUMEFOLIO
        AND SEC4.TIPO = 2
        AND SEC4.NUMERO = 4
    LEFT JOIN CTE_PROCEDIMIENTOS SEC5 ON QX.NUMINGRES = SEC5.NUMINGRES
        AND QX.NUMEFOLIO = SEC5.NUMEFOLIO
        AND SEC5.TIPO = 2
        AND SEC5.NUMERO = 5
    LEFT JOIN CTE_PROCEDIMIENTOS SEC6 ON QX.NUMINGRES = SEC6.NUMINGRES
        AND QX.NUMEFOLIO = SEC6.NUMEFOLIO
        AND SEC6.TIPO = 2
        AND SEC6.NUMERO = 6
    LEFT JOIN CTE_PROCEDIMIENTOS SEC7 ON QX.NUMINGRES = SEC7.NUMINGRES
        AND QX.NUMEFOLIO = SEC7.NUMEFOLIO
        AND SEC7.TIPO = 2
        AND SEC7.NUMERO = 7
    LEFT JOIN CTE_PROCEDIMIENTOS SEC8 ON QX.NUMINGRES = SEC8.NUMINGRES
        AND QX.NUMEFOLIO = SEC8.NUMEFOLIO
        AND SEC8.TIPO = 2
        AND SEC8.NUMERO = 8
    LEFT JOIN CTE_PROCEDIMIENTOS SEC9 ON QX.NUMINGRES = SEC9.NUMINGRES
        AND QX.NUMEFOLIO = SEC9.NUMEFOLIO
        AND SEC9.TIPO = 2
        AND SEC9.NUMERO = 9
    LEFT JOIN CTE_PROCEDIMIENTOS SEC10 ON QX.NUMINGRES = SEC10.NUMINGRES
        AND QX.NUMEFOLIO = SEC10.NUMEFOLIO
        AND SEC10.TIPO = 2
        AND SEC10.NUMERO = 10
    LEFT JOIN CTE_PROCEDIMIENTOS SEC11 ON QX.NUMINGRES = SEC11.NUMINGRES
        AND QX.NUMEFOLIO = SEC11.NUMEFOLIO
        AND SEC11.TIPO = 2
        AND SEC11.NUMERO = 11
    LEFT JOIN CTE_PROCEDIMIENTOS SEC12 ON QX.NUMINGRES = SEC12.NUMINGRES
        AND QX.NUMEFOLIO = SEC12.NUMEFOLIO
        AND SEC12.TIPO = 2
        AND SEC12.NUMERO = 12
    LEFT JOIN CTE_PROCEDIMIENTOS SEC13 ON QX.NUMINGRES = SEC13.NUMINGRES
        AND QX.NUMEFOLIO = SEC13.NUMEFOLIO
        AND SEC13.TIPO = 2
        AND SEC13.NUMERO = 13
    LEFT JOIN CTE_PROCEDIMIENTOS SEC14 ON QX.NUMINGRES = SEC14.NUMINGRES
        AND QX.NUMEFOLIO = SEC14.NUMEFOLIO
        AND SEC14.TIPO = 2
        AND SEC14.NUMERO = 14
    LEFT JOIN CTE_PROCEDIMIENTOS SEC15 ON QX.NUMINGRES = SEC15.NUMINGRES
        AND QX.NUMEFOLIO = SEC15.NUMEFOLIO
        AND SEC15.TIPO = 2
        AND SEC15.NUMERO = 15
    LEFT JOIN CTE_ANESTESIA ANE ON QX.NUMINGRES = ANE.NUMINGRES
        AND QX.CODSERIPS = ANE.CUPS
    ...
    LEFT JOIN CTE_CUENTA_PROCEDIMIENTOS CONTAR ON QX.NUMINGRES = CONTAR.NUMINGRES
        AND QX.NUMEFOLIO = CONTAR.NUMEFOLIO